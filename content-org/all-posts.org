...
thoughts on design and preventing massive breakage. In particular, using PRs
turned out to be insufficient to maintain code quality and I ended up shifting
to a submodule based workflow.

Originally, I had the library and bindings in one repository, which was easier
to work hack on, and had fewer moving parts. I rashly assumed that using
branches would provide a modicum of reproducibility. As I quickly found though,
this got out of hand when the code needed to be refactored to suit the bindings
and wasn't always backported to ~main~ in a timely fashion. In a sense, this is
another introduction to yet another ~cookiecutter~ template, but the post should
provide context behind the decisions.
** Overview
** Layout
We will use ~meson~ throughout
*** Library
For the library itself

* TODO Python Bindings for CPP Libraries
** Background
I've been writing a lot of Python bindings lately. Mostly thin wrappers to
existing libraries. These are some guidelines I keep reaching for. Most of these
evolved during my work on ~eON~, the long timescale dynamics code, but isn't (as
of April 2024) public yet[fn:academicFOSS].
...
project (library, bindings), which is small and innocuous enough to grok hopefully.
*** Tools
For most new projects, I use ~pybind11~ [fn:nanobind] with ~meson~.
- ~pdm~ is used for dependencies
- ~mesonpy~ is the backend
- ~cibuildwheel~ is used for actually getting the wheels for ~pypi~
  + This is a bit of a nightmare, need to use [[https://repology.org/project/eigen/versions][repology]] and fiddle around with figuring out which version of each OS is used by each specification
...
~meson~ doesn't support ~python~ as a first class language. The general idea is to muck around with the fact that copying over ~.py~ files to the "right place" is essentially an installation.
As seen in ~scipy~:
#+begin_src python
py3.install_sources([
    '__init__.py',
...
#+end_src
Which is, exciting in a way. ~macos_arm64~ is a cross compile currently and so not a good idea. ~numpy~ has a nice build wheels only if commit contains [wheel build] which is nice to kang.
** Details
