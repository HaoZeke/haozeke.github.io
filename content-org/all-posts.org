...
* TODO Mach-Nix for Nix Python and Niv :@programming:tools:nix:workflow:python:
:PROPERTIES:
...
Note that we have essentially written out a ~requirements.txt~ and can actually pass a path there instead as well. The key point to make it work with ~niv~ is the ~pkgs~ parameter.
