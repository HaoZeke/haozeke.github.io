...
Nick Wogan]].
** Meson
As ~scipy~ [[https://labs.quansight.org/blog/2021/07/moving-scipy-to-meson/][moves towards meson]] mainly driven by [[https://rgommers.github.io/][Ralf Gommers]], it makes sense to look into its use to drive the compilation of an extension module as well.
*** Python C Extensions
*** ~f2py~ Wrapper
We start by generating the files for the interface.
#+begin_src bash
f2py fib1.f -m fib
...
** Standard Bindings
The standard compliant approach was aptly covered almost a decade ago by [[https://ondrejcertik.com/][Ondřej Čertík]] and codified in [[https://www.fortran90.org/src/best-practices.html#python-interface][fortran90's best practices guide]]. We start by updating our free-form code to be compliant with newer ~ISO_C_BINDING~ guidelines.
#+begin_src f90
module fib1
  use iso_c_binding
  implicit none
...
This however, is closer to ~f2py~ in the sense that the ~C~ code is simply hidden. It requires a compilation step typically done via ~setuptools.py~, however this is much nicer with ~meson~ since from ~0.59.0~, [[https://mesonbuild.com/Cython.html
][Cython is supported]].
#+begin_src python
project('test_builds', 'c',
  version : '0.1',
...
cd bdircython
python -c "import fibby; import numpy as np; a=np.empty(15); fibby.fib(a[0],15); print(a); exit();"
#+end_src
...
module, although so it makes more sense to actually write a proper ~Python-C~ as
a more fair comparison. This however, involves quite some reference counting and
familiarity with the [[https://numpy.org/devdocs/user/c-info.how-to-extend.html][NumPy-C API]]. Actually it is not too bad for the function we
are considering. Even the well commented hand crafted version clocks in below 90
lines of code.
#+begin_src C
...
| Handwritten NumPy-C-Fortran | 146.6 ± 8.9  |      125.8 |      155.0 |
| F2PY (F77)                  | 149.3 ± 1.7  |      146.1 |      152.7 |
| Cython                      | 150.0 ± 13.1 |      141.6 |      198.0 |
| F2PY (F90)                  | 150.2 ± 2.4  |      145.3 |      155.1 |
| ctypes                      | 150.5 ± 2.8  |      147.1 |      158.8 |

As expected, ~ctypes~ is the slowest, since it requires the most machinery from
the Python side. Surprisingly, loading the free form file with ~f2py~ was quite
a bit slower than the ~f77~ file. Given the complexity of the files involved, it
is also not surprising that the handwritten wrapper has higher performance.
** Conclusions
...
automatically providing optimal bindings for users as initially planned by [[https://github.com/pearu][Pearu
Peterson]]. Modernization efforts with  [[https://melissawm.github.io/about-me/][Melissa Weber Mendonça]] and others at
Quansight (including [[https://rgoswami.me/about][me]]) are sure to gravitate towards generating better, more
reliable interfaces to bridge the gaps between Fortran and Python as supported
by the NumPy core developers[fn:spthanks]. Clearly, the way forward is to work in lockstep with the NumPy-C API and recent Fortran standards.
