...
#+end_src
** Conclusions
It has been very restrictive to not be able to use the keyboard layout I spent years crafting. In that regard, this post is a success story, even with the awkwardness of the implementation. On the other hand, it is baffling to see the lack of good FOSS tools on this ecosystem [fn:serious], but that is to be expected perhaps. For my purposes right now, this monolithic ~init.lua~ is enough. However, Hammerspoon is one of the more promising configuration systems especially considering the [[https://github.com/dbmrq/dotfiles/blob/master/home/.hammerspoon/vim.lua][more complete VIM bindings]] which exist in the community and would bear a second look. It would make sense to look into using Spacehammer and more VIM bindings sometime soon.
