...
* DONE Anki Decks with Orgmode :@personal:workflow:projects:tools:emacs:orgmode:
...
section to ensure we load ~anki-editor~. This is essentially via ~eval:
(anki-editor-mode)~ in the Local variables block.  With that, we just need to
fire up Anki, and we're off to the races (remove ~\~). The [[https://raw.githubusercontent.com/louietan/anki-editor/master/examples.org][Anki editor examples]]
file is excellent and the [[https://github.com/louietan/anki-editor/issues/30][issue tracker]] also has a ton of information.
Some final comments:
