...
complete/recommended/normative; *use the installation guide* and _follow the
Arch Way_.
** Archlinux baseline
...
- Audio :: ~pulseaudio~ works well as always
...
- Device details :: The ArchWiki [[https://wiki.archlinux.org/title/Lenovo_ThinkPad_X1_Carbon_(Gen_6)][has an entry]]
...
support Wayland out of the box, or atleast optionally.

However, certain applications are unusable without the HiDPI support patches,
including most ~java~ packages. Thus it is best to grab the modified versions
and configure them.
*** Display management
...
** Browsers
Firefox's ~wayland~ build on the AUR remains a nightmare to compile and work with. As of this post the stable branch of ~google-chrome~ does indeed support Wayland with a switch:
#+begin_src bash
