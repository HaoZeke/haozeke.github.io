...
* TODO Fortran OOP and Python :@notes:python:fortran:numpy:f2py:rambling:
# :PROPERTIES:
...
# :END:
#+begin_quote
Explorations beyond standardized interoperability with ~C~ for object oriented ~Fortran~
#+end_quote
** Background
Derived types. Probably one of the most divisive of the Fortran features. Rather
than rehash the history and details of their standardized interop with
structures which [[Simple Fortran Derived Types and Python][has been covered previously]] this post attempts to bridge more
traditional OOP patterns with C and by extension C-Python.

For those new to the language, some terminology:
...
Unfortunately only the first of these are actually covered in terms of
interoperability with ~C~ as of the [[https://gitlab.com/lfortran/lfortran/uploads/973db2bfcc64cd2a397f823c32718b39/18-007r1.pdf][F2018 draft standard]]. A little deviation
from the standards never hurt anyone though (much).

Type shadowing support with modern Fortran is in progress for ~numpy~, however,
although there are no direct equivalents in ~C~ for the more OOP specific
derived types, there are plenty of equivalent structures in ~Python~.

** The class
The failings of the OOP pattern, many though there may be, are easily hidden
behind a contrived geometric shape example. So following along in that grand
tradition, we will devise a derived type with functions defining a *Square* and
then generalize to a *Shape* and an extended (a.k.a. inherited) type.
** General overview
The general conception follows closely the work of
@pletzerExposingFortranDerived2008, with the main difference being in the fact
that instead of exposing a series of ~C~ functions which call ~Fortran~ methods,
we will instead define a ~Python~ class to hide the dirty work from the user.
...
* TODO Spack and PyTorch Development Workflows :@notes:ramblings:pytorch:
#+begin_quote
~pytorch~ development without tears or ~spack dev-build~
#+end_quote
** Background
...
nice niche for such situations[fn:whatelse] where the full complexity of
[[https://spack.readthedocs.io/en/latest/build_systems/cudapackage.html][CudaPackage]] is necessary. Some details of replacing a ~conda~ -environment are
[[https://spack.readthedocs.io/en/latest/replace_conda_homebrew.html][already documented]], as are the instructions for setting up [[https://spack-tutorial.readthedocs.io/en/latest/tutorial_developer_workflows.html][local
...
spack find -b # Should have clingo
#+end_src
...
For starters, we will setup a development environment to track PyTorch master.
** Basic Development
For starters we will need to setup a development environment. We will use a
named environment instead of an anonymous one. The downside is that the path of
our cloned local environment is now relative to ~$SPACK_ROOT~ but named
environments have more visible pretty path helpers.
#+begin_src bash
mkdir spackEnvs
cd spackEnvs
spack env create pytorch-main
# created in $SPACK_HOME/var/spack/environments/pytorch-main
#+end_src
...
#+begin_src bash
spack -e pytorch-main add python@3.8 py-numpy
spack -e pytorch-main config edit # Make changes
#+end_src
At this point a bare-minimum near-empty environment is ready.
#+begin_src yaml
spack:
  specs: [python@3.10]
  view: true
...
#+end_src
There is no need to alter ~view~ as it has the effect of symlinking
installations to ~$SPACK_HOME/var/spack/environments/$ENV_NAME/.spack-env/view~
and this is fine since activating a ~spack~ environment ensures appropriate
additions to ~PATH~ and friends. To ensure dependencies can be resolved, we will
test with ~concretize~, and then activate the environment.
#+begin_src bash
spack -e pytorch-main concretize # doesn't install
spack -e pytorch-main install
spack env activate . # or spacktivate .
#+end_src
Recall that changes are propagated via:
#+begin_src bash
spack -e pytorch-main concretize --force
spack -e pytorch-main install
#+end_src
** OpenBLAS and Compilers
Setting up compiler toolchains like a ~fortran~ compiler for ~openblas~ can take
a bit more effort. Although this discussion will focus on obtaining a ~fortran~
compiler it is equally applicable to updating a version. For ~spack~, unlike
many system package managers, ~gcc~ will install the ~C~, ~C++~ and ~Fortran~
toolchains. Thus:
#+begin_src bash
...
git apply -3 pytorchspack.diff
#+end_src
This modified environment can now be used as:
#+begin_src bash
spacktivate .
spack develop py-torch@npeye
#+end_src
This generates a sub-directory ~py-torch~ with the right branch checked out,
along with most of the dependencies needed for the build. Additionally, the
~python~ version is also localized to the ~spack~ installation spec. Some
dependencies are weirdly not present, but they are easily obtained.
#+begin_src bash
cd py-torch
...
# oops, not found pyyaml
spack add py-pyaml # will check dependencies
spack install # concretizes and installs
#+end_src
The rationale behind the build flags can be found in the [[https://github.com/pytorch/pytorch/blob/master/CONTRIBUTING.md][upstream contributing guide]].
*** CUDA Setup
The good news is that updating the build system to use CUDA is very straightforward.
...
** Conclusions
Dependency management is always painful. CUDA management doubly so. Although the methodology described herein worked well, a disclaimer is appropriate:
#+begin_quote
Better workflows with [[https://spack-tutorial.readthedocs.io/en/sc20/tutorial_developer_workflows.html][spack dev-build]] are available for some packages, [[https://github.com/kokkos/kokkos/blob/master/Spack.md][like KOKKOS]], but ~spack dev-build~ doesn't work yet for ~pytorch~ (presumably) since it lacks install phases.
#+end_quote
