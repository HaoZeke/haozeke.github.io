...
. ~/.bashrc
#+end_src
Now we can use ~mamba~ to get a temporary interactive environment, including ~gcc 9.3.0~.
#+begin_src bash
# Spack requirements
...
#+begin_src bash
nix-env -iA nixpkgs.nixUnstable -f $HOME/Git/Github/nixpkgs -j$(nproc) --keep-going --show-trace --cores 4 2>&1 | tee nix-install-base.log
#+end_src
Now we can activate flakes and other features like ~nix shell~ (note the space!).
#+begin_src conf
# ~/.config/nix/nix.conf
