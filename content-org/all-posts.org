...
~exec~ but it also gets re-executated after a reload. For something like a
...
configuration just because changes needed to be propagated is asinine, and
finally annoyed me enough to make the switch to a real service manager.
...
in the same environment, this might not be a significant advantage.
