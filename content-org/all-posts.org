...
* TODO GSoC21 W4: LFortran, Assumed length, and LLVM :@notes:gsoc21:fortran:rambling:
:PROPERTIES:
...
:END:
#+begin_quote
Implementing ~assumed length~ in the LLVM backend and hacking away at ~dftatom~
#+end_quote
** Background
Serialized update for the [[https://summerofcode.withgoogle.com/projects/#5790375215628288][2021 Google Summer of Code]] under the [[https://fortran-lang.org/][fortran-lang
organization]], mentored by [[https://ondrejcertik.com/][Ondrej Certik]].
...
4. *GSoC21 W4: LFortran, Assumed length, and, LLVM* <-- You are here!
** Overview
...
fixture and rent related idiosyncrasies which led to a shift in my meeting with
Ondrej. In any case, though Ondrej knows exactly how I should implement the
...
Thanks to a timely reminder from my mentor Ondrej, I managed, nevertheless, to make some concrete progress working on the compilation of ~dftatom~ modules as well.
*** New Merge Requests
*** Freshly Assigned Issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/388][Semantics: Implement TINY (#388)]]:: A classic intrinsic for error handling
- [[https://gitlab.com/lfortran/lfortran/-/issues/389][Semantics: Array Initializer Expressions (#389)]] :: Probably throwing a misleading error, as ~gfortran~ does not complain
- [[https://gitlab.com/lfortran/lfortran/-/issues/390][ODE1 Error]] :: Placeholder issue to be narrowed and chiseled into something actionable
*** Additional Tasks
I'd like to make some time to continue exploring the LLVM, not to mention familiarizing myself with the remainder of the string allocation code.
** Logistics
** Workflow Description
Now that I am a few weeks in, the workflow has roughly stabilized enough to warrant documentation.
Typically for each feature implementation issue, I add a screenshot from the relevant draft and also link to the ~pdf~. This is currently either @J321007Draft or @J318007r1F2018 as and when appropriate.
** Linking Troubles
Sometime down the line of modules and the roadmap to ~dftatom~ described in the [[https://gitlab.com/lfortran/lfortran/-/issues/319][master issue here]], I ran into an odd linker error. These seem to show up with both ~miniconda~ and ~nix~, however, it is unclear to me at the moment if it warrants an issue until I check on a native ArchLinux machine.
#+begin_src bash
...
** Conclusions
For the next week, I shall be finalizing work on the allocatable character lengths and moving on to some more semantics and intrinsics. Naturally as these are completed there will be more progress down the ~dftatom~ module listing.
