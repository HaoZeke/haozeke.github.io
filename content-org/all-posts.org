...
* TODO Customizing ArchLinux Images
** Background
I recently had to switch server boxes, due to a cascade of hardware errors stemming from a remodeling of my home. The new box, however, did not see fit to include a WiFi card, which meant I  pp
...
* TODO CPP Library Blueprints for Python Bindings
#+begin_quote
...
** Truncated
- I found ~meson.wrap~ to be much more palpable than working with ~submodules~
*** False starts
...
Linking to the library itself is best done by declaring a dependency:
#+begin_src meson
...
** Conclusions
Using PRs turned out to be insufficient to maintain code quality and I ended up
shifting to a submodule based workflow. Most of these evolved during my work on
[[https://pypi.org/project/pypotlib/][pypotlib]], and a newer version of ~eON~, the long timescale dynamics code, which
isn't (as of April 2024) public yet[fn:academicFOSS].
...
* TODO Python Bindings for CPP Libraries
** Background
These are some guidelines I keep reaching for[fn:likewhen].
** Design Guidelines

** Gotchas
For some reason using ~meson wrap install~ for ~pybind11~ as a pre-build step
doesn't work while building wheels. So all the ~wrap~ files needed should be
vendored in. It is still broken on PyPI. So now we need to look into what's
going wrong. AGAIN.
