...
* TODO Local Nix without Root :@programming:workflow:projects:hpc:nix:tools:
:PROPERTIES:
:EXPORT_FILE_NAME: local-nix-no-root
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
** Background
Nis is not well known for being friendly to users without root access. This is typically made worse by the "exotic" filesystem attributes common to HPC networks (this also plagues [[https://github.com/andrewchambers/hermes/issues/75][hermes]]).
** Setup
The premise is similar to my earlier post [[HPC Dotfiles and LMod][on HPC Dotfiles]]. For ~nix~ in particular, there are some complications we will cover.
#+BEGIN_SRC bash
...
cd nix-2.3.7
./configure  --enable-gc --prefix=$myprefix --with-store-dir=$nixdir/store --localstatedir=$nixdir/var --with-boost=$BOOST_ROOT --disable-seccomp-sandboxing --disable-doc-gen
vim src/libutil/compression.cc
# Remove everything from 239
# to allow only the else
make -j $(nproc)
make install
# CPPFLAGS="-I$HOME/.hpc/bzip2/1.0.8/include" LDFLAGS="-L$HOME/.hpc/bzip2/1.0.8/lib -Wl,-R$HOME/.hpc/bzip2/1.0.8/lib"
ml load nix/user
#+END_SRC
...
#+END_SRC
** Rebuilding Natively
At this point we can use ~nix~ to build itself:
#+BEGIN_SRC nix
      storeDir = "/home/rog32/.nix/store";
      stateDir = "/home/rog32/.nix/var";
      confDif = "/home/rog32/.nix/etc";
#+END_SRC
** Patches
If there are errors with multi-threading in the ~xz~ library, feel free to comment them out.
We will also need [[https://github.com/NixOS/nix/pull/1584/files][this patch]] and [[https://nixos.wiki/wiki/Nix_Installation_Guide][this one]].
...
** Conclusions
Though this is slow and seems like an inefficient use of cluster resources, the benefits of reproducible environments typically outweighs the cost.
