...
#+RESULTS:
| citeproc-org-render-references | org-ref-acronyms-before-parsing | org-ref-glossary-before-parsing | org-attach-expand-links |
* DONE Search
:PROPERTIES:
:EXPORT_HUGO_SECTION: /
...
:END:
#+begin_quote
Leveraging containers for streamlined remote interactive and mounted file access to intranent machines
#+end_quote
** Background
...
This will prompt in the terminal for the VNP credentials, including an MFA if
...
~rclone~ also works well. The principles can be adapted for other
OpenConnect-compatible VPNs by modifying the ~openconnect~ command in the
~entrypoint.sh~ script (potentially using environment variables for server
addresses and usernames).

A more robust variant of the container images and workflow developed into the
[[https://github.com/HaoZeke/vpn-proxyjump][vpn-proxyjump]] project.
