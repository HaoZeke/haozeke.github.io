...
Effectively, this can be done with the GUI, but we prefer the configuration located in ~$HOME/.config/karabiner/karabiner/karabiner.json~:
#+begin_src json
{
    "from": {
        "key_code": "caps_lock"
...
Note that we *do not* need to define the ~hyper~ key here with the ~complex_modifications~ in this configuration.
** Hammerspoon
...
The alerts can help while setting up muscle memory, but they are optional.
*** Remapping Keys
...
[fn:whathidden] Yes, the original post was 4 years ago and it still isn't part of the GUI
