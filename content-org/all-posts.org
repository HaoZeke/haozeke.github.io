...
* TODO Revisiting Wayland for ArchLinux
# :PROPERTIES:
# :EXPORT_FILE_NAME: revisiting-wayland-2021-archlinux
# :EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
# :END:
#+begin_quote
Fluff post about returning to ArchLinux and reconfiguring for Wayland via Sway
#+end_quote
** Background
...
*** Login management
To manage sessions, rather than relying soley on ~sway~, since
*** Wayland
...
** Screen Recorders
Unfortunately, the ~zoom~ desktop client [[https://github.com/swaywm/sway/issues/4358][does not support]] ~sway~ which is problematic.
** Reference Management
...
to web-clients like Mailpile [fn:welldesc] would have been perfect... Except it is still based around ~python2~ and shows no sign of moving forward. I really like
Astroid.
** Browsers
...
#+begin_src bash
Exec=env WAYLAND_DISPLAY= QT_DEVICE_PIXEL_RATIO=2 /usr/bin/rstudio-bin %F
#+end_src
** Resilio
Recall that:
#+begin_quote
...
maintainence load. It seems likely then, that inspite of great progress it is
...
To be perfectly fair, a lot of this post reads as a rant against HiDPI support, which makes sense given that it is rather poorly supported.
