...
:END:
#+begin_quote
Leveraging Github for managing interconnected projects.
#+end_quote
** Background
...
** First attempt: Manual checkout and ~cron~
If a library depends on another, the simplest would be to run a ~cron~ job every
day which manually checks out the dependent to see if anything broke.
...
trigger workflows.
***  User level
Scoping rules in GitHub and security considerations being what they are, the
focus will be on minimal permissions, and we will first consider
*** Organization level
*** Cross organization
...
[fn:whynotnumpy] So more akin to my doctoral work over at [[github.com/TheochemUI][Theochem]] (~potlib~, ~pypotlib~) or [[https://github.com/atomic-solvers][atomic-solvers]] than NumPy / SciPy at the moment.
