...
*** C++
This is an older back-end, and can be selected by passing the ~--backend=cpp~
flag to ~lfortran~ (e.g. with ~FFLAGS~). This is easier to work with in that
many features boil down to ~stdlib~ calls. However, this back-end requires
KOKKOS. It also has uglier error handling as shown in Fig. [[fig:llvmcpp]].
...
- Flang was first a summer project, then in 2017 the so called legacy flag was published, which was so bad Ondrej decided to start LFortran
- Intel doesn't really care
- Currently LFortran shadows things
