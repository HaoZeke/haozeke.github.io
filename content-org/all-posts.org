...
The reason why we need to switch profiles is because by default ~nix-env --switch-profile~ will use ~/nix/var/nix/profiles/per-user/$USER/profile~, and it is better to keep this where we have control as well.
...
*** Beyond ~nix-shell~
One of the nicer aspects of ~nix develop~ is that any of the phases defined in the ~default.nix~ file can be run; for example:
#+begin_src bash
cd testFlakes
nix develop
unpackPhase
...
This makes it easier to debug as well.
** Flake Helpers
