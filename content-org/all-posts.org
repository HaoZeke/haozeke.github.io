...
:END:
#+begin_quote
Leveraging containers for streamlined remote access to intranent machines
#+end_quote
** Background
...
- Multiple VPNs :: Switching between VPNs is slow, and running multiple simultaneously is generally not feasible.
...
Attempting to switch between VPNs, or run multiple VPN clients, can be
time-consuming, error-prone, and sometimes technically infeasible on a single
host operating system.

Furthermore, even when connected to a single VPN, accessing multiple internal
machines often involves direct SSH commands. A more streamlined approach
involves setting up a secure jumphost that routes traffic through the VPN, which
can then be seamlessly integrated into local SSH workflows using features like
~ProxyJump~.

This is where containerization technology offers a powerful alternative,
allowing us to isolate a VPN connection and its associated network routing
within a dedicated environment, without affecting the host system's main network
configuration.
** Container setup
...
networks without impacting your entire host's networking.  The principles can be
adapted for other OpenConnect-compatible VPNs by modifying the ~openconnect~
command in the ~entrypoint.sh~ script (potentially using environment variables
for server addresses and usernames).

A more robust variant of the container workflow developed into the [[https://github.com/HaoZeke/vpn-proxyjump][vpn-proxyjump]]
project.
