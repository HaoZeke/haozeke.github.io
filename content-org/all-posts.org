...
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :bibliography refs.bib
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/refs.bib
:EXPORT_DATE: 2020-03-25 17:04
:END:
...
$$ \hat{\mathrm{Bias}}_{boot}=\frac{n-1}{n} \hat{\mathrm{Bias}}_{jack}$$
...
\begin{align}
E*P[Y-\hat{f}^*(x)]²&=&E_P[Y-f*{ag}(x)+f*{ag}(x)-\hat{f}^\*(x)]² \\
&=&E_P[Y-f*{ag}(x)]²+E*P[\hat{f}^\*(x)-f*{ag}(x)]² ≥ E*P[Y-f*{ag}(x)]²
\end{align}
...
- The probability of the iᵗʰ example will occur at least once is then
  $1-(1/e)≈0.632$
- This means for each base learner, there are around $36.8$% originall
