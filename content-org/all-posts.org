...
** Conclusions
At this point, we have a basic setup, which we can tweak with a bunch of themes, and/or different parsers, but this is still pretty rough around the edges. However, a caveat of this setup is that the actual contents of the source are not visible in the generated documentation. In [[Publishing Doxygen and Sphinx with Nix and Rake][the next post]], we will look at automating this setup for deploying with Travis.
* TODO Publishing Doxygen and Sphinx with Nix and Rake :@programming:documentation:workflow:nix:cpp:
:PROPERTIES:
...
#+END_QUOTE
** Background
In [[Documenting C++ with Doxygen and Sphinx - Exhale][the previous post]] we generated documentatino
*** Series
