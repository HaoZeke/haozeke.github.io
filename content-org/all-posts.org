...
have configured ~layout_poetry~ as [[Poetry and Direnv][described here]]:
#+BEGIN_SRC bash
...
*Update*
#+end_alert-heading
I actually ended up refactoring my entire Dotfiles to use more Colemak native bindings, as described [[Refactoring Dotfiles For Colemak][in this post]].
#+end_alert
** Conclusion
...
:END:
** Background
As I mentioned in my [[On-boarding for Code in Place][last post]], I'm leading a section for [[https://compedu.stanford.edu/codeinplace/announcement/][Stanford CS106A: Code
...
:END:
** Background
As I mentioned [[On-boarding for Code in Place][earlier]], I'm leading a section for [[https://compedu.stanford.edu/codeinplace/announcement/][Stanford CS106A: Code
in Place]]. I did also mention I'd try to keep a set of short notes on the
...
- [[Nix with R and devtools][Nix with R and devtools]]
- [[A Short Guide to Statistical Rethinking²][Statistical Rethinking and Nix]]
...
- [[A Tutorial Introduction to Nix][A tutorial introduction to Nix and Python]]
** Slides
...
#+end_quote
** Background
I have mentioned in the past my customized [[Refactoring Dotfiles For Colemak][Colemak dotfiles]] which I used with [[Remapping Keys with XKB and KLFC][a customized keyboard layout]]. Unfortunately, the ~.keylayout~ system of MacOS is far more primitive than the elegant ~klfc~ setup [fn:nothinggood]. For an understanding of what we are trying to get at, the following poorly made video will suffice.

#+html: {{< youtube BgqlUsenmKU  >}}
*** VIM Colemak Extensions
Just as a reminder, my setup (or ~hzColemak~) consists of an augmented VIM workflow, as shown below, and [[Remapping Keys with XKB and KLFC][described in my previous post]].
...
- *Runtime Library Design* :: Inspite of [[https://fortran-lang.discourse.group/t/fortran-runtime-math-library/755][strong community opposition]] to implementing elementary functions in pure Fortran; we ended up with a very well designed elegant runtime design I am immensely satisfied with. Broadly speaking (detailed in the [[GSoC21 W10: LFortran Runtime Library Design][Week 10 post]]), the design encompasses ~impure~ parts of the library, which cannot be implemented in pure Fortran, and ~pure~ sections which can be implemented without any external calls
...
#+end_quote
** Background
As mentioned [[Regarding Quansight Labs][earlier]], I have the pleasure of being involved in an effort to modernize and maintain the
...
For a reminder, though I won't go into any real details here in this post, my preferred approach to ~nix~ in production is a combination of ~niv~ (which is brilliant) and throwaway ~nix-shell~ expressions, described in great detail in other posts on this blog, like [[Statistical Rethinking and Nix][this one]] or this [[A Tutorial Introduction to Nix][introductory post]].
** Getting Started
...
#+end_src
** Python
To work with ~python~ I typically use ~mach-nix~ as [[Niv and Mach-Nix for Nix Python][described here]]. However, with a ~flake~.
...
#+end_quote
** Background
+Barely two months ago+ A few years ago, [[Dotfiles from dotgit to bombadil][I wrote about]] moving away from the
...
out.
*** HPC Configurations
Around the same time as this post, I was [[HPC Configuration 2021][writing up my trials]] and tribulations
...
in the [[Site Rationale][original spirit of this blog]] (TL;DR: just write stuff down).
#+end_alert
...
#+END_QUOTE
**** Other Content
- [[NumPy, Meson and ~f2py~][NumPy, Meson and ~f2py~]]
*** Slides
...
0. [[Fortran, GSoC21 and Me][Fortran, GSoC21 and Me]]

1. [[GSoC21 W1: LFortran Kickoff][GSoC21 W1: LFortran Kickoff]]

2. [[GSoC21 W2: LFortran Unraveling][GSoC21 W2: LFortran Unraveling]]

3. [[GSoC21 W3: Kind, Characters, and Standards][GSoC21 W3: Kind, Characters, and Standards]]

4. [[GSoC21 W4: LFortran, Backends and Bugs][GSoC21 W4: LFortran, Backends and Bugs]]

5. [[GSoC21 W5: LFortran Design Details and ~minidftatom~][GSoC21 W5: LFortran Design Details and ~minidftatom~]]

6. [[GSoC21 W6: LFortran ASR and Values][GSoC21 W6: LFortran ASR and Values]]

7. [[GSoC21 W7: LFortran Workflow Basics][GSoC21 W7: LFortran Workflow Basics]]

8. [[GSoC21 W8: LFortran Refactors, and Compile Time Evaluations][GSoC21 W8: LFortran Refactors, and Compile Time Evaluations]]

9. [[GSoC21 W9: LFortran Bug Hunting Bonanza][GSoC21 W9: LFortran Bug Hunting Bonanza]]

10. [[GSoC21 W10: LFortran Runtime Library Design][GSoC21 W10: LFortran Runtime Library Design]]

11. [[GSoC21 LFortran and Computational Chemistry][GSoC21 LFortran and Computational Chemistry]]
...
#+end_quote
** Background
[[Simple Fortran Derived Types and Python][Derived types and their interoperability]] have been covered
...
~bind(c)~ variant of the [[Simple Fortran Derived Types and Python][previous post]].
...
** Premise
As in [[Simple Fortran Derived Types and Python][the previous posts]], we will be interested in a ~bind(c)~ derived type.
#+begin_src fortran
type :: cartesian
  real(c_double) :: x, y, z
end type cartesian
...
Of the representations described, the first approximation covered in [[Simple Fortran Derived Types and Python][a previous
post]] was the ~dictionary~. A major concern [fn:whosaidthat] is that of lineage
...
[[Reclaiming Email with Astroid][like Astroid]], but it doesn't seem to have been updated in a while either. Thunderbird works fine too, and does not require XWayland which is a plus point.
*** Electron
...
[fn:sincewhen] Since I largely retired from Android [[LineageOS Maintainer Appreciation][custom kernel / ROM work]]
...
#+end_alert
** Background
I split this post from the [[Customizing ArchLinux Installation Media][previous one]], to make each more self contained.
...
and papers [[My Life in E-ink][for consumption on my Kobo Aura HD in the past]], but this comes before
...
#+end_quote
** Background
I've written about replacing ~jupyter~ with ~orgmode~ [[Replacing Jupyter with Orgmode][in the past]], and that
...
I've dicussed working with [[My Life in E-ink][Zotero before]], but when working with cloud synced
...
[fn:: Also discussed in [[Managing cloud based ~calibre~ libraries][Managing cloud based ~calibre~ libraries]]] and
...
~spack~ including local environment use was covered in a [[Spack and PyTorch Development Workflows][previous post]].
...
#+end_src
Normally, I'm a fan of using ~tinyssh~ [[TinySSH for Docker Development Environments][for basic containers]] but since this is
...
sporadically updated [[Collection of WebLinks][collection of web-links]] involving me. There is also, a
...
- I collaborated closely with the brilliant [[https://ondrejcertik.com/][Dr. Ondřej Čertík]] over the course of both on my [[SymEngine and the Season of Docs][GSoD 2020 long project for SymEngine]] and over [[GSoC21 LFortran and Computational Chemistry][GSoC 2021 on the LFortran compiler]]
...
- Emacs News
  - [[https://sachachua.com/blog/2021/07/2021-07-26-emacs-news/][2021-07-27]] :: for [[Doom Emacs and Language Servers][Doom Emacs and Language Servers]]
