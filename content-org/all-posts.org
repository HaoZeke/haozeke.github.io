...
- preamble :: This section of the document is essentially everything before ~\begin{document}~
...
Which will actually rendered in a real document of course [fn:butwhatformula]:
...
There is also the inline form of designating LaTeX ~@@\sin{x}@@~ which is essentially $\sin{x}$.
*** Preamble
...
#+END_SRC
For larger documents, this gets quite annoying for just loading packages. We will demonstrate a more aesthetically pleasant form later in this post.
*** LaTeX classes
...
** Elisp to the rescue
The core idea is quite simple. Since ~orgmode~ files are literate documents, and ~emacs~ is self-documenting and completely programmable, it should be possible to execute code to deterministically set the state of ~emacs~ before exporting the document to TeX. Practically this needs a few moving parts. In the following sections, assume that we have a ~.cls~ file which defines a document-class ~foo~.
...
#+END_SRC
This indirection required to call the function as a ~hook~. Now this can be used as:
#+BEGIN_SRC org :exports code
# Local Variables:
...
# End:
#+END_SRC
This method could be extended to essentially resetting all ~emacs~ variables on a per-file basis, though I cannot really think of another realistic use-case. The method presented here is really general enough to work with any arbitrary LaTeX ~.cls~ file or other draconian measures.
