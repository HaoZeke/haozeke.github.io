...
6. [[GSoC21 W6: LFortran ASR and Values][GSoC21 W6: LFortran ASR and Values]]
7. [[GSoC21 W7: LFortran Workflow Basics][GSoC21 W7: LFortran Workflow Basics]]
8. *GSoC21 W8: LFortran and Backend Values* <-- You are here!
** Logistics
- Discussed more refactors over MRs and Zulip
** Overview
Intrinsic functions and more bug hunting. A lot of starts in different directions, but I will need to trim these down a bit. A major goal
*** New Merge Requests
...
#+end_quote
** Background
* TODO Nix Flakes :@programming:tools:nix:workflow:
:PROPERTIES:
...
This creates a single ~flake.nix~ file, shown below, and running ~nix flake lock~ to get ~lockfile~ with all the details.
#+begin_src nix
{
...
*** ~nix-shell~ Goodness
Just like ~nix-shell~ and ~nix-build --run-env~, ~nix develop~ allows us to inspect and run any of the phases defined in the ~default.nix~; for example:
#+begin_src bash
cd testFlakes
nix develop
# Viewing only
...
** Flake Helpers
Note that just like with the standard workflow, we have some tools which can help matters move along.
-
** Python
