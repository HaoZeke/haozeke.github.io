...
#+end_quote
** Background
[[Simple Fortran Derived Types and Python][Derived types and their interoperability]] has been covered
...
** Setup
We will re-use at first the basic cartesian type of the previous
post, along with a *non-type-bound* procedure which is to be called
from ~C~.
...
we now a (not strictly exact) probable ~kind~.

The corresponding ~C~ caller is exactly the same, though in this
...
cite:petersonF2PYToolConnecting2009.

Perhaps a more formal approach to wrapper generation is the one elaborated upon in great
detail in the literature by
textcite:grayShadowobjectInterfaceFortran1999 which details the
...
types with pointers in textcite:pletzerExposingFortranDerived2008 and forms the basis
for the implementation in ~f90wrap~ cite:kermodeF90wrapAutomatedTool2020.
...
cite:reidNewFeaturesFortran2003, it is not surprising they have been
