...
* TODO GSoC21 W10: LFortran and Wrapping Up :@notes:gsoc21:fortran:rambling:
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-w10
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
The road to intrinsic functions is paved with IEEE standards
#+end_quote
** Background
Serialized update for the [[https://summerofcode.withgoogle.com/projects/#5790375215628288][2021 Google Summer of Code]] under the [[https://fortran-lang.org/][fortran-lang
organization]], mentored by [[https://ondrejcertik.com/][Ondrej Certik]].
...
10. *GSoC21 W10: LFortran and Wrapping Up* <-- You are here!
** Logistics
- Met with Ondrej daily from Sunday till Friday
  + On Sunday we finished the compile time evaluation (partially)
...
** Trignometric Functions
The quote is motivated by my (limited) reading of two texts in the field of implementing elementary functions, namely the
*** Sin
...
** Sollya and Remez
One of the many online nameless luminaries of stack overflow and the lik suggested that the Remez algorithm would be of more use than the Taylor series approximation being used right now.

Thankfully testing this is trivial with Sollya. Since it does nothave a terminal inbuild we need ~rlwrap~ too.
#+begin_src bash
nix-shell -p sollya remez
#+end_src
...
elemental real(dp) function kernel_dsin(x) result(res)
  real(dp), intent(in) :: x
  res = 2.087908370826698101212920076154545995e-37 + x * (0.999999999999999998041119383719347623 + x * (-1.064032149680587544373098154082500645e-35 + x * (-0.1666666666666666190025821826687444107 + x * (7.30686333395136116304293345061277226e-35 + x * (8.33333333333299311754500597882861296e-3 + x * (-1.943708819070258466622845490191457106e-35 + x * (-1.984126984115944978429391691458880356e-4 + x * (-5.30338548309302786712332804817767749e-34 + x * (2.75573192045805570474164007876304645e-6 + x * (1.185501640257404955632191608977614537e-33 + x * (-2.505210638108326593133314706727558551e-8 + x * (-1.130558221840288033840556502976715689e-33 + x * (1.605891864396986879258207676930409245e-10 + x * (5.55078059098189832214306988928325169e-34 + x * (-7.64251167703190749535921829507420458e-13 + x * (-1.378921764094824341145286520847883578e-34 + x * (2.716665406378575502010982726791586904e-15 + x * 1.374874330886318513437613275675104639e-35)))))))))))))))))
end function
#+end_src
...
#+end_src
** Conclusions
This has been the last of the weekly updates. This doesn't mean anything will change other than the format. I quite like keeping track of the work I do in this manner; keeps me honest and not complacent. For those of you following along; this has been a fun ride; for those who come to this in the future; feel free to stick to the zeroth and the final posts and refer to these only when necessary.
