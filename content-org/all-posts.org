...
;; Use citeproc-org by default
;; similar to running citeproc-org-setup
;; (setq org-export-before-parsing-hook '(citeproc-org-render-references org-ref-acronyms-before-parsing org-ref-glossary-before-parsing org-attach-expand-links))
(citeproc-org-setup)
#+END_SRC
...
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("The Orgmode Almanac")
:END:
** Background
...
have configured ~layout_poetry~ as {{{relref(described here,poetry-direnv)}}}:
#+BEGIN_SRC bash
...
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Colemak necessities")
:EXPORT_DATE: 2020-02-29 14:06
:END:
#+BEGIN_QUOTE
Thoughts on and rationale behind leaving QWERTY and touch typing in general. Followed {{{relref(by this post,colemak-dots-refactor)}}} on refactoring my Dotfiles.
#+END_QUOTE
** Background
...
#+attr_html: :class info-alert
#+begin_alert
#+begin_alert-heading
*Update*
#+end_alert-heading
I actually ended up refactoring my entire Dotfiles to use more Colemak native bindings, as described {{{relref(in this post,colemak-dots-refactor)}}}.
#+end_alert
** Conclusion
...
I actually use [[https://windscribe.com][Windscribe]] for my VPN needs, and they
have a neat [[https://windscribe.com/getconfig/socks][SOCKS5 proxy setup]]. This
works out to a line like ~socks5 $IP $PORT $USERNAME $PASS~ being added. The
default generator gives you a pretty server name, but to get the IP I use ~ping
$SERVER~ and put that in the ~conf~ file. Any online DNS resolver will also do
the trick, like [[https://www.vedbex.com/dns/socks-us.windscribe.com][this one]].

#+attr_html: :class note-alert
#+begin_alert
#+begin_alert-heading
*Note*
#+end_alert-heading
The SOCKS5 proxy from Windscribe has been [[https://windscribe.com/knowledge-base/articles/why-was-socks5-discontinued][phased out]] but it is actually easier
to do this via the [[https://windscribe.com/features/proxy-gateway][proxy gateway]] and split tunneling.
#+end_alert

For inclusive split tunneling, add the following (minimally):
#+begin_src txt
tuner.pandora.com
audio-usc-mp1-t1-2-v4v6.pandora.com
#+end_src

At some point it might be nice to actually have this done via a WireGuard
configuration to cut out the app in the middle..
...
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("$HOME...")
...
- User installation of ~nix~ is sadly non-trivial, so this might be of some use[fn:: Much of this section is directly adapted from [[https://nixos.wiki/wiki/Nix_Installation_Guide#Installing_without_root_permissions][the NixOS wiki]]]
...
first[fn:: This used to be called linuxbrew, but the [[https://docs.brew.sh/Homebrew-on-Linux][new site]] makes it clear
...
**UPDATE:** This site has since shifted to Clicky, for {{{relref(reasons outlined here,goat-clicky)}}}.
...
:END:
** Background
As I mentioned in my {{{relref(last post,scp-onboarding)}}}, I'm leading a section for [[https://compedu.stanford.edu/codeinplace/announcement/][Stanford CS106A: Code
...
:END:
** Background
As I mentioned {{{relref(earlier,scp-onboarding)}}}, I'm leading a section for [[https://compedu.stanford.edu/codeinplace/announcement/][Stanford CS106A: Code
in Place]]. I did also mention I'd try to keep a set of short notes on the
...
* DONE Refactoring Dotfiles For Colemak :@programming:workflow:dots:
CLOSED: [2020-05-02 Sat 20:30]
:PROPERTIES:
:EXPORT_FILE_NAME: colemak-dots-refactor
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Colemak necessities" "$HOME...")
:END:
#+BEGIN_QUOTE
A more actionable follow up to {{{relref(my personal recollections,colemak-switch)}}} relating to my switch to Colemak.
#+END_QUOTE
** Background
I have, in the past written about how I {{{relref(made the switch,colemak-switch)}}} to Colemak. However,
...
with colemak keybindings {{{relref(described here,colemak-dots-refactor#i3)}}}. Recall that, [[https://en.wikipedia.org/wiki/Compositing_window_manager][from Wikipedia]]:
#+BEGIN_QUOTE
...
screen sharing, especially after fixing the {{{relref(dark screen glitch,compton-zoom-shadow)}}}. However, though
...
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("R x Nix Nexus: Crafting Cohesive Environments")
:END:
#+BEGIN_QUOTE
...
*Update:* There is {{{relref(another post,emacs-nix-r)}}} with methods to reload this configuration automatically
** Conclusions
...
~rethinking~ is {{{relref(continued here,rethinking-r-nix)}}}.
...
** Background
As detailed {{{relref(in an earlier post,nix-r-devtools)}}}[fn:wherehook], I had set up Nix to work with
non-CRAN packages. If the rest of this section is unclear, please refer back to {{{relref(the earlier post,nix-r-devtools)}}}.
*** Setup
...
*** Lorri and Direnv
{{{relref(In the past,poetry-direnv)}}}, I have made my admiration for ~direnv~ very clear (especially for
...
*Update:* There is {{{relref(a final part,emacs-nix-r)}}} detailing automated ways of reloading the system configuration

[fn:wherehook] My motivations were laid out in the {{{relref(aforementioned post,nix-r-devtools)}}}, and will not be repeated
[fn:explaaiin] For why these are the way they are see the this is written, see the {{{relref(aforementioned post,nix-r-devtools)}}}
...
- {{{relref(Part I,nix-r-devtools)}}} covered ways of working effectively with ~R~ and ~nixpkgs~
- {{{relref(Part II,rethinking-r-nix)}}} dealt with composing dependent ~devtools~ packages in a per-package environment, with a focus on ~rethinking~ and ~tidybayes.rethinking~
...
#+END_SRC
If any of these look strange, refer to the {{{relref(earlier posts,nix-r-devtools)}}}.
** Automation Pains
~direnv~, ~lorri~ and
~niv~ (the heroes of {{{relref(Part II,rethinking-r-nix)}}}) are not really useful for working with the system-wide configuration, but
...
:END:
#+BEGIN_QUOTE
A follow up to {{{relref(my earlier post on analytics,goat-google)}}}, and on migrating from Goat Counter to Clicky.
#+END_QUOTE
...
#+END_QUOTE
** Background
Of late, I have been attempting to move away from paper, for environmental reasons[fn:environ]. Years of touch typing in Colemak ({{{relref(rationale,colemak-switch)}}}, {{{relref(config changes,colemak-dots-refactor)}}}) and a very customized [[https://dotdoom.rgoswami.me][Emacs setup]] (including {{{relref(mathematica,org-mathematica)}}}, {{{relref(temporary latex templates,org-arb-tex)}}}, {{{relref(Nix,emacs-nix-r)}}}, and {{{relref(org-roam annotations,org-note-workflow)}}}) have more or less kept me away from analog devices. One might even argue that my current website is essentially a set of tricks to move my life into ~orgmode~ completely.
...
** Conclusion
We now have a setup which ensures that the touch enabled area is constant, without any explicit manual interventions for when devices are added or removed. There isn't much else to say about this workflow here. Additional screens can be {{{relref(configured from older laptops described here,laptop-as-second-screens)}}}. Separate posts can deal with how exactly I meld Zotero, ~org-roam~ and Xournal++ to wreak havoc on all kinds of documents. So, in-lieu of a conclusion, behold a recent scribble with this setup:
...
** Background
As {{{relref(detailed previously,rethinking-r-nix)}}}, I recently was part of a course centered around Bayesian modeling for the Icelandic COVID-19 pandemic. The Bayesian mindset needs no introduction, and this post is completely inadequate to explain why anyone should be interested (that's what the book is for!). That said, especially for self-paced study groups, it might help to have some structure.

** Solutions
These are meant to be sample solutions, and everyone *should* solve these for themselves. Each solution contains the packages used, as well as a colophon in the later posts to ensure reproducibility.
...
** Background
As regular readers might know, I have a multi-screen setup, which accounts for having touch enabled on my primary laptop screen (detailed {{{relref(here,multi-monitor-touch)}}}). A failing of this setup was that I was not able to control the brightness of both monitors at the same time.
** Existing Stack
...
Unfortunately, my existing setup (with ~light~, since that is in the Arch ~community~ repo) did not actually allow dimming external screens arbitrarily. To be more exact,
...
** Materials
This is a Carpentries style single page lesson on setting up and working with Nix for reproducible environments. It was conceived to be a complimentary resource to the content of [[https://github.com/HaoZeke/CarpentryCon2020_Nix][this repository]], namely:
...
- {{{relref(Nix with R and devtools,nix-r-devtools)}}}
- {{{relref(Statistical Rethinking and Nix,some-sol-sr2)}}}
...
#+END_SRC
** Python Dependencies
As R dependency management has been covered {{{relref(in an earlier post,nix-r-devtools)}}}, we will focus on the management of ~python~ environments.
...
We will use the standard setup described in {{{relref(the tutorial post,ccon-tut-nix)}}}:
#+BEGIN_SRC bash
nix-env -i niv lorri
niv init
niv update nixpkgs -b nixpkgs-unstable
...
#+END_QUOTE
** Background
In previous posts, there was a {{{relref(discussion on a ground up approach to adding packages,ccon-tut-nix)}}} which aren't on the core ~nixpkgs~ channels using GitHub or PyPi sources. However, this lacked a way to do so programmatically, and also a way to convert existing python projects.
...
#+END_QUOTE
** Background
Nix is not well known for being friendly to users without root access. This is typically made worse by the "exotic" filesystem attributes common to HPC networks (this also plagues [[https://github.com/andrewchambers/hermes/issues/75][hermes]]). An {{{relref(earlier post,prov-dots)}}} details how and why ~proot~ failed.
...
#+END_QUOTE
This concept is strongly influenced by the work [[https://github.com/jefdaj/nix-no-root][described in this repo]]. The premise is similar to my earlier post {{{relref(on HPC Dotfiles,hpc-dots-lmod)}}}. For the purposes of this post, we will assume that all the packages in the {{{relref(previous post,hpc-dots-lmod)}}}
...
#+END_SRC
- lmod packages :: If you do not or cannot use modulefiles as {{{relref(described in the earlier post,hpc-dots-lmod)}}}, inspect the module-files being loaded and set paths accordingly.
#+BEGIN_SRC bash
cd $HOME/Git/Github
...
The extensions to this are fairly standard, and follow the CRAN guidelines in {{{relref(the previous post,nix-r-devtools)}}}. Installation works the same way too.
#+BEGIN_SRC bash
...
** Conclusion
* DONE Documenting C++ with Doxygen and Sphinx - Exhale :@programming:documentation:workflow:cpp:sphinx:doxygen:
CLOSED: [2020-09-22 Tue 06:58]
:PROPERTIES:
:EXPORT_FILE_NAME: doc-cpp-dox-sph-exhale
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("C++ documentation practices")
:END:
#+BEGIN_QUOTE
...
** Conclusions
At this point, we have a basic setup, which we can tweak with a bunch of themes, and/or different parsers, but this is still pretty rough around the edges. However, a caveat of this setup is that the actual contents of the source are not visible in the generated documentation. {{{relref(In the next post,pub-doc-cpp-dox-sph-nix)}}}, we will look at automating this setup for deploying with Travis.
* DONE Publishing Doxygen and Sphinx with Nix and Rake :@programming:documentation:workflow:nix:cpp:ruby:rake:
...
#+END_QUOTE
** Background
In {{{relref(the previous post,doc-cpp-dox-sph-exhale)}}} we generated documentation using Doxygen with Exhale to handle Sphinx. Now we will clean up the earlier workflow with ~rake~ and ensure the environment is reproducible with ~nix~ while deploying to [[https://travis-ci.com/][Travis CI]].
** Setup
...
At this point one might assume we will leverage the ~requirements.txt~ based workflow described earlier in {{{relref(Niv and Mach-Nix for Nix Python,mach-nix-niv-python)}}}. While this would make sense, there are two barriers to its usage:
...
*** Shell Environment
We prep our sources in the usual way, by running ~niv init~ in the project root to generate the ~nix/~ folder and the sources therein. With all that in mind, the ~shell.nix~ file at this point is fairly standard, keeping the general ~niv~ setup in mind (described in a {{{relref(previous Nix tutorial,ccon-tut-nix)}}}):
...
:PROPERTIES:
:EXPORT_FILE_NAME: doc-cpp-dox-sph-doxyrest
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("C++ documentation practices")
:END:
#+BEGIN_QUOTE
...
#+END_QUOTE
** Background
* TODO Adding Tutorials to Sphinx Projects :@programming:documentation:tutorials:sphinx:
:PROPERTIES:
:EXPORT_FILE_NAME: doc-tut-sph
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+BEGIN_QUOTE
...
- {{{relref(A tutorial introduction to Nix and Python,ccon-tut-nix)}}}
** Slides
...
At this point, given that I was to set up a pre-recorded video for {{{relref(PyCon India 2020,pycon-in-2020-meta)}}}, I decided to look into alternatives.
** Alternatives
...
#+END_QUOTE
** Background
Much the same as the rationale behind my {{{relref(meta-post on my talk at PyCon India 2020,pycon-in-2020-meta)}}}, that is:
- I would like to preserve questions
- I would like to collect the video, slides and other miscellaneous stuff in one location [fn:officialsite]
- It would be nice to have my own thoughts here afterwards
*** Details
...
#+END_QUOTE
**** Linked Posts
- {{{relref(Local Nix without Root,local-nix-no-root)}}} :: Motivation and installation
  + {{{relref(Provisioning Dotfiles on an HPC,prov-dots)}}} :: Looks into how standard approaches fail (~proot~)
  + {{{relref(HPC Dotfiles and LMod,hpc-dots-lmod)}}} :: All the ugly manual install steps
**** Other Content
...
#+end_src
However, in the interests of sanity, we will leverage the [[https://ankiweb.net/shared/info/1463041493][Syntax Highlighting Anki plugin]] for managing the actual style-sheets instead of manual edits to each card type.
...
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Managing Email")
:END:
#+begin_quote
...
- ~astroid~ :: for handling the email interface
- ~isync/mbsync~ :: for general IMAP account synchronization
- ~lieer~ :: for GMail accounts
- ~davmail~ :: for Exchange accounts
- ~notmuch~ :: for indexing emails
- ~pass~ :: for handling secrets (~secret-tool~ or anything else would work too)
- ~msmtp~ :: for sending email
- ~nix~ :: for handling dependencies in a reproducible manner (or ~pixi~)
- ~cryptomator~ :: for encryption at-rest (or ~tomb~)
- ~rclone~ :: for handling cloud synchronization of email contents
** The Accounts
...
** Security
We will use ~pass~ for handling the secrets [fn:whatkey].
...
Additionally, we will store the actual contents of the mail in an encrypted
store. Generally ~Cryptomator~ works really well for data which is not updated
very often, but has performance issues for many small file[fn:: See the [[https://community.cryptomator.org/t/is-cryptomator-a-safe-replacement-for-veracrypt-durability-and-performance-wise/5157][discussion here]] and the security [[https://docs.cryptomator.org/en/latest/security/security-target/#security-security-target][target here]]]. However, given its focus, and the fact that this setup can end up
with a rather heavy set of files, we will instead use ~tomb~ which interacts
with a ~LUKS~ volume ([[https://dyne.org/docs/tomb/#get-started][website]]).

#+attr_html: :class warning-alert
#+begin_alert
#+begin_alert-heading
*Warning*
#+end_alert-heading
Although LUKS volumes can be expanded to become larger, shrinking to reclaim
space is not a supported operation [fn:: Deleting email didn't seem like an operation I'd do very often].
#+end_alert
...
*** General IMAP
For a standard IMAP account (anything which isn't Exchange or Gmail) the only
thing we need to keep track of is a good way to obfuscate passwords. We will use
~pass~ for this, as configured earlier.

#+begin_src bash
pass add mail/rgoswami.iitk
#+end_src

With this, we can configure a general IMAP account in ~$HOME/.config/isyncrc~
[fn:: an IITK webmail in this instance, but it could be anything].
...
*** Exchange Accounts
For Exchange accounts we need to setup ~davmail~. Thankfully the process is
actually excessively simple for a single account. For each account, a
~.properties~ file needs to be generated. The password section is kept blank in
this case, since we will authenticate with the ~O365Interactive~ protocol.
...
** Notmuch
At this point, we have all mail synced into local directories, but we have no
access to view or interact with them. We will start by setting up ~notmuch~ to
search and index our mail. This is pretty basic for now, in ~$HOME/.notmuch-config~.
#+begin_src toml
[database]
...
can replace ~thunderbird~ unfortunately. However, there are several pain points
...
* DONE Remapping Keys with XKB and KLFC :@personal:workflow:tools:keyboard:
CLOSED: [2020-12-05 Sat 22:05]
:PROPERTIES:
:EXPORT_FILE_NAME: remap-keys-xkb-klfc
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Colemak necessities" "Keboard management")
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
...
The first mapping is a [[http://www.keyboard-layout-editor.com/#/gists/93c31b6e61608c19c26933b0a212b262][basic Colemak setup]] as shown in Fig. [[fig:colemak][fig:colemak]].
...
* DONE Icelandic with Compose Keys on Linux :@personal:workflow:tools:keyboard:linux:
CLOSED: [2024-03-08 Fri 01:35]
:PROPERTIES:
:EXPORT_FILE_NAME: xcompose-keys-linux-icelandic
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Colemak necessities" "Keboard management")
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
Minimally expanding my existing layouts for supporting Icelandic orthography.[fn:whatcompose]
#+end_quote
** Background
Life outside Emacs can be daunting for a lot of reasons, the lack of ~auctex~
and ~org-latex~ being a major one. Additionally, for the purposes of my
(somewhat current) country of residence[fn:whatwhyweird], it is often useful to
be able to type Icelandic characters without resorting to something [[https://icelandic.typeit.org/][like typeit]].
** Compositional Choices
I already use ~klfc~ to generate a very portable (in a Unix sense) customized
[[https://rgoswami.me/posts/remap-keys-xkb-klfc/][VIM based Colemak binding layout]]. Some reasons why I first decided to modify the
compose keybindings instead of extending that are[fn:whatfirst]:
- The keys and bindings defined here aren't in use enough for me to justify
  actually adding them to one of my levels.
- It is slightly easier to share ~.XCompose~ files compared to the ~klfc~
  compatible ~json~ files.
- It also integrates nicely with [[https://github.com/HaoZeke/Dotfiles][my dotfiles]] and templates
  + There are a few tools for concatenating fragments[fn:likewhatxcomposers]  but it seemed more
    straightforward to use my existing ~chezmoi~ machinery
    - The tools, especially [[https://gitlab.com/ekaunt/ext-compose/][ext-compose]], can be used as a pretty definitve source of viable mappings
...
placed on the dead-greek (~AltGr~) level and the rest are via multi-key (a.k.a
~compose~) chords.
*** Setting the Compose Key
Modern keyboards don't often come with a dedicated ~compose~ key.  For
...
I went with ~compose:ralt~ since ~compose:lalt~ isn't thing. Setting this in
~sway~ is simple and stacks well with the other settings:
#+begin_src bash
# To test
...
** Mapping Icelandic
I used [[https://icelandic.typeit.org/][TypeIt]] as a reference, since I'm rather used to it. The mapping therefore
proceeds in two stages, an AltGr layer, and some compose keychords.
*** AltGr Layer
For the base keys, I did end up first extending the AltGr layer, with the following, as
depicted in Fig. [[fig:altgrlayeris][fig:altgrlayeris]] [fn:figwherefrom]:

#+name: fig:altgrlayeris
#+caption: AltGr layer mapping
[[file:images/keyboards/IS_AltGr_hz.png]]
...
cabal v1-build
# Build layout
./dist/build/klfc/klfc ~/.config/klfc/colemak.json ~/.config/klfc/extendVIM.json ~/.config/klfc/is_altgr_Colevim.json -o colevimIS
# Install
cd colevimIS/xkb/
./install-system.sh && ./scripts/install-xcompose.sh
#+end_src
...
** Conclusions
This is going to take a while to get used to for sure, but eventually it should
help me reduce the time it takes me to get thoughts on (digital) paper in
Icelandic. I will probably need to touch type on something like [[https://10fastfingers.com/typing-test/icelandic][fastfingers]] [fn:whynoothers].

I had hoped to include a bunch of unicode math and other compose keychords, but
it is annoying to have to keep track of the translations like ~*~ to ~asterisk~
and similar[fn:sowhatnow].

[fn:whatcompose] Recall that the compose key is essentially for keychords, like a dead key on steriods.
[fn:whatwhyweird] Where the phrasing is mostly for legal purposes, as anyone dealing with immigration and taxation would know.
[fn:likewhatxcomposers] Like [[https://github.com/np/xcompose/tree/master][xcompose]], or [[https://gitlab.com/ekaunt/ext-compose/][ext-compose]], or [[https://github.com/hugomg/xcomposer][xcomposer]], or [[https://github.com/EdAyers/xcomposegen][xcomposegen]], or even something with a GUI like [[https://github.com/QazCetelic/Composition][Composition]].
[fn:whatfirst] I ended up extending the layout a bit for the Icelandic mappings though.
[fn:figwherefrom] As in [[https://rgoswami.me/posts/remap-keys-xkb-klfc/][the previous post]] the images are made with [[http://www.keyboard-layout-editor.com/][keyboard-layout-editor]].
[fn:whynoothers] [[https://www.typingclub.com/][Typingclub]] or [[https://www.typing.com/][typing.com]] or [[https://www.keybr.com/][keybr]] are favorites but don't support Icelandic yet.
[fn:sowhatnow] Perhaps I too will try my hand at a cleaner ~xcompose~ generator.
...
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_LASTMOD: 2025-09-04T04:01:41+02:00
:END:
#+begin_quote
A workflow for managing private submodules in a private repository without
personal access tokens for Github actions
#+end_quote
** Background
...
#+end_src
** Alternative
Another way to handle this with less boiler-plate for cloning a private
repository is to use the ~webfactory/ssh-agent~ action.
#+begin_src yaml
...
** Project Layout
The standard ~niv~ setup, as described in the {{{relref(longer tutorial post,ccon-tut-nix)}}} will suffice.
#+begin_src bash
niv init -b master
#+end_src
Since the bindings were for ~python~, ~mach-nix~ was leveraged for the ~shell.nix~; reasons for which {{{relref(have been outlined elsewhere,mach-nix-niv-python)}}}. This led to the following expression:
...
* DONE My Life in E-ink :@personal:workflow:tools:ramblings:books:
CLOSED: [2021-02-20 Sat 01:41]
:PROPERTIES:
:EXPORT_FILE_NAME: my-life-in-eink
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Modern reading")
:END:
#+begin_quote
...
*** Zotero Sync
Calibre provides a handy [[https://www.mobileread.com/forums/showthread.php?p=3339191#poststop][ZMI (Zotero Metadata Importer) plugin]] which allows for
exported papers to be imported into ~calibre~ and from then into the e-reader as
expected. Combined with the folder mounts facilitated by ~calibre~ this allows
for a painless way to ensure a quick export; optimize; sync; read and delete
workflow.
*** Koreader
...
[fn:whynotes] These are annotated {{{relref(in Emacs with org-mode,org-note-workflow)}}} when necessary
[fn:whytype] For which I have been perfecting a rather {{{relref(personalized workflow with Colemak,colemak-switch)}}}
...
- Also is now pretty
*** Tooling Decisions
This is more of a technical specification; but after rooting around, it was decided to throw consistency of design to the wind and use native tools for each language binding as shown in Table [[tbl:langtool]].
...
|-------------------+--------------------------|
| [[https://symengine.org][Notebooks or MyST]] | [[https://www.sphinx-doc.org/][Sphinx]] + [[https://myst-nb.readthedocs.io/en/latest/][myst]] + [[https://jupytext.readthedocs.io/en/latest/install.html][jupytext]] |
|-------------------+--------------------------|
...
* DONE HPC Configuration 2021 :@programming:workflow:projects:hpc:
CLOSED: [2021-05-09 Sun 23:12]
:PROPERTIES:
...
#+begin_quote
A post on configuring a user account for heavy interactive HPC usage with ~Spack~,
~Nix~ and self-hosted applications.
#+end_quote
** Background
I have in the past written a few posts about configuring my HPC setup. Since the
[[https://www.rhnet.is/][Research and University Network of Iceland hf. (RHnet)]] recently recieved funding
for a new machine, called Elja [fn:whyname], I decided to jot down my notes from
...
| ~find~   | [[https://github.com/sharkdp/fd][fd-find]]          | Find things                          |
...
| ~gibo~   | bliss            | ~gitignore~ generation utility       |
| ~diff~   | delta            | Nicer diffs                          |
| ~watch~  | [[https://github.com/watchexec/watchexec][watchexec]]        | Watch and execute                    |
| ~CTRL+R~ | mcfly            | Reverse search                       |

These are trivial to obtain:
#+begin_src bash
cargo install bottom ripgrep hyperfine skim mcfly
# The following need some system libraries (OpenSSL) and shall be dealt with later, or with micromamba
# bat exa fd-find tealdeer starship sd procs zoxide bliss navi watchexec
#+end_src
Not immediately obvious, but [[https://docs.rs/cargo-update/2.5.3/cargo_update/][easy to update]] as well.
#+begin_src bash
...
#+begin_src bash
# Spack requirements
PKGS="git patch curl make tar gzip unzip bzip2 xz zstd subversion lsof m4 openssl pkg-config"
mkdir ~/.mamba_venvs
micromamba create -p ~/.mamba_venvs/intBase $PKGS
micromamba activate ~/.mamba_venvs/intBase
#+end_src
...
curl -fL "https://github.com/cdr/code-server/releases/download/v$VERSION/code-server-$VERSION-linux-amd64.tar.gz" \
  | tar -C ~/.local/lib -xz
mv "$HOME/.local/lib/code-server-$VERSION-linux-amd64" "$HOME/.local/lib/code-server-$VERSION"
...
** Conclusions
This is a work in progress, and would likely need to be updated ever so often,
perhaps like a "State Of My Dots" or a yearly "HPC Configuration $YEAR" or
something like that.

[fn:whyname] A step in the right direction in terms of female representation among the university servers
...
#+end_quote
** Background
In the same spirit as my similar post on {{{relref(Nix Shells for Node Projects,nix-shell-node)}}}. As a prelude to writing up the details of how this site is generated, I realized I should write up a ~nix~ oriented workflow for ~ruby~ packages managed with ~bundle~. This site (which is in ~hugo~), I also manage many beautiful sites built with ~jekyll~. This should serve as a useful reminder.
** Tooling and Idea
The basic concepts are:
...
* DONE Dotfiles from dotgit to bombadil :@personal:ramblings:tools:workflow:dots:
...
#+end_quote
#+begin_alert
*EDIT:* Superseded by my ~chezmoi~ configuration {{{relref(described here,mig-dots-chezmoi)}}}
#+end_alert
** Background
...
** Conclusions
Though the ~shell.d~ setup is still overly verbose and not as flexible as I had initially hoped, this is still a few steps ahead of my previous setup. No part of this focused on the configurations themselves (more interesting examples, of {{{relref(switching to Colemak are here,colemak-dots-refactor)}}}) Will be fleshing this out more with auxilliary posts on my configuration (browsers, etc.) and its management perhaps.
* TODO Browser Wars
...
* DONE Remapping Keys for ColemakVIM on MacOS :@personal:workflow:macos:tools:
CLOSED: [2021-07-23 Fri 04:45]
:PROPERTIES:
:EXPORT_FILE_NAME: keybindings-colevimhz-macos
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Colemak necessities" "Keboard management")
:EXPORT_HUGO_LASTMOD: 2021-08-22T13:43:02+00:00
:END:
#+begin_quote
Struggling to emulate ~klfc~ for VIM Colemak bindings on Darwin (macOS) systems with Hammerspoon and Karabiner
#+end_quote
** Background
I have mentioned in the past my customized {{{relref(Colemak dotfiles,colemak-dots-refactor)}}} which I used with {{{relref(a customized keyboard layout,remap-keys-xkb-klfc)}}}. Unfortunately, the ~.keylayout~ system of MacOS is far more primitive than the elegant ~klfc~ setup [fn:nothinggood]. For an understanding of what we are trying to get at, the following poorly made video will suffice.

#+html: {{< youtube BgqlUsenmKU  >}}
*** VIM Colemak Extensions
Just as a reminder, my setup (or ~hzColemak~) consists of an augmented VIM workflow, as shown below, and {{{relref(described in my previous post,remap-keys-xkb-klfc)}}}.

#+name: fig:layall
#+caption: Set of layouts
[[file:images/keyboards/extendLayer_hz.jpg]]
[[file:images/keyboards/extShift_hz.jpg]]
[[file:images/keyboards/extAltGr_hz.png]]
...
- Manually write ~.keylayout~ :: These then go in ~$HOME/Library/Keyboard\ Layouts~
- Use [[https://software.sil.org/ukelele/#about][Ukelele]] :: The incredibly poorly named (for search purposes) versatile tool is able to ease the pain slightly for writing ~.keylayout~ files
- Use [[https://karabiner-elements.pqrs.org/][Karabiner Elements]] :: This seems to be closer to [[https://www.autohotkey.com/][AutoHotKey]] and the like, runs in the background and actively intercepts keys based on ~json~ configurations; though there seems to be a more rational method (a ~hidutil~ [[https://hidutil-generator.netlify.app/][remapping file generator]]) for newer kernels [fn:wheresay]
- Script things with [[http://www.hammerspoon.org/][Hammerspoon]] :: Uses a ~lua~ engine to interact with the system, can be configured for the most part with [[https://fennel-lang.org/][Fennel]] using [[https://github.com/agzam/spacehammer][Spacehammer]]

Now of the four, I had a predilection to move towards manually writing, with the help of Ukelele. However, evidently, there is no real way to remove the stickiness from the Caps Lock key. It can either be remapped using *system settings* [fn:whatswrong] to one of the other modifier keys, but *not to Extend*. The closest possible solution would be to do a very awkward ~Esc~ based layout. Also, rapid prototyping was out of the question, since Ukelele requires a log-out log-in cycle to set things up. Of Ukelele and manually writing things then, nothing more need be said.
...
#+DOWNLOADED: screenshot @ 2021-07-23 01:12:58
#+name: fig:stdcolmk
#+caption: Standard Colemak layout (not what MacOS provides)
[[file:images/Karabiner/2021-07-23_01-17-01_screenshot.png]]
[[file:images/Karabiner/2021-07-23_01-12-58_screenshot.png]]


Apparently it is possible to overload the keyboard system with a "Hyper" key [fn:nicename], which is the closest to Extend.

This is setup through a ~karabiner.json~ file, since it appears that the "Complex modifications" referred to in the GUI (Fig. [[fig:stdcolmk]]) don't really allow for more than downloading rules off of the internet [fn:whathidden], like the one below.
...
#+end_src
** Hammerspoon
Not that an operating system should need a scripting engine, but apparently [[http://www.hammerspoon.org/][Hammerspoon]] is worth looking into fox better control over the entire OS [fn:whyhammmer]. This made it more attractive than writing a hidden configuration file for Karabiner.
...
An alternative to using ~Karabiner~ to map ~f19~ is to use the [[https://github.com/hetima/hammerspoon-foundation_remapping][foundation remapping plugin]] which needs to be placed in ~$HOME/.hammerspoon~ with the following added to our ~init.lua~:
#+begin_src lua
...
Anyway, once ~f19~ has been bound, one way or another, we need to setup the hyper key itself[fn:sayzwho]:
#+begin_src lua
-- init.lua
...
#+end_src
** Conclusions
It has been very restrictive to not be able to use the keyboard layout I spent years crafting. In that regard, this post is a success story, even with the awkwardness of the implementation. On the other hand, it is baffling to see the lack of good FOSS tools on this ecosystem [fn:serious], but that is to be expected perhaps. For my purposes right now, this monolithic ~init.lua~ is enough, and the entire configuration corresponds to this commit in my Dotfiles. However, Hammerspoon is one of the more promising configuration systems especially considering the [[https://github.com/dbmrq/dotfiles/blob/master/home/.hammerspoon/vim.lua][more complete VIM bindings]] which exist in the community and would bear a second look. It would make sense to look into using Spacehammer and more VIM bindings sometime soon.

[fn:nothinggood]  There are some [[http://xahlee.info/kbd/osx_keybinding.html][text snippet expansion methods]], but nothing to simply modify keys.
[fn:whatswrong] A nauseating prospect, far too inelegant and difficult to keep track of a million manually changed GUI settings
[fn:nicename] Coined in its current form by Brett Terpstra [[https://brettterpstra.com/2017/06/15/a-hyper-key-with-karabiner-elements-full-instructions/][here]] or Jonathan Hollin [[https://www.perpetual-beta.org/weblog/magical-caps-lock-key.html][here]], first described by Steve Losh [[https://stevelosh.com/blog/2012/10/a-modern-space-cadet/][many years ago]]
[fn:whathidden] Yes, the original post was 4 years ago and it still isn't part of the GUI, but it is [[https://karabiner-elements.pqrs.org/docs/json/root-data-structure/][now documented]]
[fn:whyhammmer] One of the FOSS tools, plus its in my favorite language (used in [[https://dseams.info][d-SEAMS]]) for embedding things, ~lua~
[fn:serious] It might also just be that there is a lot more media spam, like [[https://www.alfredapp.com/][Alfred]] is basically [[https://spacelauncherapp.com/][SpaceLauncher]] but the latter isn't well known
[fn:wheresay] As discussed here on the [[https://www.naseer.dev/post/hidutil/][Random Bits blog]]
[fn:sayzwho] This is close to the method [[https://kalis.me/setup-hyper-key-hammerspoon-macos/][Rosco Kalis has]]
[fn:youwrotebro] This is adapted from these [[https://ke-complex-modifications.pqrs.org/#JesseHyperPlusVimNavigation][regular VIM Karabiner rules]]
...
* DONE GSoC21 W1: LFortran Kickoff :@notes:gsoc21:fortran:ramblings:
...
:EXPORT_BIBLIOGRAPHY: biblio/refs.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
:END:
#+begin_quote
Charting paths towards concrete ~lfortran~ usage
#+end_quote
** Background
As mentioned in {{{relref(an earlier post,fortran-gsoc-me)}}}, I have had the immense pleasure of continuing
...
* DONE GSoC21 W2: LFortran Unraveling :@notes:gsoc21:fortran:ramblings:
...
* DONE GSoC21 W3: Kind, Characters, and Standards :@notes:gsoc21:fortran:ramblings:
...
* DONE GSoC21 W4: LFortran, Backends and Bugs :@notes:gsoc21:fortran:ramblings:
CLOSED: [2021-07-02 Fri 23:26]
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-w4
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
:END:
#+begin_quote
...
~dftatom~ modules.
*** New Merge Requests
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1015][ASR Bug with Arrays (997)]] :: Fixes a bug in the existing handling of arrays and ~allocate~ calls
*** Freshly Assigned Issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/388][Semantics: Implement TINY (388)]] :: A classic intrinsic for error handling
- [[https://gitlab.com/lfortran/lfortran/-/issues/389][Semantics: Array Initializer Expressions (389)]] :: A bugfix related to the merge request
- [[https://gitlab.com/lfortran/lfortran/-/issues/390][ODE1 Error (390)]] :: Placeholder issue to be narrowed and chiseled into something actionable
- [[https://gitlab.com/lfortran/lfortran/-/issues/395][ASR mismatch between `gfortran` modules and source (395)]] :: Issue relating to the completeness and equivalence of ~mod~ formats
*** Additional Tasks
I'd like to make some time to continue exploring the LLVM, and continue
familiarizing myself with formal language and automata theory.
** Logistics
...
Also related, probably, was an issue I noticed while co-working with Ondrej, namely, that my stacktraces in ~nix~ were and are a good deal less informative than his, probably due to [[https://nixos.wiki/wiki/Debug_Symbols][the default nix lack of debug symbols]]. This can be quickly reproduced with:
#+begin_src bash
...
Traceback (most recent call last):
  ...
  File "/users/home/rog32/Git/Github/Fortran/mylf/src/bin/lfortran.cpp", line 601, in (anonymous namespace)::compile_to_object_file(std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> > const&, std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> > const&, bool, bool) [clone .isra.0]
    result = fe.get_asr2(input);
  File "/users/home/rog32/Git/Github/Fortran/mylf/src/lfortran/codegen/evaluator.cpp", line 469, in LFortran::FortranEvaluator::get_asr2(std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> > const&)
  ...
  File "/users/home/rog32/Git/Github/Fortran/mylf/src/lfortran/ast.h", line 3235, in LFortran::SymbolTableVisitor::visit_TranslationUnit(LFortran::AST::TranslationUnit_t const&)
  ...
  File "/users/home/rog32/Git/Github/Fortran/mylf/src/lfortran/ast.h", line 3369, in void LFortran::AST::visit_expr_t<LFortran::SymbolTableVisitor>(LFortran::AST::expr_t const&, LFortran::SymbolTableVisitor&)
    static void visit_expr_t(const expr_t &x, Visitor &v) {
  Binary file "/nix/store/hp8wcylqr14hrrpqap4wdrwzq092wfln-glibc-2.32-37/lib/libc.so.6", in killpg()
...
#+begin_src bash
lfortran mod b14.mod --show-asr
(TranslationUnit (SymbolTable 1 {g: (Subroutine (SymbolTable 2 {}) g [] [] GFortranModule Public Interface)}) [])
#+end_src
Which is contains a lower amount of information.
The differences between the module versions are not major:

#+DOWNLOADED: screenshot @ 2021-07-02 18:41:23
[[file:images/Gfortran_Modules/2021-07-02_18-41-23_screenshot.png]]
...
* DONE GSoC21 W5: LFortran Design Details and ~minidftatom~  :@notes:gsoc21:fortran:ramblings:
CLOSED: [2021-07-09 Fri 19:23]
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-w5
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
:END:
#+begin_quote
...
- Zeroed in on several alternate designs [[https://gitlab.com/lfortran/lfortran/-/wikis/Intrinsic-Functions][implementations of intrinsic functions]]
  + We have prototype calls for the two kinds of compile evaluation
  + My ~sin~ implementation ([[https://gitlab.com/lfortran/lfortran/-/merge_requests/717][pre-GSoC21]]) is pretty unwieldy and a cleaner method which we elected to pursue is the ~minval~ strategy
- Discussed concrete methods of working
- Talked about the runtime library
  + This is a late stage issue, after the ASR generation
** Overview
This week focused around a subset of ~dftatom~ which Ondrej prepared. In fact the main highlight was working through possible code design directions and iterating on these with Ondrej (and Gagandeep asynchrously) to come up with the [[https://gitlab.com/lfortran/lfortran/-/wikis/Intrinsic-Functions][intrinsic function design]] methods and the compile time evaluation of expressions plan issue (discussed below in issue 420).
*** New Merge Requests
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1024][Restructuring of the ASR (1024)]] :: A code cleanup task, to reduce code smell from ~using~ statements and to refactor into more logical file structures
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1037][Grammar modifications (1037)]] :: Name changes to keep a consistent clear implementation structure
*** Freshly Assigned Issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/401][minidftatom Roadmap (401)]] :: A clone of the ~SNAP~ and ~dftatom~ issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/416][size() not handled correctly for arrays (416)]] :: This is an ASR bug, with a fix which should not be too difficult to track down
- [[https://gitlab.com/lfortran/lfortran/-/issues/420][Compile time evaluation of expressions (420)]] :: This is an internal implementation design specification document, which essentially introduces a ~value~ option for the ASR
*** Additional Tasks
...
7s          -0.130948    2.000
 Print the first 10 values of the 1st and 2nd orbitals:
   1753.2892641883584        1753.2812175211841        1753.2731512531607        1753.2650645481160        1753.2569495840532        1753.2488203554758        1753.2406710770906        1753.2325014795395        1753.2243110250588        1753.2160993495806
   587.50702296395195        587.50432661503442        587.50162369317059        587.49891391928361        587.49619467268258        587.49347064291044        587.49073989144335        587.48800232804547        587.48525777252200        587.48250610287175
#+end_src
*** Parsing and AST Generation
The test for the parser is essentially to check if the output before and after formatting with ~lfortran~ still compiles to the same result.
Note that because of the nature of this, we need to ensure ~-ffree-line-length-none~ is passed when working with the ~gfortran~ compiler.
#+begin_src bash
cd minidftatom
...
#+end_src
*** Dependency Tree
Before going on to the ASR, a dependency tree is required. As before, it is reproduced in excruciating detail in the issue, and represented by a pretty picture shown in Fig. [[mdftatomgit][mdftatomgit]] for this post.
#+begin_src bash
pip install fortdepend
cd minidftatom/src
fortdepend -g -c
...
#+name: mdftatomgit
#+caption: ~minidftatom~ compilation dependency graph
file:images/minidftatom_depgraph.svg

*** ASR Generation
I found a new favorite command, which I should have remembered while it was being implemented (by Thirumalai Shaktivel in [[https://gitlab.com/lfortran/lfortran/-/merge_requests/765][765]]); namely the  ~--indent~ option which makes for a much more readable ASR output. Apart from that ~git diff --color-words~ works well enough as well.
** Conclusions
The main work plan outcomes for the week were the [[https://gitlab.com/lfortran/lfortran/-/issues/420][new design specifications for the compile time evaluation of expressions]]. It is always an enriching experience to work with Ondrej directly (even over the ether) and this week felt more productive than most. I hope to keep the same momentum going. This will allow faster progress on the main issue of getting ASR generated for ~minidftatom~.
* DONE GSoC21 W6: LFortran ASR and Values :@notes:gsoc21:fortran:ramblings:
CLOSED: [2021-07-16 Fri 17:19]
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-w6
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
:END:
#+begin_quote
...
*** New Merge Requests
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1045/][Implement expr values for Integer Binops (1045)]] :: First example of using value-enriched nodes for operating during the ~ast2asr~ pass
** Misc Logs
*** False Starts
...
*** New Merge Requests
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1072][Cleanup Integer Binary Operations (1045)]] :: There was an extraneous check for something the ~LFORTRAN_ASSERT~ would catch
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1080][Values for Real Binary Operations (1080)]] :: An implementation, which is similar conceptually and in implementation to the Integer Value MR
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1087][Symbol Table Scope Fix for Function Calls (1087)]] :: A bug which haunted me for a few weeks turned out to have a similar fix in the ~BodyVisitor~
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1088][Intrinsic Issue Template (1088)]] :: I am rather fond of a particular style of opening missing intrinsic issues, and this codifies it for future use
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1085][Draft: ci: Use Ninja everywhere (1085)]] :: A draft MR to return to after updating the CI sometime
*** Freshly Assigned Issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/467][Suggestion to refactor ~ast_to_asr~ (467)]] :: In particular this has to do with redundancies (of sort) in the ~SymbolTable~ and ~Body~ visitors
- [[https://gitlab.com/lfortran/lfortran/-/issues/468][Implement the sqrt intrinsic (468)]] :: Or at-least, for this issue, recognize it as a valid intrinsic
- [[https://gitlab.com/lfortran/lfortran/-/issues/468][Implement the where conditional (475)]] :: Should be implemented along the same lines as the other conditionals
- [[https://gitlab.com/lfortran/lfortran/-/issues/476][Semantic error: Variable not declared (476)]] :: Another fascinating episode of "bug detective", this one might be more of a red herring
- [[https://gitlab.com/lfortran/lfortran/-/issues/477][Semantics: Minidftatom Math (477)]] :: Collection of intrinsic math functions to be implemented at the ASR level
*** Additional Tasks
Perhaps adding some more templates would make things easier (for me and other contributors). I intend to update the [[https://gitlab.com/lfortran/ci-images/][CI images]] to leverage ~ninja~.
** Misc Logs
*** Build Times
~ninja~ is fantastic. The ~make~ and ~cmake~ based workflow I was using often had me stalling for upto half a minute.

#+caption: Not a rigorous benchmark at all, estimates in seconds
...
* DONE GSoC21 W8: LFortran Refactors, and Compile Time Evaluations :@notes:gsoc21:fortran:ramblings:
CLOSED: [2021-07-30 Fri 17:11]
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-w8
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
:END:
#+begin_quote
...
*** New Merge Requests
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1096][Split ast_to_asr]] :: An MR started last week, completed and approved this week
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1111][tiny: Runtime implementation skeleton]] :: What will eventually be compiled, hooks into ~C~ for now
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1107][tiny: Compile time implementation]] :: The population of ~value~ for ~tiny~ function calls
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1106][Draft: Shift runtime intrinsic design]] :: Harmonizing the code-base, much of this is cleaning up my own earlier math implementations
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1103][Draft: expr_value for Kind]] :: A WIP MR which will clean up the slightly strange ~extract_kind~ function
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1103][Draft: Implement where construct]] :: An MR along the lines of ~if~, related but distinct from [[https://gitlab.com/lfortran/lfortran/-/merge_requests/915][Gagandeep's masked optimization WIP]]
*** Freshly Assigned Issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/480][--show-asr For larger values]] :: A visual glitch in the prettied output
*** Additional Tasks
...
- Compile time :: These are intrinsic functions like ~tiny~ or ~kind~ or even ~sin~ which can be evaluated immediately to populate the ~expr_t* value~ object
- Runtime :: These are the actual implementations, currently the goal is to have these hook into ~C~ libraries

*** Tiny Concerns
I am not a Fortran language lawyer, but I found myself puzzling over the [[https://gitlab.com/lfortran/lfortran/-/issues/388][legalese of the F-2018 draft standard]] with respect to the ~C++~ nearest neighbor, ~std::numerical_limits~.
...
For a while though I was thrown by the fact that ~C++~, within ~<limits>~ also has ~std::numeric_limits<T>lowest()~ (described [[https://en.cppreference.com/w/cpp/types/numeric_limits/lowest][here]]), which is smaller than the corresponding ~min()~ calls and has no direct ~C~ equivalent. It is infact, specifically mentioned to not be ~min~ for floating-point types.
...
* DONE GSoC21 W9: LFortran Bug Hunting Bonanza :@notes:gsoc21:fortran:ramblings:
CLOSED: [2021-08-06 Fri 19:50]
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-w9
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
:END:
#+begin_quote
...
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1107][tiny: Body visitor implementation (1112)]] :: The population of ~value~ for ~tiny~ calls beyond the symbol table i.e. in the body
*** Freshly Assigned Issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/480][--show-asr Scientific notation (480)]] :: A visual glitch in the prettied output; not really under my ambit but it might be fun
- [[https://gitlab.com/lfortran/lfortran/-/issues/481][Compiling LAPACK (481)]] :: This is the first fixed form program to be tested, there are some bugs to be ironed out but it should be very much within what we can do by the end of the program. This issue is mostly handled by Ondrej, but since it is application based I assigned myself as well
- [[https://gitlab.com/lfortran/lfortran/-/issues/482][Namespace proposal for compile time intrinsics (482)]] :: This is part of a refactoring design plan
- [[https://gitlab.com/lfortran/lfortran/-/issues/483][Semantics: Implement REAL(A [, KIND]) (483)]] :: Conversions! The bedrock of many a numerical program, this is one of the most relevant casts
- [[https://gitlab.com/lfortran/lfortran/-/issues/484][Semantics: Implement INT(A [, KIND]) (484)]] :: All conversions are similar, and sort of pointless in the SymbolTable, but still necessary and fun
- [[https://gitlab.com/lfortran/lfortran/-/issues/485][Formatting: Array pointout (485)]] :: Another minor visual glitch
** Misc Logs
*** LAPACK Setup
...
#+begin_src bash
mkdir build && cd build
ccmake .. # Make sure to turn on testing
FC=$(which gfortran) cmake .. -DCMAKE_INSTALL_LIBDIR=$(pwd)/tmp -G "Ninja"
cmake --build .
ctest
#+end_src
...
#+begin_src bash
cp make.inc.example make.inc # Modify to set paths
#+end_src

Well the very first issue is that we should remove the ~doxygen~ style comments which are marked by ~*>~. Actually we should go ahead and remove all lines starting with ~*~. The [[https://regex101.com/r/maGoQR/1][regexp for this]] is ~\*.*$~ which simply captures any line starting with ~*~. I use ~sd~ (a [[https://github.com/chmln/sd][grep alternative]]) but any standard tool will work, including ~grep -vE~.
#+begin_src bash
# Stadard
cat blah.f | grep -vE '^\*'
# Easier
...
* DONE GSoC21 W10: LFortran Runtime Library Design :@notes:gsoc21:fortran:ramblings:
CLOSED: [2021-08-14 Sat 00:28]
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-w10
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/GSoC21.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
...
*** New Merge Requests
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1113][llvm: More values when possible (1113)]] :: Backend values from compile time evaluations
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1132][kind: Compile time (1132)]] :: A rather thorny refactor; but one with great benefits
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1136][asr: Clean up extract_kind (1136)]] :: Fruitful usage of the new functions
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1114][Compile Time Intrinsic: Real (1114)]] :: A ~cast~ based approach
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1167][selected_kinds: Body visitors (1167)]] :: Possibly my favorite MR, a nice pure ~fortran~ implementation backed by a rough ~sollya~ approximation
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1167][Initial sin implementation (1167)]] :: Possibly my favorite MR, a nice pure ~fortran~ implementation backed by a rough ~sollya~ approximation
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1173][sin: Rework runtime to get better approximations (1173)]] :: Possibly my favorite MR, a nice pure ~fortran~ implementation backed by a rough ~sollya~ approximation
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1176][Compile time floor (1176)]] :: Fairly standard implementation requiring casts
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1162][Integer2real (1162)]] :: An implicit cast which had not previously been implement
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1169][Compile time int (1169)]] :: Along the lines of ~real()~
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1194][Sincos tests (1194)]] :: Benchmarking and setting up of the new test harness
*** Freshly Assigned Issues
- [[https://gitlab.com/lfortran/lfortran/-/issues/510][Semantics: Implement correct handling of PARAMETER (510)]]
- [[https://gitlab.com/lfortran/lfortran/-/issues/507][ASR Pass: Order functions (507)]]
- [[https://gitlab.com/lfortran/lfortran/-/issues/506][Semantics: Implement FLOOR (A [, KIND]) (506)]]
- [[https://gitlab.com/lfortran/lfortran/-/issues/504][Semantics: Implement MODULO (A, [, KIND]) (504)]]
- [[https://gitlab.com/lfortran/lfortran/-/issues/501][[RFC] Compile Time Intrinsic Design (501)]]
- [[https://gitlab.com/lfortran/lfortran/-/issues/500][Strange Runtime issue with Implicit Casts (500)]]
- [[https://gitlab.com/lfortran/lfortran/-/issues/499][Implicit cast for parameter (499)]]
- [[https://gitlab.com/lfortran/lfortran/-/issues/494][Better handling of intrinsic module names (494)]]
*** Additional Tasks
...
- We have [[https://www.netlib.org/fdlibm/s_sin.c][s_sin]] and also [[http://www.netlib.org/fdlibm/k_sin.c][k_sin]]
  + Written in C and assembly
Older compilers are much more readable compared to the newer ones.
** Trignometric Functions
The quote is motivated by my (limited) reading of two texts in the field of implementing elementary functions, namely @mullerElementaryFunctions2016 and @mullerHandbookFloatingPointArithmetic2018.
*** Brainstorming Sin
- [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1167/diffs?commit_id=991f1c8ca5642b4e8ebbf9766405c3d06f897a28][This and]] then the [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1167/diffs?commit_id=8314a883925187d830492a6ea32bb4e6b6968daa][simpler version here]]
- Multiple interval based approximation used for [[https://github.com/amd/aocl-libm-ose/blob/4033e022da428125747e118ccd6fdd9cee21c470/src/optmized/sin.c][AMD's optimized sin]]

We were looking for a pedagogical, simple approach to calculating ~sin~. Unsurprisingly, the entire ecosystem of compilers seems to mostly follow one implementation from the early 90s. Which is great, but Fortran doesn't really implement IEEE 754's argument reduction routine (a.k.a. ~quadmath~). It turns out with a little massaging; the pedagogical [[https://realtimecollisiondetection.net/blog/?p=9][approach presented in this blog post by Christer Ericson]] can be adapted for use.
...
#+end_src
Where the signature of ~remez~ is ~func, degree, interval, default=1, quality=1e-5, bounds=[0;infty]~;
Now we can implement ~kernel_dsin~ as follows:
#+begin_src fortran
...
#+end_src
Note that we need to use double precision for the numbers, so we can replace ~SPACE+~ with ~_dpSPACE+~.
We would like to also get some statistics of the output, in ~bash~ just becuse we can [fn:whybashwhere].
#+begin_src bash
gfortran -ffree-line-length-none lfortran_intrinsic_sin.f90 test_sina.f90 && ./a.out > remez16
awk 'NR == 1 || $2 < min {line = $0; min = $2}END{print line}' remez16
awk 'NR == 1 || $2 > max {line = $0; max = $2}END{print line}' remez16
#+end_src
...
** Better Polynomials
Some basic understanding of series expansions and / or visual inspection would make it clear that for ~sin~ at-least; the odd terms are essentially $0$. ~Sollya~ can handle these; as described in @mullerElementaryFunctions2016. We opted to go the rounding error route.
** Rounding Errors
With Ondrej, we were able to isolate the inaccuracy in our pure ~fortran~ implementation and we realized it stemmed from the manner in which our argument reduction is carried out. This is a well known problem with attempted solutions from 1980s @payneRadianReductionTrigonometric1983a (including a related implementation [[http://ftp.netbsd.org/pub/NetBSD/NetBSD-current/src/lib/libm/arch/vax/n_sincos.S][in BSD]])[fn:wherewhysohist]. In particular, as per this [[https://www.csee.umbc.edu/~phatak/645/supl/Ng-ArgReduction.pdf][WG document of the SunPro group]] @ngArgumentReductionHuge2006 describing the cannonical implementation; the error is actually from the division and difference. This is also touched upon elsewhere in the literature, @brisebarreNewRangeReductionAlgorithm2005 and @defourNewRangeReduction, along with @mullerHandbookFloatingPointArithmetic2018 . Naturally one of first places to check is the relevant [[http://www.open-std.org/JTC1/SC22/WG11/docs/n462.pdf][ISO draft standard on elementary functions]] @N462Pdf .

#+DOWNLOADED: screenshot @ 2021-08-13 13:18:16
#+caption: The ISO/IEC FCD 10967-2.4:1999(E) suggestion for ~sin~
[[file:images/Rounding_Errors/2021-08-13_13-18-16_screenshot.png]]

This isn't immediately actionable; but might be something for later. Naturally the discussions here in context of IEEE 754 @IEEEStd754 (which the Fortran standard does not fully support) and presumes basic knowledge of floating point numbers @goldbergWhatEveryComputer1991 . The clearest explanation for requiring higher precision for some constants in these reduction algorithms is given nicely by the draft standard:
#+begin_quote
For reduction of an argument given in radians, implementations use one or several approximate
value(s) of π (or of a multiple of π), valid to, say, n digits. The division implied in the argument
reduction cannot be valid to more than n digits, which implies a maximum absolute angle value
for which the reduction yields an accurate reduced angle value.
#+end_quote
...
#+end_src
** Conclusions
Rather than move forward with the closing comments; and going into more detail about the manner in which further ~sollya~ based polynomial approximations are being looked into; I will end this post here; as a trailer and invitation to the closing report. This has been the last of the weekly updates. This doesn't mean anything will change other than the format. I quite like keeping track of the work I do in this manner; keeps me honest and not complacent [fn:butresearch]. For those of you following along; this has been a fun ride; for those who come to this in the future; feel free to follow along from the beginning and drop comments below.

[fn:whybashwhere] From this [[https://stackoverflow.com/questions/13099202/awk-script-for-finding-smallest-value-from-column/13100039][stackoverflow post]] of course
[fn:wherewhysohist] From this [[https://berthub.eu/articles/posts/help-my-sin-is-slow-and-my-fpu-is-inaccurate/][blog post by Bert Hubert]]
[fn:butresearch] I maintain similar notes for my research, but those sadly never see the light of day
* DONE GSoC21 LFortran and Computational Chemistry :@notes:gsoc21:fortran:ramblings:
CLOSED: [2021-08-20 Fri 00:43]
:PROPERTIES:
:EXPORT_FILE_NAME: gsoc21-fin-reprot
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/GSoC21.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("GSoC21: LFortran")
...
#+end_quote
** Background
If the last ten weeks of weekly posts have not made it clear; as a student developer in the Google Summer of Code in the year 2021; I was privileged to work on the [[https://lfortran.org/][LFortran compiler]] alongside [[https://czgdp1807.github.io/][Gagandeep Singh]] and [[https://fortran-lang.discourse.group/t/lfortran-gsoc-student-progress-thirumalai-shaktivel-ast-project/1271][Thirumalai Shaktivel]] under the fantastic mentorship of [[https://ondrejcertik.com/][Ondřej Čertík]] under the Fortran-Lang organization.
** Pencil Pushing
...
*** Project Overview
Given that the LFortran compiler is implemented in a manner reminiscent of both ~CPython~ and ~clang~, the main goal of the project was to leverage the compiler to parse, compile, link and run a production code base. For this, the code base chosen was @certikDftatomRobustGeneral2013, which was further trimmed down to a [[https://gitlab.com/lfortran/examples/minidftatom][miniature version here]]. Practically speaking, this involved tackling the Fortran standard itself, segregating the functionality at various stages in the "automatic coding" pipeline; and implementing standardese. Also, a lot of **debugging** naturally.
*** Statistics
For a single overview of the work done; the following table of contributions sums things up nicely.
#+caption: Code contributions to LFortran repository as of 2021-08-19T23:16:22+00:00
| *Contribution* | *Open* | *Closed/Merged* | *Total* |
| [[https://gitlab.com/lfortran/lfortran/-/merge_requests?scope=all&state=all&author_username=HaoZeke&label_name=GSoC][Merge Requests]] |     10 |              22 |      36 |
| [[https://gitlab.com/lfortran/lfortran/-/issues?author_username=HaoZeke&label_name%5B%5D=GSoC&page=2&scope=all&state=all][Issues]]         |     20 |              11 |      31 |

This is an ever evolving set of metrics, and the finer details of these contributions can be found by following the weekly series above. There were some extra contributions across the other repositories, involving minor fixes and some work done towards [[https://github.com/mesonbuild/meson/pull/9137][integrating with Meson]].
*** MR Overview
These are better described in the weeks they were made; and only the highlights of the 22 will be briefly alluded to here.
- *Implement more kind()* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/997][997]]] :: This was the first compile time enhancement / bugfix

- *Restructures and Grammar changes* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1024][1024]], [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1037][1037]]] :: Code quality enhancement for more readable ASR values

- *Binary Operations* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1045][1045]], [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1072][1072]], [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1080][1080]]] :: Together these MRs implemented the population of value members for integer and real binary operations

- *Split ast_to_asr* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1096][1096]]] :: A major code refactor for improved readablity and rapid development

- *tiny implementations* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1107][1107]], [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1112][1112]]] :: Together these implement the ~tiny~ intrinsic at compile time

- *LLVM values* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1131][1131]]] :: This enhancement patchset furthered the population of value members at the backend

- *Compile time functions (5)* :: ~real~ [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1114][1114]]], ~selected_kinds~ [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1154][1154]]], ~floor~ [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1176][1176]]], ~int~ [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1169][1169]]], ~Integer2Real~ cast [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1162][1162]]]

- *Runtime sin* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1167][1167]], [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1173][1173]]] :: Together these showcase the hopes and dreams of the current runtime library implementation

- *Testing framework for runtime benchmarks* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1194][1194]]] :: Jinja based rapid templating setup for benchmarking the runtime functions

- *Bugfixes* [[[https://gitlab.com/lfortran/lfortran/-/merge_requests/1015][1015]], [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1087][1087]]] :: These span various growing pains, and were debugged mostly via the stacktrace
** Reflections
...
- This is a rather vague point, given that we have a [[https://gitlab.com/lfortran/lfortran/-/issues][fantastic set of issues]]
- Also, [[https://lfortran.zulipchat.com/][the Zulip instance]] is great for getting in touch with the community
...
*** Target Practice
In terms of finally compiling our minimum viable computational chemistry project, minidftatom; described in the {{{relref(Week 5 post,gsoc21-w5)}}}, we are tantalizingly close; so much so that is a mere bugfix or two away; as can be seen from the state [[https://gitlab.com/lfortran/lfortran/-/merge_requests/1219][of this MR]]. Naturally my choice of issues are biased towards my own interests:
- Better benchmarking of the pure Fortran intrinsics
- Finish improving the CI (~ninja~)
- Meson integration
- More work on the runtime library
  + We plan to have multiple backends for the runtime (the [[https://gitlab.com/lfortran/lfortran/-/wikis/Intrinsic-Functions][relevant wiki]])
  + We need also to implement the compile time intrinsics in a more streamlined manner (a [[https://gitlab.com/lfortran/lfortran/-/issues/501][RFC issue]] for the same)
- Document all things!
...
- *Compile Time Intrinsics* :: These are the C++ equivalent of the Fortran standard, and are evaluated when possible at compile time (the backend then just uses the value)
- *Runtime Library Design* :: Inspite of [[https://fortran-lang.discourse.group/t/fortran-runtime-math-library/755][strong community opposition]] to implementing elementary functions in pure Fortran; we ended up with a very well designed elegant runtime design I am immensely satisfied with. Broadly speaking (detailed in the {{{relref(Week 10 post,gsoc21-w10)}}}), the design encompasses ~impure~ parts of the library, which cannot be implemented in pure Fortran, and ~pure~ sections which can be implemented without any external calls

Both of these intrinsic designs were accompanied by practical implementations and a template evolved over time to aid future contributions. One of the most interesting parts of the runtime design was using certifiably correct polynomial approximations generated with [[https://www.sollya.org/][Sollya]].
...
*** Learning
Compiler design touches almost every aspect of the compuation; from hardware to linkers, and I managed to expand my understanding of several key concepts early on with @cooperEngineeringCompiler2011a, @mullerElementaryFunctions2016, @levineLinkersLoaders2000 and a host of other texts referenced in the early weeks.

A special mention must be made of @J318007r1F2018, which happens to be the first full language standard I have read almost cover to cover.
** Misc Logs
...
*** Exp
The method to calculate the exponent is largely taken from @detreyParameterizedFloatingpointExponential2005. Basically this needs a reduction to $-\frac{log 2}{2}, \frac{log 2}{2}$ but just to get the ball rolling an initial terrible approximation is essentially:
...
Visual inspection and basic trignometry (along with series expansions) brings about the understanding that our polynomial approximation should only contain odd terms. This is easily accomplished in Sollya with [[https://www.sollya.org/sollya-4.1/help.php#fpminimax][fpminimax]].
#+begin_src bash
...
** Conclusions
I like writing; but somehow this was one of the most difficult things to write about; mostly because I'm still itching to get back to the codebase. I believe this is probably a good sign. Surprisingly, GSoC21 turned out to be one of my most enriching experiences as a developer till date. Unsurprising given the fantastic people involved, actually. There is very much left to be said about ~LFortran~, some of which will be said by me at the [[https://tcevents.chem.uzh.ch/event/14/contributions/69/][upcoming FortranCon 2021]], and in other outlets.
* TODO About F2PY
:PROPERTIES:
:EXPORT_FILE_NAME: about-f2py
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:END:
#+begin_quote
An introduction and rough interpretation
#+end_quote
** Background
As mentioned {{{relref(earlier,joining-quansight-labs21)}}}, I have the pleasure of being involved in an effort to modernize and maintain the
~f2py~ code base under the aegis of [[https://www.quansight.com/labs][Quansight Labs]] with  [[https://rgommers.github.io/][Dr. Ralf Gommers]],  [[https://melissawm.github.io/about-me/][Dr.
Melissa Weber Mendonça]] and [[https://github.com/pearu][Dr. Pearu Peterson]]. Here I will outline a few sample
tasks which serve as a gentle introduction to the ecosystem.
** Trivial Tasks
Several trivial tasks were to become routine over the course of my exploration of the existing code. Some of these are documented here for posterity. No claims of correctness or completeness can be entertained.
** Testing
Unit testing is being discussed for ~f2py~, however at the moment the gold
standard is to ensure that new changes do not break ~SciPy~. This then makes for
a good starting point to get acquainted with ~f2py~ in production.
*** Setting Up
Although I normally default to using ~nix~ for everything; in this particular
instance, I tend to take a more ~conda~ centric approach.
#+begin_src bash

#+end_src
* TODO Testing F2PY
** Slatec
[[http://www.netlib.org/slatec/][Netlib's slatec]] is a common mathematical library, circa 1993. This great candidate for ~f2py~; since it is:
- Written in ~F77~
- Overlaps with ~numpy~
  + Which means benchmarks become more feasible
Starting out is fairly routine:
#+begin_src bash
F2PYEX=$HOME/Git/Github/Fortran/f2py_examples
mkdir -p $F2PYEX && cd $F2PYEX
wget http://www.netlib.org/slatec/slatec_src.tgz
tar -xf slatec_src.tgz && rm -rf slatec_src.tgz
#+end_src
It so happens that ~slatec~ sources come without any build system whatsoever, which makes sense as, being a library, it isn't meant to be run as a standalone program. That said, it would be nice to actually build it. Thankfully the [[https://github.com/fortran-lang/fpm][Fortran Package Manager]] handles this very easily.
#+begin_src bash
fpm new slatec --lib
mv src slatec/src
cd slatec
# fpm build # gfortran 9.x.x or lower
fpm build --flag "-fallow-argument-mismatch" # anything above 7.x
#+end_src
Note that for ~gfortran~ versions greater than ~9.x.x~ an error will be thrown on rank mismatches (as opposed to a warning), but ~7.x.x~ does not recognize the flag and will error out with it.

SLATEC consists of 13 modules, many of which have been packaged elsewhere, making this a bit of a wasted effort.
* TODO Fortran and Spectroscopies
:PROPERTIES:
:EXPORT_FILE_NAME: fortran-spetroscopies
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/GSoC21.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:END:
#+begin_quote
Computational spectroscopy and the Fortran community
#+end_quote
** Background
I recently spent two weeks at the Les Houches School of Physics for a workshop on Multiple Scattering Green's Functions for Electronic Structure and Spectroscopy. Apart from being a fantastic place to work, I became acquainted with several Fortran programs for the same. This post pays homage to them.
** Conclusions
The school was an enriching experience, as I mentioned in passing to one of the more senior members, while stupid questions in person are still alright, online the ability to ask basic dumb big picture questions is severely reduced. It was also exhausting, due to my many existing commitments.
* DONE NumPy, Meson and ~f2py~ :@notes:python:fortran:numpy:f2py:ramblings:
CLOSED: [2021-09-23 Thu 05:45]
:PROPERTIES:
:EXPORT_FILE_NAME: numpy-meson-f2py
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Bridging Fortran \& Python")
:END:
#+begin_quote
Exploring ~meson~ for interfacing ~fortran~ and ~python~ via ~f2py~ and standard techniques, with benchmarks.
#+end_quote
** Background
A [[https://fortran-lang.discourse.group/t/about-f2py-ipc-and-wrappers/1838][recent post]] gauging community interest in ~f2py~ brought to light (among other
aspects) the fact that the build systems of ~f2py~ are rather opaque to the end
...
~f2py~[fn:whynomake]. Nevertheless, build systems have come a long way from the early 2000s
and the next two decades of ~f2py~, will certainly involve less dependence on
the soon-to-be-gone ~numpy.distutils~ approach to building extension modules.
** The Fibonacci example
Though rather contrived and overused[fn:whyover], for the rest of this post I will assume the following Fortran program, in a variety of guises.
#+begin_src fortran
C FILE: FIB1.F
      SUBROUTINE FIB(A,N)
C
...
#+end_src
This is good enough, and there are standard extensions to expose more information to the binding, detailed [[https://numpy.org/devdocs/f2py/f2py.getting-started.html][in the documentation]]. That, however isn't the point. The ~-c~ flag, convenient in many ways though it is, is a proxy for a rather more involved set of files.
#+begin_src bash
mkdir blah
...
custom command approach for ~CMake~ and ~scikit-build~ is a [[https://github.com/Nicholaswogan/skbuild-f2py-examples][nice workaround by
Nick Wogan]].
** Meson and F2PY
As ~scipy~ [[https://labs.quansight.org/blog/2021/07/moving-scipy-to-meson/][moves towards meson]] mainly driven by [[https://rgommers.github.io/][Ralf Gommers]], it makes sense to look into its use to drive the compilation of an extension module as well. We start by generating the files for the interface.
#+begin_src bash
f2py fib1.f -m fib
...
** Standard Bindings
The standard compliant approach was aptly covered almost a decade ago by [[https://ondrejcertik.com/][Ondřej Čertík]] and codified in his [[https://www.fortran90.org/src/best-practices.html#python-interface][fortran90 best practices guide]]. We start by updating our free-form code to be compliant with newer ~ISO_C_BINDING~ guidelines.
#+begin_src f90
module fib1
  use iso_c_binding
  implicit none
...
#+begin_quote
We have gone from over a $1000$ lines of ~C~ to *one line*[fn:wherecomplexity]
#+end_quote
*** Cython wrapper
...
This is closer to ~f2py~ in the sense that the ~C~ code is simply hidden. It requires a compilation step typically done via ~setuptools.py~, but, this is much nicer with ~meson~ since from ~0.59.0~, [[https://mesonbuild.com/Cython.html
][Cython is supported]].
#+begin_src python
project('test_builds', 'c',
  version : '0.1',
...
This is vastly more code compared to the ~f2py~ generated wrapper code; which
seems pretty unattractive.

*** Python-C API
Neither ~ctypes~ nor ~cpython~ is as friendly/clean as loading an extension
...
reference counting and familiarity with the [[https://numpy.org/devdocs/user/c-info.how-to-extend.html][NumPy-C API]]. Actually it is not too
...
wc -l fibbyhand.c
      85 fibbyhand.c
#+end_src
It is rather remarkable since the ~C~ code here is not optimized in the least.
The twin ~for~ loops can almost certainly be handled more efficiently, and also
...
** Benchmarks
We can use [[https://lib.rs/crates/hyperfine][hyperfine]] for basic benchmarks.
#+begin_src bash
...
#+end_src
Results for the various approaches are collected below in Table [[tbl:bench][tbl:bench]].
...
As expected, ~ctypes~ is the slowest, since it requires the most massaging of
inputs on the Python side. Surprisingly, loading the free form file with ~f2py~
was quite a bit slower than the ~f77~ file. Given the complexity of the files
...
** Conclusions
~f2py~, like ~Fortran~, is here to stay. As the standards strive towards better
compliance with ~C~ however, it also gets larger and rather more
complicated [fn:reallylarger]. One thing which was not covered here was the
actual ~ISO_Fortran_binding.h~[fn:findex] and other modifications to the ~C~
code while interpolating with Fortran itself.

To hold its place in the larger ecosystem, ~f2py~ will be essential in
protecting the average user from the standard itself, automatically providing
optimal bindings for users as initially planned by [[https://github.com/pearu][Pearu Peterson]]. Modernization
efforts with  [[https://melissawm.github.io/about-me/][Melissa Weber Mendonça]], Ralf, Pearu, and others at Quansight
(including the rest of the community and [[https://rgoswami.me/about][me]]) are sure to gravitate towards generating better, more reliable
interfaces to bridge the gaps between Fortran and Python as supported by the
NumPy core developers[fn:spthanks]. Clearly, the way forward is to work in
lockstep with the NumPy-C API and recent Fortran standards.

[fn:wherecomplexity] This is a bit unfair, the complexity has been offloaded to the compiler, and they're notorious for not supporting newer Fortran standards
[fn:findex] Some examples from the community [[https://fortran-lang.discourse.group/t/iso-c-binding-pass-an-array-from-c-to-fortran-edit-python-interop-content/514][are here]]
[fn:whynomake] ~Make~, ~autotools~ and the like are definitely worse than ~np.distutils~
[fn:whyover] It is the first example from the [[https://numpy.org/devdocs/f2py/f2py.getting-started.html][canonical reference]]
[fn:reallylarger] Though it is and will remain far more svelte compared to ~C++~
[fn:spthanks] Including a special shout out to [[https://github.com/charris][Charles Harris]] for reviewing unreasonably large DOC PRs
* DONE 42nd Annual Confereence of the Tex Users Group :@conferences:presentations:ramblings:docs:
CLOSED: [2021-08-07 Sat 15:39]
:PROPERTIES:
:EXPORT_FILE_NAME: tug-2021-meta
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+BEGIN_QUOTE
...
#+END_QUOTE
** Background
Much the same as the rationale behind my other presentation [[https://rgoswami.me/tags/presentations/][meta-posts]], but with some added shilling; since TUG is fantastic. Plus like the last year, I was gven the opportunity to coordiate social events with Jennifer Claudio.
** Social Events
- Since the 41st TUG in 2020; there is a Zulip instance which is active all year round [fn:whyhowzu]
  + Can be [[https://tug.zulipchat.com/join/z7zb37mniu4p6ooptvmmuoep/][joined here]]
- This year there was a Topia instance for more "pseudo-real" interactions
  + Details sent to [[https://tug.org/tug2021/index.html#register][people who registered]] (free)
** Slides
...
- A ~pdf~ copy of the slides are embedded below
- The ~orgmode~ source is here on the [[https://github.com/HaoZeke/haozeke.github.io/blob/src/presentations/TUG2021/tug21rgpres.org][site's GH repo]]
#+BEGIN_EXPORT html
<script async class="speakerdeck-embed" data-id="bed5bfe07de04f22a8a79c69a705c063" data-ratio="1.77966101694915" src="//speakerdeck.com/assets/embed.js"></script>
#+END_EXPORT
** Video
- As seen in the [[https://tug.org/tug2021/sched.html][schedule]]:
#+begin_export html
<iframe width="560" height="315" src="https://www.youtube.com/embed/EnrvU6kFp8Q" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
#+end_export
** Thoughts
...
** Conclusions
The ~TeX~ community is one of the best online communities around. There aren't any friendlier or better versed in the topics presented, so this is definitely a group I hope to continue to be a part of. There were some great suggestions for developing the concepts further to make things more user friendly as well; which might be great fodder for another presenation next year perhaps. Plus there's [[http://tug.org/TUGboat/][TUGBoat]].

[fn:whyhowzu] Zulip sponsors TUG as an open source community and provides Zulip Standard for our users
* TODO Compiler Design
In any case, though Ondrej knows exactly how I should implement the
assumed length, I decided it was time to dive into the LLVM infrastructure
itself, at least insofar as it concerns the ~lfortran~ project. This therefore,
is the first in a series on *Dragons and Automated Coding for
FormulaTranslation* [fn:notaname].
...
Now that I am a few weeks in, the workflow has roughly stabilized enough to
warrant documentation.  Typically for each feature implementation issue, I add a
screenshot from the relevant draft and also link to the ~pdf~. This is currently
either @J321007Draft or @J318007r1F2018 or even @J397007R295 as and when
appropriate. These
** Visualizing the IR
The first problem I ran into is that ~lfortran~ has representations for the
~AST~ and the ~ASR~, but I haven't covered yet how to output the LLVM
intermediate representation (IR) itself.

[fn:notaname] This is (not yet) a formal series, but might be
...
#+end_quote
** Background
* TODO Nix Flakes and NUR :@programming:tools:nix:workflow:
:PROPERTIES:
:EXPORT_FILE_NAME: nix-flakes
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
Upgrading to the bleeding edge of ~nix~ development.
#+end_quote
** Background
I shall be mentoring a fantastic group at for the inaugural [[http://summer.nixos.org/][Summer of Nix]]. The effort will be focused around the ~flake~ workflow, which is something I have been planning to look into for quite some time. Rather than have a post for each major language workflow, I'll just go over replacing all my existing workflows to use ~flakes~.
For a reminder, though I won't go into any real details here in this post, my preferred approach to ~nix~ in production is a combination of ~niv~ (which is brilliant) and throwaway ~nix-shell~ expressions, described in great detail in other posts on this blog, like {{{relref(this one,rethinking-r-nix)}}} or this {{{relref(introductory post,ccon-tut-nix)}}}.
** Getting Started
A couple of caveats about this post and using development features in general, this post might become outdated rather quickly. Anyway, the version used is:
#+begin_src bash
nix --version
#+end_src
...
To enable ~flakes~ we need some things set up:
- In ~$HOME/.config/nix/nix.conf~:
#+begin_src toml
# Nix Flakes
experimental-features = nix-command flakes
#+end_src
- We also need an unstable version for ~nix~:
#+begin_src nix
nix-env -f '<nixpkgs>' -iA nixUnstable
#+end_src
** Replacing Niv
Typically one of the key issues with ~nix~ is that impurities can sneak in in terms of the system's ~nix-channel~, along with other environment variables (like ~$NIX_PATH~). However, recent indications suggest that ~niv~ will [[https://www.reddit.com/r/NixOS/comments/ogy8vf/niv_naersk_napalm_moving_on/][not be supported as much]] in the near future.
*** A First Flake
The replacement is fairly straightforward. We can start with the ~niv~ equivalent:
#+begin_src bash
mkdir testFlakes && cd testFlakes
nix flake init
#+end_src

This creates a single ~flake.nix~ file, shown below with some minor comments, and running ~nix flake lock~ to get ~lockfile~ with all the details.
#+begin_src nix
{
...
This replaces in effect; a ~default.nix~ file, and can be built with ~nix build .~. The ~legacyPackages~ invocation essentially defines the derivation being used.

The ~flake.lock~ file generated can be used to get reproducible builds across machines. This is a simple first approximation of getting reproducibility across machines, but it is still a bit too magical. We still haven't pinned which version of ~nixpkgs~ we are using explicitly.
What we'd like to do is actually pin to a particular commit / branch. This is pretty straightforward as well, and we will need to modify the input section.
#+begin_src nix
{
  inputs = {
...
Note that we don't actually need to specify ~flake = true;~ since that is the default.
*** Basic Derivation Introspection
Just like ~nix-shell default.nix~ and ~nix-build --run-env~, ~nix develop~ allows us to inspect and run any of the phases defined in the ~default.nix~; for example:
#+begin_src bash
cd testFlakes
nix develop
# Viewing only
typeset -f buildPhase
typeset -f genericBuild
# Run
unpackPhase
configurePhase
buildPhase
...
This makes it easier to debug phases as well.
** Augmenting Existing Packages
Let us consider a stand-alone package; with a ~default.nix~ and ~shell.nix~. We will use [[https://dseams.info][d-SEAMS]] as a concrete example; since it is both in ~nixpkgs~ and uses ~niv~ and ~*.nix~ files in development.

** Flake Helpers
Just like with the standard workflow, we have some tools which can help matters move along. Additionally, the Nix User Repository is also distributed in a flake-friendly manner, which we can use to our advantage.
*** Flake Utilities
For migrating existing package derivations, the ~flake-utils~ [[https://github.com/numtide/flake-utils][of numtide]] are a fantastic.
#+begin_src nix
{
...
#+end_src
*** NUR Usage
A basic flake using the NUR is then:
#+begin_src nix
{
...
#+end_src
** Python
To work with ~python~ I typically use ~mach-nix~ as {{{relref(described here,mach-nix-niv-python)}}}. However, with a ~flake~.
** Concrete Example: GPAW
As an example of a relatively interesting package we will consider augmenting [[https://wiki.fysik.dtu.dk/gpaw/index.html][DTU's GPAW]] with a ~nix~ flake.
** Resources
Some other great starting resources are:
- The ~nix flake~ [[https://nixos.org/manual/nix/unstable/command-ref/new-cli/nix3-flake.html][manpage]]
- A fantastic series by Tweag
  + Nix Flakes, Part 1: [[https://www.tweag.io/blog/2020-05-25-flakes/][An introduction and tutorial]]
  + Nix Flakes, Part 2: [[https://www.tweag.io/blog/2020-06-25-eval-cache/][Evaluation caching]]
  + Nix Flakes, Part 3: [[https://www.tweag.io/blog/2020-07-31-nixos-flakes/][Managing NixOS systems]]
- NixOS [[https://nixos.wiki/wiki/Flakes][Wiki]]
- Zimbatm has a [[https://zimbatm.com/NixFlakes/][nice source]] with more resources
** Conclusions
~flakes~ are the future of ~nix~. Their adoption makes it easier to decentralize and shift away from the unwieldy ~channel~ dissemination format and allows packagers to work cleanly with different variants.
* TODO Nix and Meson
** Background
** Meson and Nix
Unlike ~cmake~, which ~nix~ is able to pick up on, ~meson~ does not seem to be able to use any of.
** Meson Pain Points
- As of ~0.59.0~, it is not possible to use ~meson init~ with libraries
#+begin_src bash
meson init -C . --type library --name ams
Using "ams" (project name) as name of executable to build.
Detected source files: AMS.cpp
...
This means that each sub-directory needs to be configured by hand.
* DONE Nix, Channels and the NUR :@programming:tools:nix:workflow:
CLOSED: [2021-07-21 Wed 02:01]
:PROPERTIES:
:EXPORT_FILE_NAME: nix-channel-nur
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
...
#+end_quote
** Background
For multi-user ~nix~ installations [fn:whocares], the ~NIX_PATH~ variable is empty. Here I briefly go over two approaches to mitigate this, one with ~nix-channel~ and the other by manual pinning. Note that this post will eventually be superseded for most cases by a better ~flake~ workflow.
** Channels
...
- For more on ~packageOverrides~, there's [[https://nixos.org/guides/nix-pills/nixpkgs-overriding-packages.html][a pill]] for that
- For a refresher on ~profiles~, there's the ~nix~ manual, [[https://nixos.org/manual/nix/stable/#sec-profiles][specifically chapter 10]]
- Caveat :: Defining ~nixpkgs~ as a channel causes ~warning: name collision in input Nix expressions~ and it gets skipped
- Caveat :: *The problem with channels* is of course that it is hard to keep control of update which occur when users run ~nix-channel --update~
** Pinning Packages
...
#+end_src
** Conclusions
Much of the channel workflow is discussed directly [[https://github.com/nix-community/NUR/issues/150][in this issue]]. The main usage of this is actually to setup and use user defined packages in a more natural way, and to move away from ~channels~ which typically only update ~NIX_PATH~ when run as ~root~, which in turn [[https://github.com/NixOS/nix/issues/2033][causes much confusion]]. NUR is also ~flake~ compatible and this will be explored further in a another post.

[fn:whocares] Like the setup on the ~elja~ super-computing cluster of the [[https://irhpc.github.io/docs/intro][Icelandic Research High Performance Compute]] cluster
* DONE Doom Emacs and Language Servers :@programming:workflow:projects:tools:emacs:
CLOSED: [2021-07-20 Tue 02:28]
:PROPERTIES:
:EXPORT_FILE_NAME: emacs-lang-servers
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
...
#+end_quote
** Background
For most of my ~emacs~ configuration[fn:howemacs], there normally isn't very much to write about which isn't immediately evident from my [[https://dotdoom.rgoswami.me][configuration site]]. However, since my shift to a MacBook, I have needed to fine tune my existing ~lsp-mode~ default setup for ~TRAMP~ and this post will cover a little bit of that. Though most of the post is about ~doom-emacs~, it is also applicable to vanilla emacs after porting the snippets over to ~use-package~ instead.
...
- [[https://github.com/joaotavora/eglot][eglot]] :: Feels more minimal, maintained by the author of ~yassnippet~
- [[https://github.com/emacs-lsp/lsp-mode/][lsp-mode]] :: Much more configuration, better documentation in some aspects, strangely difficult ~TRAMP~ setup
...
#+DOWNLOADED: screenshot @ 2021-07-19 20:19:38
[[file:images/Language_Servers/2021-07-19_20-19-38_screenshot.png]]
...
*** Caveats
- No ~projectile~ support planned (though there [[https://github.com/joaotavora/eglot/issues/129][are workarounds]])
  + Uses ~project.el~ to pick root directories (i.e. ~.git~)
    - In particular this means it works poorly with ~git~ sub-modules

Overall the main issue with ~eglot~ seems to be the insistence to be accepted into ~emacs~ core someday. This is great, and a good direction for the project to grow in, but it constrains my workflow unnecessarily. I'm more interested in working with ~doom-emacs~ than vanilla ~emacs~ and so am pretty invested in non-core libraries like ~projectile~ [fn:nofair].
** Language Servers
For getting the language server providers themselves, we will mostly leverage direct binaries where possible, but also, depending on the implementation, virtual environments[fn:nonix]. Essentially we have the following Rosetta stone.
#+caption: Language servers and installation methods
...
#+end_src
*** C++
For ~clangd~ (details [[https://clangd.llvm.org/installation.html][here]]) and ~tramp~ this amounted to:
#+begin_src bash
mkdir -p ~/.local/lsp
cd ~/.local/lsp
...
In combination with the standard ~doom-emacs~ [[https://github.com/hlissner/doom-emacs/tree/develop/modules/lang/cc][cc module]], this is a very good workflow, with the only issue being the ability to set the project root and files to be considered. There is ~doom-emacs~ version of appending and setting the language server used, which will be used here:
#+begin_src emacs-lisp
(after! eglot
  :config
...
This will ensure the best experience, since the ~compile_commands.json~ (described [[https://clang.llvm.org/docs/JSONCompilationDatabase.html][here]]) file contains paths to all the files used, including system and write-only files [fn:whocares]. The setup here works well with enough with the ~ccls~ language server as well, which in turn is a more maintained fork of ~cquery~.
*** Python
Palantir's ~python-language-server~ is EOL, and was [[https://github.com/joaotavora/eglot/pull/688][weird to begin with]], but the Spyder team has taken up the gauntlet and maintains the ~python-lsp-server~ ([[https://github.com/python-lsp/python-lsp-server][repo]]).
#+begin_src bash
micromamba activate lsp # should be done in the ~/.bashrc
micromamba install python-lsp-server[all] -c conda-forge
#+end_src
For working with this in an optimal manner the ~eglot~ [[https://github.com/joaotavora/eglot][readme]] suggests using ~.dir-local.el~ files. For example:
#+begin_src emacs-lisp
((python-mode
...
#+end_src
*** Bash
This is fairly straightforward, and implemented in ~js~ [[https://github.com/mads-hartmann/bash-language-server][here]].
#+begin_src bash
nvm use node
...
#+end_src
*** Fortran
The installation of ~fortls~ ([[https://github.com/hansec/fortran-language-server][repo]]) is simple enough.
#+begin_src bash
micromamba activate lsp
...
#+DOWNLOADED: screenshot @ 2021-07-20 01:23:39
[[file:images/Fortran/2021-07-20_01-23-39_screenshot.png]]

*** Nix
~rnix-lsp~ ([[https://github.com/nix-community/rnix-lsp][repo]]) has no non ~nix~ installation, and a slightly uncertain future, but it is still fantastic and shouldn't be left out[fn:tragedy].
#+begin_src bash
nix-env -i -f https://github.com/nix-community/rnix-lsp/archive/master.tar.gz
nix-env -i nixkpgs-fmt
#+end_src
...
#+end_src
*** TeX
The last of the language servers, ~digestif~ ([[https://github.com/astoff/digestif][repo]]) is implemented in ~lua~, and probably shouldn't be on a remote machine[fn:whynotremote], but nevertheless.
#+begin_src bash
...
** Conclusions
Embarrassingly, neither setup provided as pleasant an SSH based workflow as VS Code. However, that probably has a lot more to do with TRAMP than any of the language servers, and in any case [[https://magit.vc/][Magit]] still makes ~emacs~ far superior. In any case, the setup described here is still far superior to not using anything other than ~projectile~ and ~ripgrep~ so I am satisfied for now. The path setup in this post is deliberately polluting for simplification however, and in practice [[https://github.com/HaoZeke/Dotfiles/blob/bombadil/readme.org][my Dotfiles]] are managed a lot better with ~bombadil~ which is something for a later post. Each of these will take some getting used to, and new keybindings will probably be needed, especially for my {{{relref(idiomatic Colemak setup,colemak-dots-refactor)}}}.

[fn:howemacs] More than adequately managed by ~hlissner~'s fantastic [[https://github.com/hlissner/doom-emacs][doom-emacs]]
[fn:nofair] If you need more information, Doug Davis has a more [[https://ddavis.io/posts/eglot-cpp-ide/][in-depth vanilla emacs write-up]] on ~eglot~ and C++
[fn:nonix] Though my university cluster has ~nix~, most remote machines do not, so we don't depend on it here
[fn:whocares] This means, libraries like ~Eigen~ or even the standard library can be looked up via ~eglot~
[fn:tragedy] The original author [[https://www.redox-os.org/news/open-source-mental-health/][unfortunately passed away earlier]] this year
[fn:whynotremote] Visualizing the resultant ~pdf~ would be a chore
* TODO Spacehammer and Hammerspoon
Though ~lua~ has great syntax, for the particular setup (key remaps) there is an elegant framework, [[https://github.com/agzam/spacehammer][Spacehammer]] which leverages the lisp-like [[https://fennel-lang.org/][Fennel]] for configurations. It is a little bit more managable for larger configurations, so we will consider it briefly.
...
#+html: {{< youtube wsn8c5vSBD8 >}}

[fn:officialsite] One location I am going to be able to keep track of

* DONE Talk Supplements for SERI'21 :@conferences:presentations:ramblings:docs:
CLOSED: [2021-07-18 Sun 21:44]
:PROPERTIES:
:EXPORT_FILE_NAME: seri-2021-meta
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments false
:END:
#+BEGIN_QUOTE
A meta-post on a talk I was to give at SERI'21
#+END_QUOTE
** Background
Much the same as the rationale behind my other presentation [[https://rgoswami.me/tags/presentations/][meta-posts]], that is:
- I would like to preserve questions
...
- It would be nice to have my own thoughts here afterwards
*** Details
[[https://isoft.acm.org/seri2021/][SERI or the Software Engineering Research in India]] meeting is the only informal event dedicated to software engineering in the subcontinent, so it was rather gratifying to be accepted, and, to present on *Modern documentation across languages*.
** Slides
- [[file:/revealjs/seri21/docuSeri.html][Best viewed here]] using a browser (in a new tab)
- A ~pdf~ copy of the slides are embedded below
- The ~orgmode~ source is here on the [[https://github.com/HaoZeke/haozeke.github.io/tree/src/presentations/SERI2021/acmReveal.org][site's GH repo]]
#+BEGIN_EXPORT html
<script async class="speakerdeck-embed" data-id="6d2872a798a24c2495e904e59fa898f4" data-ratio="1.37081659973226" src="//speakerdeck.com/assets/embed.js"></script>
#+END_EXPORT
** Video

#+html: {{< youtube weEqpRUvNqk >}}

[fn:officialsite] One location I am going to be able to keep track of

* DONE JuliaCon and Remote Machines
CLOSED: [2021-07-25 Sun 01:40]
:PROPERTIES:
:EXPORT_FILE_NAME: juliacon-remote-machines
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
...
The code snippets arise in the [[https://github.com/mfherbst/juliacon_dft_workshop][context of the DFT workshop]][fn:whydft], but are applicable across all the talks. Additionally, the snippets will assume *no forks* but in practice it is best to clone modifiable forks of these repositories so your mutations last forever.
** Paths and Directories
For me, I use the following layout:
#+begin_src bash
export jc21=$HOME/Git/Github/Julia/juliacon21
mkdir -p $jc21
cd $jc21
git clone git@github.com:mfherbst/juliacon_dft_workshop.git
#+end_src
** Environment
...
#+begin_src bash
cd $jc21
micromamba create -p ./.jc21tmp pymatgen python==3.8 jupyterlab pip ipython
micromamba activate $(pwd)/.jc21tmp
#+end_src
...
#+end_src
** Exposing Access
To make sure we can access our remote server we can use ~ngrok~ [fn:whyngrok] or ~ssh~ forwarding [fn:nothere].
#+begin_src bash
...
We have assumed again, that ~$HOME/.local/bin~ is in our path.
~tmux~ is optional, but it is fantastic for all headless setups.It is probably better to not do this on your login / head node, and with ~ngrok~ we don't need to care about how deeply nested we are in terms of exposing the tunnel, so just grab an interactive job. Assuming ~tmux~ then, the workflow is simply:
#+begin_src bash
ssh mysuperhpc
# 6.5 hours on 1 node
srun -N 1 -t 6:30:00 --pty /bin/bash
# Or whatever configuration works for you
tmux new -s jc21 # enter an environment
cd $jc21
micromamba activate $(pwd)/.jc21tmp
...
#+DOWNLOADED: screenshot @ 2021-07-25 01:39:06
[[file:images/Exposing_Access/2021-07-25_01-39-06_screenshot.png]]

Now we get a standard server with the appropriate kernel and can continue working through the workshops.

#+DOWNLOADED: screenshot @ 2021-07-25 01:40:40
[[file:images/Exposing_Access/2021-07-25_01-40-40_screenshot.png]]
...
[fn:whyngrok] Alan Shreve is a fantastic person and ~ngrok~ blows all competition out of the water, great student pricing too
[fn:nothere] This is a [[https://benjlindsay.com/posts/running-jupyter-lab-remotely][very good resource]] about setting up the SSH chain required, but ~ngrok~ *just works* across nodes too
[fn:whydft] Most of my existing experience with other codes like Quantum Espresso were at CECAM events with VMs... so it wouldn't be fair to plunge into ~dftk~ on my laptop


* DONE The State of my Dots :@personal:ramblings:tools:workflow:config:
CLOSED: [2023-08-30 Tue 21:37]
:PROPERTIES:
:EXPORT_FILE_NAME: mig-dots-chezmoi
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("$HOME...")
:END:
#+begin_quote
Yet another post on provisioning machines with ~dotfiles~...
now with ~chezmoi~
#+end_quote
** Background
+Barely two months ago+ A few years ago, {{{relref(I wrote about,dotfiles-dotgit-bombadil)}}} moving away from the
+(now)+ ~python~ based [[https://github.com/EvanPurkhiser/dots][dots]] to the ~rust~ based [[https://github.com/oknozor/toml-bombadil][toml-bombadil]]. Unfortunately, the
structure I backed myself into there was needlessly complicated, and involved
manually making entries in the ~bombadil.toml~ file for each sub file. This was a
good workflow in principle, since it enforces separation of concerns, but it was
rather annoying in practice [fn:maybenomore].

This post got sidelined a bunch of times over the years, as did a bunch of my
work in progress posts, mostly because there were always some warts to be ironed
out.
*** HPC Configurations
Around the same time as this post, I was {{{relref(writing up my trials,hpc-conf-2021)}}} and tribulations
as a lowly user without admin access of the (then) new ~elja~ HPC cluster at the
University of Iceland. Anyway, since HPC work (user / admin / whatever) is one
of my main use-cases, this post could also be seen as a sequel of sorts to that
one.
** Dotfile management with ~chezmoi~
[[https://github.com/twpayne/chezmoi/][chezmoi]] is almost an institution in and of itself, with [[https://www.chezmoi.io/][detailed documentation]]
and a plethora of users. Rather than go over the features of ~chezmoi~, I'll
outline the components of my ~dotfiles~ as they appear in the course of setting
up a new user instance on a remote headless machine [fn:whatwhere].

** Conclusions
There's not a whole lot to say about the configuration. My setup has been fairly
stable across a few systems with ~chezmoi~ but I haven't also really shifted
machines very much. There are a few moving parts I wish were simpler, like
setting up encryption, but ~chezmoi~ is the sort of fully loaded footgun one
needs while adrift on a sea of random impersonal ~/bin/bash~ terminals.
...
Info
#+end_alert-heading
As an aside, there are *a lot* of backlogged posts I meant to get around to
writing but no longer feel all that strongly about. Most of these are written up
in the {{{relref(original spirit of this blog,rationale)}}} (TL;DR: just write stuff down).
#+end_alert


[fn:maybenomore] I haven't used ~bombadil~ in a few years now, it might have changed
[fn:whatwhere] An HPC (~elja~, which I used to work more with).
* DONE Regarding Quansight Labs :@personal:history:ramblings:fortran:
CLOSED: [2021-08-29 Sun 02:05]
:PROPERTIES:
:EXPORT_FILE_NAME: joining-quansight-labs21
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments false
:END:
#+begin_quote
...
#+end_quote
** Background
Rumors of my affiliation with Quansight Labs are indeed true. I have had the pleasure of joining the Labs division as a software engineer [fn:whatabouttime], whose [[https://labs.quansight.org/blog/2019/05/community-driven-opensource-funded-development/][philosophy is fantastically and succintly described]] by their director [[https://rgommers.github.io/][Dr. Ralf Gommers]]. This means, in nutshell, that in my spare time I will be able to contribute to the continued maintenance of key FOSS projects.
*** Why?
A reasonable question. I have been contributing to and managing FOSS projects for over ten years now under the assumption of one day aspiring to be described by Fig. [[fig:xkcddep]]. Over time however, there have been consequences which have required a restructuring of how I view my commitments. The projects I have had the pleasure to work have grown in scope to the point where it became increasingly difficult keep contributing to at a sustained pace.

#+label: fig:xkcddep
#+name: fig:xkcddep
#+caption: A vision of my understanding of FOSS maintenance [[https://xkcd.com/2347][courtesy xkcd]]
file:./images/xkcd/dependency_2x.png
** Interviews
...
- [[https://rgommers.github.io/][Ralf Gommers]] :: Ralf and I discussed several projects which might benefit from my commitment along with wider contexts surrounding FOSS ecosystems; I was also reintroduced to the ~meson~ build system
- [[https://chrisostrouchov.com/about/][Chris Ostrouchov]] :: We discussed the ~nix~ ecosystem and how it could and should be used to gain more ground in the High Performance Computing community [fn:whythis]
- [[https://github.com/IvanYashchuk][Ivan Yashchuk]] :: I met with Ivan, and we discussed parallelism in C++, the standards committee, and he introduced me to SYCL
These interviews convinced me that I would continue to grow during the proposed projects.
** Projects
For starters I shall be working on the ~f2py~ section of  [[https://melissawm.github.io/about-me/][Dr. Melissa Weber Mendonça]]'s [[https://figshare.com/articles/online_resource/Improving_usability_and_sustainability_for_NumPy_and_OpenBLAS/13269167][CZI grant]] with [[https://github.com/pearu][Dr. Pearu Peterson]].
** Misc
...
[fn:whythis] Which tied into a larger discussion taking place also at [[https://www.tweag.io/][Tweag]] (and the [[https://irhpc.github.io/][Icelandic HPC cluster]]) revolving around Nix-HPC
[fn:whatabouttime] For 20-25 hours a week
* DONE Presentation Supplements for Greens Function School :@conferences:presentations:ramblings:fortran:
CLOSED: [2021-09-10 Fri 13:12]
:PROPERTIES:
:EXPORT_FILE_NAME: leshouches-2021-meta
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/GSoC21.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments false :link-citations true
:END:
#+BEGIN_QUOTE
A meta-post on an oral presentation around Fortran and languages
#+END_QUOTE
** Background
I was a participant at the 2021 Les Houches school of physics on "[[https://www.ebert.cup.uni-muenchen.de/kkr/summer_school_2020_des_houches/home][Green's function approach to multiple scattering theory in electronic structure and spectroscopies]]". I opted to give a student talk on programming languages and elementary functions as a cautionary tale for IEEE 754.
*** Details
...
*** Slides
- [[file:/revealjs/lesHouches21/progLangInters.html][Best viewed here]] using a browser (in a new tab)
- A ~pdf~ copy of the slides are embedded below
- The ~orgmode~ source [[https://raw.githubusercontent.com/HaoZeke/haozeke.github.io/src/presentations/lesHouches21/fortran_aseViz.org][is here on the site's GH repo]]
#+BEGIN_EXPORT html
<script async class="speakerdeck-embed" data-id="a6e11d26499b44b2bcb1cb630803de0d" data-ratio="1.37081659973226" src="//speakerdeck.com/assets/embed.js"></script>
#+END_EXPORT
** Video
TBD
** Thoughts
This was the first time in a long while since I gave an in-person short presentation. I should have used a timer.
* DONE Presentation Supplements for FortranCon'21 :@conferences:presentations:ramblings:fortran:
CLOSED: [2021-09-24 Fri 01:37]
:PROPERTIES:
:EXPORT_FILE_NAME: fortrancon-2021-meta
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments false
:END:
#+BEGIN_QUOTE
...
#+END_QUOTE
** Background
I had the honor and pleasure to have two presentations at the [[https://tcevents.chem.uzh.ch/event/14/overview][second annual FortranCon]] in 2021. The fun part was both talks featured the language standard prominently, but were meant to form two sides of a coin.
** f2py: Two Decades Later
- Co-Authors :: [[https://rgommers.github.io/][Dr. Ralf Gommers]] (Quansight Labs), [[https://melissawm.github.io/][Dr. Melissa Mendonca]] (Quansight Labs), [[https://github.com/pearu][Dr. Pearu Peterson]] (Quansight Labs)
...
#+END_QUOTE
**** Other Content
- {{{relref(NumPy Meson and ~f2py~,numpy-meson-f2py)}}}
*** Slides
- [[file:/revealjs/fortrancon21/quansight/f2py2declater.html][Best viewed here]] using a browser (in a new tab)
- A ~pdf~ copy of the slides are embedded below
- The ~orgmode~ source [[https://raw.githubusercontent.com/HaoZeke/haozeke.github.io/src/presentations/fortranCon21/quansightF2PY/fortrancon_quansightf2py.org][is here on the site's GH repo]]
#+BEGIN_EXPORT html
<script async class="speakerdeck-embed" data-id="d3840612af464e5baea5ee5e6dcd6692" data-ratio="1.37081659973226" src="//speakerdeck.com/assets/embed.js"></script>
#+END_EXPORT
*** Video

#+html: {{< youtube 9nMSKcRimuw >}}

** Implementing Fortran Standardese within LFortran
- Co-Author(s) :: [[https://ondrejcertik.com/][Dr. Ondřej Čertík]] (Los Alamos National Laboratory)
- Duration :: 5 minutes
*** Abstract
#+BEGIN_QUOTE
...
0. {{{relref(Fortran GSoC21 and Me,fortran-gsoc-me)}}}

1. {{{relref(GSoC21 W1: LFortran Kickoff,gsoc21-w1)}}}

2. {{{relref(GSoC21 W2: LFortran Unraveling,gsoc21-w2)}}}

3. {{{relref(GSoC21 W3: Kind Characters and Standards,gsoc21-w3)}}}

4. {{{relref(GSoC21 W4: LFortran Backends and Bugs,gsoc21-w4)}}}

5. {{{relref(GSoC21 W5: LFortran Design Details and GSoC21 W5: LFortran Design Details and minidftatom,gsoc21-w5)}}}

6. {{{relref(GSoC21 W6: LFortran ASR and Values,gsoc21-w6)}}}

7. {{{relref(GSoC21 W7: LFortran Workflow Basics,gsoc21-w7)}}}

8. {{{relref(GSoC21 W8: LFortran Refactors and Compile Time Evaluations,gsoc21-w8)}}}

9. {{{relref(GSoC21 W9: LFortran Bug Hunting Bonanza,gsoc21-w9)}}}

10. {{{relref(GSoC21 W10: LFortran Runtime Library Design,gsoc21-w10)}}}

11. {{{relref(GSoC21 LFortran and Computational Chemistry,gsoc21-fin-reprot)}}}

*** Slides
- [[file:/revealjs/fortrancon21/gsoc/implFSLFortran.html][Best viewed here]] using a browser (in a new tab)
- A ~pdf~ copy of the slides are embedded below
- The ~orgmode~ source [[https://raw.githubusercontent.com/HaoZeke/haozeke.github.io/src/presentations/fortranCon21/gsocLFortran/fortrancon_gsoclfortran.org][is here on the site's GH repo]]
#+BEGIN_EXPORT html
<script async class="speakerdeck-embed" data-id="b663e416185f43d88a9598ab0c221901" data-ratio="1.37081659973226" src="//speakerdeck.com/assets/embed.js"></script>
#+END_EXPORT
*** Video
From around 10:43 in.

#+html: {{< youtube 4236hgpMBLY >}}

** Thoughts
I could go on about these topics all day, thankfully for the most part, I do.
* DONE Simple Fortran Derived Types and Python :@notes:python:fortran:numpy:f2py:ramblings:
CLOSED: [2021-10-02 Sat 04:19]
:PROPERTIES:
:EXPORT_FILE_NAME: cython-derivedtype-f2py
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/Quansight.bib, biblio/refs.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Bridging Fortran \& Python")
:END:
#+begin_quote
...
#+end_quote
** Background
Object oriented programming has been part of Fortran for longer than I have been
alive [fn:sowhat]. Fortran has derived types now. They've been around for
around for over three decades. The standards at the same time, have been
supporting more and more interoperable operations. Details of these pleasant
historical improvements are pretty much the most the Fortran standards committee
have managed to date in the 21st century. The point of this exploration is to
design interfaces to ~Python~, mostly for the long term advancement of ~f2py~,
and just for the heck of it. Simple in the context of the title means that we consider only derived types whose members have direct ~C~ equivalents as defined in the Fortran 2003 @reidNewFeaturesFortran2003 standard.
...
Now by definition, ever since the ~ISO_C_BINDING~ of the Fortran 2003 @reidNewFeaturesFortran2003 standard, we know this is equivalent to:
#+begin_src C
typedef struct {
  double x,y,z;
} cartesian;
...
The function declaration is straightforward, but for the fact that since we are
passing something both in and out of the function, it must be a pointer, and as
we have no real error propagation or feasible return type, ~void *~ makes sense
too. The calling semantics are equally transparent, creating an instance of the
~struct~ and then passing it by reference.
...
*** Cython and Fortran
So far so good. Let's get to something slightly more interesting. Although in
the long run ~cython~ isn't really the approach ~numpy~ is going towards at the
moment, it still makes sense to give it a shot as a first approximation.
#+begin_src cython
...
The most interesting aspect is that, having defined our ~C~ structure and the
external function, we need something to offer to ~Python~. Since ~cython~
structures can be mapped from simple ~python~ dictionaries, we take an input of
one, construct the ~structure~ on the fly in the function and then use our
Fortran subroutine. The return type can also be coerced into a list, and that's
what can be printed or manipulated further. This isn't the most efficient way to
work with this, but it will do for now.

The ~meson~ update follows from the explanation in the previous post, namely,
that we need to include our ~Python~ sources.
#+begin_src meson
...
The ~NumPy~ dependency in this situation is really optional, but makes for a
more interoperable structure. Now we're ready to try this out in an
interactive manner.
#+begin_src bash
...
print(opoint)
#+end_src
Aside from the output string, this works as expected, and is pretty neat, all
things considered. This might feed into an ~f2py~ workflow, but then, it adds a
layer of indirection which seems overly circuitous. The design could have also
been far improved by using a ~bind(c, name=uniquname)~ as well.
...
It so happens that much of the information needed for generation of the relevant
~C~ code is already present in ~f2py~, and can be inspected via the
~crackfortran~ module.
...
The most natural way to feed into ~f2py~ would be to directly write out an
equivalent ~Python-C~ API which effectively mimics the extension module of the
last section.
...
** Conclusions
Slowly but surely, compilers have caught up with to the standards, and soon,
with some support, ~numpy~ shall too. A subsequent post shall cover more exotic
derived types, typically (until 2018) treated by non-portable opaque pointer
tricks. There are still some rather awkward design decisions in this case;
including things like considering the mapping and their interoperability with
~NumPy~ arrays, but by and large this would depend on the functions which use
these. In terms of derived types of simple structures, without allocatable
arrays, the extensions appear to be straightforward.

[fn:sowhat] This is more surprising with each passing year
* TODO F2PY and the Python Packaging Story :@notes:python:ramblings:fortran:f2py:
** Background
One of the nicer aspects of working on the LFortran project and considering its
long term viability with [[https://ondrejcertik.com/][Ondřej Čertík]] is its modular layout. This makes it
possible for example, for a contributor to work on the frontend without worrying
about the backend or the middle end and vice versa. This enviable position is
purely one of good design, and one of the pain points in the F2PY ecosystem is
that its embedding into NumPy is more grown in an adhoc way than actually
designed.

** Build systems
No discussion about any Python packaging story can be considered complete
without talking about [[https://www.python.org/dev/peps/pep-0517/][PEP 517]], [[https://www.python.org/dev/peps/pep-0518][PEP 518]] and a bunch of specification extensions
like [[https://www.python.org/dev/peps/pep-0660][PEP 660]]. However, none of these really matter for the F2PY context. Readers
are referred to [[https://snarky.ca/clarifying-pep-518/?utm_source=pocket_mylist][better explanations]] ([[https://blog.ganssle.io/articles/2021/10/setup-py-deprecated.html][including this]]) or just more [[https://discuss.python.org/t/pronouncement-on-peps-660-and-662-editable-installs/9450][opinionated
ones]]. Suffice to say that there are [[https://discuss.python.org/t/building-extension-modules-the-2020-way/5950][very few overall success stories]] /
directions for projects with a lot of Python-C API code, and projects still end
up having to hack around ~setup.py~ files.

That said, a relatively compliant method is to simply use [[https://thiblahute.gitlab.io/mesonpep517/][mesonpep517]] or
[[https://pypi.org/project/cmake-build-extension/][cmake-build-extension]] or [[https://pypi.org/project/enscons/][enscons]]. Neither of these are foolproof, but while the
standardisations keep moving towards abstract interfaces in the name of
flexibility it seems likely that neither of these are going anywhere any time
soon.

** Design
Originally intended to rip out the ~numpy~ namespace and provide a testbed for
pure ~f2py~ content, I later realized that for ease of interoperability with the
upstream would be to retain a "empty" namespace approach.
*** Sidebar: NumPy Modules
This is a good point at which to catch up with the design of NumPy itself (from
a bird's eye perspective).

All we really need to know is easily found out with ~tree~.
#+begin_src bash
/usr/bin/tree -P 'setup.py' --prune -L 2 numpy
numpy
├── array_api
│   └── setup.py
├── compat
...
In other words, there are 14 sub-modules within ~numpy~ of which ~f2py~ is one.
It is slightly different than the rest in that it is both a library and a CLI,
but that's a problem for the refactoring. The idiosyncrasies of the actual files
can also be left for later. We only really care to check for this pattern since
it encourages the possibility of a clean namespace implementation.

* TODO Logging and Argument Parsing for F2PY
Part of the struggle of retaining users has to do with the fact that during the active development of ~f2py~ standard user friendly modules like ~logging~ and ~argparse~ were in their infancy. A natural remedy suggests itself.
** Command Line Interfaces
There are a plethora of libraries for coloring and styling the output. Far too many to list or enumerate. Also, more importantly, none of them matter. A library like ~numpy~ can and should be averse to adding new dependencies. Instead we will work with the standard library's ~argparse~.
*** F2PY CLI 101
The ~f2py~ interface is meant to facilitate three different end-user cases:
- Wrapper generation :: What it says on the tin, this generates some Fortran wrappers and also the ~.c~ files which are later built by any means into a python extension module
- Signature file generation :: To fully unlock the power and uniqueness of ~f2py~ [fn:whatunique] [[https://numpy.org/devdocs/f2py/signature-file.html][a full grammar of extensions]] are defined, documented and can be used by either commenting codes or just generating the signature files
- Compilation :: Via the ~-c~ flag, ~f2py~ becomes a sort of pimp for ~np.distutils~, producing the ~.so~ file without so much as a visible set of source headers and translation units

So far so good. Now the older design will not be discussed here, but it was
rather good for its time, being reminiscent of standard UNIX commands, perhaps
with the exception of the ~-h~ flag which doesn't do what one might immediately
expect.

*** Enter ~argparse~
The +three heads of Cerberus+ three end-user functionalities make for an almost
perfect use case for the ~subparser~. Almost in the sense that, well, to ensure
backwards compatibility [fn:lolback] we can't abruptly remove the flags ~f2py~
used to take. So we end up with a rather odd structure.

Another very fun fact is that actually the wrapper generation and signature file
generation take the same options. This means there need be only two sub-parsers.

**** Colors
ANSI colors are easy, at-least they have been ever since Windows began stepping
up their game and providing ANSI code support. In any case, a partial workaround
is ~os.system("color")~ on these operating systems. Personally I was rather
looking forward to using perhaps [[https://github.com/tartley/colorama][colorama]] or [[https://github.com/SuperMaZingCoder/colorit][color-it]]. In fact, since ~color-it~
is MIT licensed and tiny, it might even get vendored in perhaps. Possibly.
Anyway, back to the harsh unyielding present. Color support is essentially just
an ANSI string.

[fn:whatunique] Like perhaps wrapping ~.c~ code way [[https://scipy-cookbook.readthedocs.io/items/f2py_and_NumPy.html][back in 2006]], or exotic callbacks
[fn:lolback] A wonderful endeavor, much like the way aeroplanes and horse driven sleighs are still compatible... *wait*..
* DONE Exploring ~ISO_C_BINDING~ and type-bound procedures :@notes:fortran:f2py:ramblings:
CLOSED: [2021-12-19 Sun 16:25]
:PROPERTIES:
:EXPORT_FILE_NAME: iso-c-type-bound-fortran
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/Quansight.bib, biblio/refs.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Bridging Fortran \& Python")
...
#+begin_quote
A closer look at the standard Fortran C compatibility layer by
exploring objects and linkers
#+end_quote
** Background
{{{relref(Derived types and their interoperability,cython-derivedtype-f2py)}}} have been covered
previously in the context of usage from Python. However, much of the
focus of the previous approach revolved around the ~iso_c_binding~
intrinsic module. A closer inspection of the functionality provided therein is the
first step towards extending beyond the standards to support calling type
bound procedures. This is an often overlooked aspect of the derived
type usage pattern, in terms of interoperability.
** Setup
We would like to understand the effect of the intrinsic ~iso_c_binding~ module.
So, we will re-use at first, the basic cartesian type of the previous post,
along with a *non-type-bound* procedure which is to be called from ~C~.
...
labels have been dropped, and correspondingly, instead of ~c_double~
we now have an approximate ~kind~ mapping.

The associated ~C~ driver is exactly the same, though in this
particular situation the function signature is optional.
...
~meson~ or another build system, we will compile everything by hand.
...
A very literal attempt at satisfying the two step process above is
...
Naturally, the linker will fail at this point. Recall that we can
check which symbols are actually part of ~vec.o~.
...
Where the first few undefined functions are to be resolved by ~-lgfortran~ directive to
the linker [fn:whatnm]. Note that the function we care to call is actually called
~__vec_MOD_unit_move~. We can also check the symbols required by our
program.
...
forcibly rename the symbol in our Fortran code, thus allowing us to
link and call the function. ~objcopy~ is fantastic for this.
#+begin_src bash
...
Indeed, apart from this very satisfying result, we can verify that our
symbols are not undefined as well.
#+begin_src bash
nm -u gf_vec
# nothing but library functions
#+end_src
*** Switching compilers
There are a number of caveats with the approach described so far, but
perhaps one of the more striking ones has to do with changing the
compiler. Consider the symbols generated by the Intel compiler.
...
As Fortran remains one of the few languages to have a rich and varied
set of compilers with varying levels of support and standardisation,
it would be rather a tall order to keep track of the symbol mangling
...
*** Simplifying labels
The first functionality of the ~iso_c_binding~ is actually rather easy
...
~bind(c)~ variant of the {{{relref(previous post,cython-derivedtype-f2py)}}}.
...
We would like to extend the discussion to beyond where the light of
the standard reaches, that is to consider calling type-bound
procedures, which are not supported by the standard. We will begin by
a suitable modification of our code. In an ideal world, we would
simply annotate the type-bound procedure.
...
Removing the attribute, from both the type and the subroutine and
rearranging slightly, we get the following.
...
Which does not seem to be very different at all. We will make an
attempt to directly modify the symbol-table as before.

#+begin_src bash
objcopy --redefine-sym=__vec_MOD_unitmv=unit_move vec_typeb.o
#+end_src

An attempt to call this is bound for failure, a segmentation fault to
be exact. In order to be able to call the type bound procedure then,
we can begin by writing a wrapper subroutine.
...
It is useful to at this point, that Fortran passes arguments
by reference and not by value, so there are no additional copy related
overheads incurred by this "wrapper" approach.

#+begin_quote
It was correctly pointed out on the [[https://fortran-lang.discourse.group/t/exploring-iso-c-binding-and-type-bound-procedures/2447/6?u=rgoswami][Fortran Discourse]] that the pass-by-reference
calling convention is not mandated by the standards, though [[https://gcc.gnu.org/onlinedocs/gfortran/Argument-passing-conventions.html][in practice]] many
compilers do pass by reference when the ~VALUE~ attribute is missing.
#+end_quote


On the other hand, it does leave the programmer in a world bereft of
the ~iso_c_binding~, which means also that the mapping of types and
precisions become rather fluid.

** Conclusions
This brief interlude on derived types and functions is more relevant
in the context of automated binding generators like ~f2py~
@petersonF2PYToolConnecting2009.

Perhaps a more formal approach to wrapper generation is the one elaborated upon in great
detail in the literature
@grayShadowobjectInterfaceFortran1999 which details the
concept of a logical interface and a physical interface.

A similar but more pragmatic approach is the opaque pointer method used for derived
types with pointers in @pletzerExposingFortranDerived2008 and forms the basis
for the implementation in ~f90wrap~ @kermodeF90wrapAutomatedTool2020.

The (ab)use of type-bound procedures in the context of these binding
generation methodologies is to follow at some point since none of them
make any explicit mention of the same. Given that
type-bound procedures were not introduced before 2003
@reidNewFeaturesFortran2003, it is not surprising they have been
overlooked, however their usage is foundational for long-lasting,
sustainable Fortran code.

[fn:whatnm] As in all things, ~info nm~ provides the details on interpreting the remaining symbols
* TODO Testing F2PY Derived Type Representations
#+begin_quote

#+end_quote
** Testing Frameworks
There are a plethora of testing frameworks, however, rather than go with a ~C/C++~ testing framework, I wanted something which was pure ~C~. I ended up
*** ~C~ Testing


* DONE TinySSH for Docker Development Environments :@notes:tools:workflow:
CLOSED: [2021-12-26 Sun 22:14]
:PROPERTIES:
:EXPORT_FILE_NAME: tinyssh-dockerdev-env
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
...
#+end_quote
** Background
~act~ works well enough for most Github Actions workflows, until something fails
in the pipeline, or interactive debugging is required. Naturally this is beyond
the purview of ~act~ itself, but it is easy enough to set up shop with a
project's GitPod ~docker~ image being used. For the purposes of the post we'll
consider ~numpy~.
...
~docker attach~ can be used in tandem with ~docker stop~ and
~docker start~, but these have the disadvantage of only providing one
synchronous view of the machine.

An *SSH server* seems like the most natural solution. Rather than cover OpenSSH
and its superuser based configuration, we will instead consider the [[https://tinyssh.org/][TinySSH
project]], which works well without superuser access.

** TinySSH
We also need to be able to listen for incoming TCP connections and run
~tinysshd~ in response to a connection. This is a picture perfect scenario for
~tcpserver~ which is part of the ~uscpi-tcp~ [[https://cr.yp.to/ucspi-tcp/tcpserver.html][set of tools]].

#+begin_src bash
sudo apt install tinysshd uscpi-tcp # for tcpserver
# On focal it is:
# sudo apt install tinysshd ucspi-tcp-ipv6
#+end_src

Although this assumes ~sudo~ access, compiling both [[https://www.bytereef.org/howto/djb/ucspi-tcp-install.html][uscpi-tcp]] and [[https://tinyssh.org/install.html][tinysshd]] from source is a remarkably simple process.

As ~tinyssh~ [[https://github.com/janmojzis/tinyssh][does not implement RSA]] and older cryptography algorithms, it is easiest to make a new key pair on the host machine.
#+begin_src bash
# On the host
...
# or
cat ~/.ssh/id_ed25519_tiny.pub | xclip # X11
#+end_src
...
~numpy~ and other projects have opted into Gitpod as a lower barrier of entry
method to aid contributors. Often the ~docker~ images backing these do not have
~systemd~ or service management set up, and so local environments based off of
such images can be a little off putting.

Nothing in this post constitutes any kind of best practice. In an ideal world
one would have either physical machines to access or well configured virtual
machines. Nevertheless, for edge cases, this works well enough in practice  .
* DONE Fortran OOP and Python :@notes:python:fortran:numpy:f2py:ramblings:
CLOSED: [2022-05-09 Mon 03:39]
:PROPERTIES:
:EXPORT_FILE_NAME: fortran-oop-python
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/Quansight.bib, biblio/refs.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Bridging Fortran \& Python")
...
#+end_quote
** Background
Derived types are easily one of the most visible of the modern Fortran
(post-F90) features and are central to object oriented programming paradigms in
Fortarn.
...
interoperability with ~C~ as of the [[https://gitlab.com/lfortran/lfortran/uploads/973db2bfcc64cd2a397f823c32718b39/18-007r1.pdf][F2018 draft standard]].

For F2PY within ~numpy~, however, although there are no direct equivalents in
~C~ for the more OOP specific derived types, there are plenty of equivalent
structures in ~Python~. This post will bridge Fortran derived types to their
logically equivalent Python class definitions.
** Premise
As in {{{relref(the previous posts,cython-derivedtype-f2py)}}}, we will be interested in a ~bind(c)~ derived type.
#+begin_src fortran
type :: cartesian
  real(c_double) :: x, y, z
end type cartesian
...
- *Lists* :: In theory, a record type can be represented and constructed from a list, with the members of the type specified in order of appearance or lexicographically. However, this is a lossy, incomplete representation as it would remove the ability to refer to and operate on the member variables by name.
- *Dictionaries* :: These have the benefit of being relatively simple to implement, and retain member addressable semantics.
- *Named Tuples* :: As immutable data structures with labels, these make good return types, however, they are poor representations for objects which may be modified in place (e.g. in a Fortran ~subroutine~).
- *Dataclasses* :: These make sense from a logical perspective, however, they are hard to programmatically generate and use in C-extensions, limiting their utility for inter-operability with Fortran.
- *Classes* :: The most general and arguably the most flexible construct is to generate code for a ~PyTypeObject~, which can be fleshed out to contain both attributes, as well as memory allocation rules.

Of the representations described, the first approximation covered in {{{relref(a previous
post,cython-derivedtype-f2py)}}} was the ~dictionary~. A major concern [fn:whosaidthat] is that of lineage
and extensibility; ~python~ users would expect that a dictionary can be extended
with keys, which would not make much sense if the dictionary is meant to
represent a derived type. Additionally, it is difficult to programmatically
ascertain which dictionaries are meant to be interoperable with Fortran
functions or subroutines. Note that the dictionary representation code is also [[https://github.com/HaoZeke/f2py_derived_nep/tree/main/examples/bindc/pydict][present here]] with usage examples.

The rest of this post will enumerate the construction and usage of a ~bind(c)~
derived type represented as a Python class.
** Pythonic Equivalent
For a more pedagogical understanding, the derived type will be emulated (from
within a Python-C exention) by the following class definition:
#+begin_src python
class fakecart:
...
#+end_src
Note that apart from the members which mimic the those in the derived type, for
illustration's sake we will implement the ~unit_step~ free function as a member
function.
** C-Python-Fortran Implementation
For ~bind(c)~ types, we will focus on implementing the member data in terms of
~C~ structs, and rely on ~bind(c)~ to ensure that symbols are mangled in a
compatible manner, i.e. we will not need to hardcode any compiler specific name
mangling conventions for the functions we call from Fortran. The complete
Fortran code of interest is:
...
For a source to source translation from the ~Python~ code, we will start with a
header file containing the basic C-Fortran structures and function declarations:
...
*** Class Methods
The heavy lifting of the function itself is in Fortran, while the ~C~ struct can
be used inter-operably with a derived type, leaving us with an extremely light
definition:
#+begin_src C
...
representations line up nicely to prevent any extraneous copy operations.
Representing the module subroutine as a class member function is a design choice
unlikely to be made when generating bindings from ~f2py~, but this was more for
exploratory purposes.

To end on a rather provocative note [fn:whatprovoc], consider the timings for the wrapped
derived type ~pycart~ and its equivalent pure-Python ~fakecart~ in Fig. 1 below:

#+caption: Results of comparing the pure-python class (~fakecart~) to the C-extension class backed by a Fortran derived type (~pycart~)
[[file:./images/fdtclasstimings.png]]

All the code for this post, along with other Fortran derived type
representations is present [[https://github.com/HaoZeke/f2py_derived_nep/tree/main/][in this repository]] under the examples folder.
*** Addendum and Future Directions
I had the pleasure of demonstrating part of this logic to fellow NumPy
maintainers Dr. Pearu Peterson and Dr. Melissa Weber Mendonça and we were in
agreement that the class strategy is the most flexible [fn:whoelse]. Pearu suggested
leveraging pointers passed between Fortran and C in an attempt to cover more
than ~bind(c)~ compatible code, by generating simpler Fortran wrapper functions
as well, and those will likely be among the next steps forward. The work of
@pletzerExposingFortranDerived2008 is also of interest, although their focus was
on exposing a series of ~C~ functions which call ~Fortran~ methods.

[fn:whosaidthat] Noted by [[https://github.com/cthulahoops][Adam Kelly]]
[fn:whoelse] I also discussed this at a few NumPy meetings and also informally mentioned this during some LFortran meetings as well
[fn:whatprovoc] Benchmarks are notoriously [[https://pythonspeed.com/articles/consistent-benchmarking-in-ci/][divisive and finicky]], and no attempt was made to ensure statistical stability of the results nor were the plotted points obtained on machines dedicated and isolated from spurious loads
* TODO Fortran Derived Type Arrays and Python
:PROPERTIES:
:EXPORT_FILE_NAME: fortran-deriv-type-arrays-py
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/Quansight.bib, biblio/refs.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Bridging Fortran \& Python")
:END:
#+begin_quote
Passing dynamic arrays of derived types from ~C~, ~Fortran~ and ~Python~
#+end_quote
** Background

Appropos of nothing, we will assume that one is familiar with the rational for
wanting to have arrays of record types passed between languages as efficiently
as possible.

We will liberally use features from the [[https://gitlab.com/lfortran/lfortran/uploads/973db2bfcc64cd2a397f823c32718b39/18-007r1.pdf][F2018 draft standard]], but will only
focus concretely on 1D and 2D numeric data.

For F2PY within ~numpy~, however, although there are no direct equivalents in
~C~ for the more OOP specific derived types, there are plenty of equivalent
structures in ~Python~. This post will bridge Fortran derived types to their
logically equivalent Python class definitions.
* DONE Types from Fortran to Python via Opaque Pointers :@notes:python:fortran:numpy:f2py:ramblings:
CLOSED: [2023-11-12 Sun 23:36]
:PROPERTIES:
:EXPORT_FILE_NAME: types-fortran-python-opaque
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/Quansight.bib, biblio/refs.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Bridging Fortran \& Python")
:END:
#+begin_quote
An exploration of the opaque object and pointer interface approach to unleash
more modern features than those offered by the ISO Fortran standard.
#+end_quote
** Background
Since this has been covered a few times before, just some quick pointers[fn:seriously]. The ultimate
context is to be able take nice, modern Fortran code with derived types and
generate equally nice, user friendly, efficient Python wrappers via ~f2py~.
*** Why?
Where the last post (and [[https://github.com/numpy/numpy/pull/20770][the test implementation]] in ~f2py~) left off, the type
shadowing approach for having exactly interoperable ~bind(c)~ derived types (and
[[https://github.com/numpy/numpy/pull/24555][functions]]) was covered. However, there are several aspects of derived types
which cannot be covered ("deep" types in the nomenclature of
@pletzerExposingFortranDerived2008). A brief list:
...
Of these, the first three are probably the most common use cases, which need to
be addressed by any serious attempt to bride Fortran and Python.
*** Scope
This post is just going to cover the development of Python examples facilitated
by the pointer implementation where:
- A simple derived type, ~point~, is manipulated via pointers

We will first demonstrate a Fortran example, followed by its call from ~C~,
before demonstrating its bridging ~Python-C~ code and finally a user-facing
~python~ example. The full code may be found in the examples folder and pull
requests [[https://github.com/HaoZeke/f2py_derived_nep][here]]. Typically the snippets here will have elided error handling to
drive home the approach over the boilerplate / best practices [fn:egdouble].
...
- ~C~ bindings call the ~bind(c)~ proceedures and pass handles (opaque pointers)
  as ~void*~.
- At the ~python~ level, these are stored in ~PyCapsule~ instances, and exposed
  to the user as a standard Python class.

The ~bind(c)~ proceedures are meant to separate the implementation from the
~C~-interface in a manner which can be automated (eventually) by ~f2py~.
Similarly, the ~Python-C~ code is eventually to be generated as well.
** ~point~ with pointers
We will start with the prototypical class in applications, a ~point~, with 2
dimensions.
*** Fortran baseline
...
*** ~C~-Wrappers
Lets focus a bit on what we need to do, creation and destruction are fairly
straightforward since we just need to remember to return a ~c_ptr~ and create
interface it to our existing functions..
#+begin_src fortran
...
Note the use of the ~c_loc~ intrinsic which is used to get a pointer compatible
with ~C~ as required by the return type too. For bindings, it is never a good
idea to neglect error handling, so we do guard for allocation failures, using
the NULL helper in the ~iso_c~ intrinsic. The ~value~ attribute is important
enough (and a recurring theme) so it is relegated to the box.
...
*Warning*
#+end_alert-heading
The ~value~ attribute is *required* in the to ensure data is copied and
interpreted correctly from ~C~ to Fortran.
#+end_alert
...
#+end_src
*** Python-C Interface
The key take-away here is to use a [[https://docs.python.org/3/c-api/capsule.html][PyCapsule]] object which:
#+begin_quote
...
#+end_quote
In our case the opaque value needs to travel a bit further along and go through
to Fortran and back, but the principle (and implementation) is the same. The ~C~
function calls will be the same, the only difference being the structure used,
unlike in "shadowing" approaches, the ~struct~ will have neither ~C~ interop nor
~Fortran~ information. So we start with:
...
This is simplicity itself. Naturally we need to actually grab the right object
from the capsule before use. Additionally, there are a few nuances, e.g. before
defining creation we need to define cleanup:
#+begin_src C
...
Creation is simply via ~PyCapsule_New~ once appropriate steps (unpacking, error
handling) are taken.
...
Since the rest of this is standard, including ~PyFloat_FromDouble~ and the rest
of the C-Python API, we will move onto the building and testing phase.
**** Building and testing
...
Which needs some additions to the ~meson.build~ [fn:yaymeson]
#+begin_src meson
py_mod = import('python')
py3 = py_mod.find_installation('python3')
py3_dep = py3.dependency()
...
Note that we have not (yet) defined any type bound proceedures, so there are no
operations defined between ~Point~ objects or anything else.
...
So far we have not really covered new ground compared to the previous iteration using
~C~ compatible ~struct~ information. The remarkable extensibility of the pointer
approach really shines when we consider adding a new type, with an allocatable
component[fn:saywhat].

That being said, there is more to be done over on the ~f2py~ side before it
makes sense to delve too deeply into enhancements on the ~Fortran-C-Python~
pipeline. The biggest blocker at the moment is:
- Suport for ~C_PTR~ in ~f2py~
  + This includes ~void*~ handling

The exact details shall be covered later, mostly because the Python-C code gets
a little rough. It is easier with ~f2py~ which provides efficient routines to
and from ~C~ and Python via ~numpy~ arrays but still too long to be in the same
post.
** Conclusions
More examples notwithstanding, the design discussed here is generic enough to
form a more coherent and pragmatic basis for further improvements. Some of these
are enumerated below for a future attempt:
- Circumventing the need to copy data from Python / ~C~ to Fortran [fn:saywhat]
  + Either by careful usage of ~transfer~ or just via ~c_loc~ with compatible types
- Using the ~CFI~ descriptors to operate efficiently on discontinous
  memory [fn:whydiscont]
  + Actually generally using the ~CFI~ descriptors more, since 2018
    @reidNewFeaturesFortran2018, some of the pointer highjinks are enshrined as
    cannon
    + ~c_loc~ and ~c_funloc~ may have non-interoperable arguments
      @metcalfModernFortranExplained2018

All that being said, with ~meson~ support in ~f2py~ from ~1.26~ ([[https://github.com/numpy/numpy/pull/24532][npgh-24532]]),
basic types based modern Fortran [[https://github.com/HaoZeke/GaussJacobiQuad#f2py-generated-interface][can already be used]], so the future remains
bright.

[fn:yaymeson] It is at this point (linking, handling dependencies) that ~meson~ pays off over hand compilation
[fn:egdouble] Hence ~real(8)~ over the more [[https://fortran-lang.discourse.group/t/best-way-to-declare-a-double-precision-in-fortran/69/21?u=rgoswami][explicit alternatives]]
[fn:seriously] Pun intended
[fn:saywhat] This Fortran-Lang [[https://fortran-lang.discourse.group/t/allocate-interoperability-and-c-descriptors/5088/9?u=rgoswami][discourse thread]] includes examples from Ivan Pribec demonstrating this
[fn:whydiscont] A popular NumPy usage pattern, and a visible bottleneck in interactive workflows
* DONE Revisiting Wayland for ArchLinux :@notes:ramblings:workflow:setup:archlinux:
CLOSED: [2021-12-24 Fri 18:13]
:PROPERTIES:
:EXPORT_FILE_NAME: revisiting-wayland-2021-archlinux
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("ArchLinux Chronoscope" "Wayland Wayfinders")
:END:
#+begin_quote
Ruminating on ~wayland~ and ~sway~ for daily use with ArchLinux
#+end_quote
** Background
[[https://grimoire.science/working-with-wayland-and-sway/][Many years ago]], I found time to make an attempt to switch away from the X11
Window system [[https://wiki.archlinux.org/title/Wayland][to Wayland]]. At the time, I ended up switching back to Xorg, but I
did want to revisit it. Since I returned from home recently and was gifted a [[https://www.lenovo.com/us/en/p/laptops/thinkpad/thinkpadx1/thinkpad-x1-carbon-(6th-gen)/22tp2txx16g][Gen 6 ThinkPad X1 Carbon]], I had a perfect opportunity to do so [fn:yeahright].

Part of this post will be interspersed with generic non-Wayland observations,
but it has been a few years since my last setup and things have changed. Rather
than regurgitate the fantastic [[https://wiki.archlinux.org/title/installation_guide][ArchWiki installation guide]], a FAQ style
short-doc seems more appropriate.

As before, none of these configurations should be considered
complete/recommended/normative; *use the installation guide* and _follow the Arch Way_. These are install notes, not tutorials.
** ArchLinux baseline
- *Networking* :: The days of ~wifi-menu~ are gone. The new ~iwd~ interface ([[https://wiki.archlinux.org/title/Iwd][ArchWiki]]) is [[https://lwn.net/Articles/770991/][the newest, swankiest kid]] on the block [fn:potificate] and works great with ~NetworkManager~
- *Input devices* :: ~loadkeys~ for the console, ~libinput~ configurations can be handled via the window manager under Wayland
- *Audio* :: ~pulseaudio~ works well as always, though ~pipewire~ is pretty neat as well
- *Nix* :: Via the [[https://nixos.org/guides/install-nix.html][multi-user install]]
- *Time* :: Being as this is a laptop configuration, it is best to configure ~chrony~ ([[https://wiki.archlinux.org/title/Chrony][described here]]) as the ~ntp~ client and server of choice
- *AUR Helper* :: ~trizen~, though I retain a soft spot for ~yay~
- *Portability* :: Details are presented concisely on [[https://wiki.archlinux.org/title/Laptop][the ArchWiki]]
- *Device details* :: The ArchWiki [[https://wiki.archlinux.org/title/Lenovo_ThinkPad_X1_Carbon_(Gen_6)][has an entry]], notably including modem setup woes and also ~throttled~ details for better power management (I ended up using [[https://github.com/erpalma/throttled][throttled]] over ~thermald~)
- *Window and login* :: ~sway~, and ~greetd~
*** Configurations
In general the idea is to run as few services as possible so ~systemctl
--type=service~ is very helpful.
#+begin_src bash
...
- ~boot/refind_linux.conf~ :: for kernel parameters used with auto-detected stanzas, e.g. enabling app-armor everywhere via ~lsm=landlock,lockdown,yama,apparmor,bpf~
  - This *is not* where boot stanzas are to be defined
- ~boot/EFI/refind/refind.conf~ :: for manual stanzas, settings, themes and other things
...
**** Network and Time
Networking with ~iwd~ is very well [[https://wiki.archlinux.org/title/Iwd][described on the ArchWiki]], with the exception of its interaction with NetworkManager which essentially involves configuring it to use the IWD backend via ~/etc/NetworkManager/NetworkManager.conf~:
#+begin_src conf
[device]
wifi.backend=iwd
#+end_src
...
#+end_src
Personally I combine this the [[https://aur.archlinux.org/packages/networkmanager-dispatcher-chrony/][NetworkManager dispatcher script]] for ~chrony~.
***** Eduroam
...
#+begin_src bash
6: wwan0: <POINTOPOINT,MULTICAST,NOARP,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UNKNOWN group default qlen 1000
    link/none
#+end_src
...
#+end_src
**** Audio
As [[https://grimoire.science/working-with-wayland-and-sway/][discussed earlier]], no special requirements need to be considered beyond
setting up ~/etc/libao.conf~ which in itself is only needed for [[https://github.com/PromyLOPh/pianobar][pianobar]].
#+begin_src src
default_drive=pulse
# dev=default
quiet
#+end_src
Actually this also holds true for ~pipewire~ and that's pretty much what I
decided to go with this time around.
**** Snap and AppArmor
In general, it seems that ~snap~ packages have better support for ~wayland~,
like for Telegram. Setting this up involves working out ~apparmor~ as well via
the kernel parameters. However, for me and [[https://forum.peppermintos.com/index.php?topic=4338.0][some other people]] apparently,
enabling ~apparmor.service~ corresponds to taking a few seconds of added boot
time. Unacceptable for me at any rate. So no ~snapd~ either.

Flatpak uses namespaces, but honestly on an ArchLinux machine it is always
better to just build things.
**** Intel graphics
Following along the [[https://wiki.archlinux.org/title/Intel_graphics#Enable_GuC_/_HuC_firmware_loading][standard wiki page]], I ended up setting ~fastboot~ and ~enable_fbc~, verified with:
#+begin_src bash
...
Window and session managers are not exactly a dime a dozen. The X window system
has served me well for many many years yet, for this post, of the X window
system, the [[https://www.x.org/archive/X11R6.8.0/doc/xorg.conf.5.html#sect8][less said the better]]. Wayland on the other hand, is fast, shiny, and
doesn't as yet have [[https://www.explainxkcd.com/wiki/index.php/963:_X11][233 configuration flags]].
** Sway basics
~Weston~ remains the reference implementation, and GNOME has continued
...
MacOS' "tiling managers" was seamless and very reminsicent of my older ~i3-gaps~
setup.
*** High DPI
An interesting quirk of ~sway~ is that Hi-DPI support is still [[https://github.com/swaywm/wlroots/pull/2064#issuecomment-622647075][at the draft
stage]]. For the most part this is becoming a non-issue as more applications
support Wayland out of the box, or at-least optionally.

However, certain applications are unusable without the HiDPI support patches,
including most ~java~ packages like ~cryptomator~. Thus it is best to grab the
modified versions and configure them.
#+begin_src bash
trizen -S sway-hidpi-git wlroots-hidpi-git xorg-xwayland-hidpi-git
#+end_src

Note that by default ~sway~ supports fractional scaling too via, ~swaymsg output
$MON scale 1.5~ and also when in the configuration. However, the patches are
required for ~xwayland force scale 2~ and sundry commands.
...
*** Display management
A Reddit user ~WhyNotHugo~, has an excellent [[https://www.reddit.com/r/swaywm/comments/qiz4aa/comment/hin41hg/?utm_source=share&utm_medium=web2x&context=3][line of reasoning]] for why a display
manager is needed, rephrased to be:
...
Anyway, the basic ~agreety~ should suffice for the most part; though a more
pleasant setup is with ~greetd-tuigreet~.

More importantly, the session itself should include the appropriate environment
variables as [[https://man.sr.ht/~kennylevinsen/greetd/#how-to-set-xdg_session_typewayland][described here]] and reproduced below. Essentially this involves using a runner script, ~/usr/local/bin/sway-run.sh~ which ensures the correct variables are setup.
#+begin_src bash
#!/bin/sh
...
Now instead of using ~tuigreet --cmd sway~ in ~/etc/greetd/config.toml~ we can
use ~sway-run~ as the ~cmd~.

It is important to optionally provide ~GDK_BACKEND~ with a fallback option
otherwise ~zotero~ and others might fail with ~Error: cannot open display: :0~.

This still breaks for a bunch of special cases for which I went with a
specialized ~desktop~ file in ~$HOME/.local/share/applications/BLAH~. For example, consider the ~enpass~ file (since it only supports ~xcb~):
...
** XWayland proxies
Rather than work with the patched ~sway~ and ~wlroots~ packages, a recent
approach by [[https://roscidus.com/blog/blog/2021/10/30/xwayland/?utm_source=pocket_mylist][Thomas Leonard]] based on isolating the XWayland setup works a lot
...
than the standard XWayland setup with ~sway~. However, ~xlsclients~ does not
track the X11 applications started in the manner described here.
#+begin_src bash
...
[Service]
ExecStart=/home/$USER/.opam/default/bin/wayland-proxy-virtwl --tag="[my-vm] " --wayland-display wayland-0 --x-display=0 --xrdb Xft.dpi:150 --x-unscale=2
...
** Miscellaneous
*** Terminal Emulators
Though ~kitty~ and a few other stalwarts do support ~wayland~, foot seems to be
the fastest, especially since it has a server client mode much like ~emacs~ with
~foot -s~ and ~footclient~.
*** Screenshots
...
*** Screen Recorders
Unfortunately, the ~zoom~ desktop client [[https://github.com/swaywm/sway/issues/4358][does not support]] ~sway~ which is
problematic. However, with pipewire setup, Firefox passes the [[https://mozilla.github.io/webrtc-landing/gum_test.html][gUM test]] for
screen-sharing which is good enough. For recording stuff, OBS has a plugin, but
[[https://github.com/foxcpp/ssr-wlroots][simplescreenrecorder has a fork]] which works out of the box, with XWayland
required for the UI though.
#+begin_src bash
trizen -S simplescreenrecorder-wlroots-git
DISPLAY=":0"  WAYLAND_DISPLAY="wayland-1" simplescreenrecorder
#+end_src
*** Reference Management
I've been a ~zotero~ user for many years now, however it suffers from the same
UI scaling issues and cannot be fixed either by forcing ~xwayland~ execution nor
by trying other scaling methods. The last resort is to set up a ~user.js~ file in ~$HOME/.zotero/zotero/$PROFILE/user.js~ with:
#+begin_src js
...
*** Mail Clients
The mail client story is increasingly complicated on Linux machines in general.
Since Thunderbird tanked with XUL ecosystem eons ago there haven't been many
contenders. Given the situation with window management, a ~localserver~ approach
to web-clients like Mailpile [fn:welldesc] would have been perfect... Except it
is still based around ~python2~ and shows no sign of moving forward. I really
{{{relref(like Astroid,reclaim-email-astroid)}}}, but it doesn't seem to have been updated in a while either. Thunderbird works fine too, and does not require XWayland which is a plus point.
*** Electron
...
**** Firefox
Firefox's ~wayland~ build on the AUR remains a nightmare to compile and work
with. The developer version can leverage the ~MOZ_ENABLE_WAYLAND=1~ to get
...
#+end_src
** Conclusions
Things have definitely improved in three years, however the overall aggravation
...
particular, as before, forcing programs to run in ~xwayland~ makes them
effectively worse than their ~x11~ counterparts and requires far more
...
[fn:yeahright] This precludes any actual tasks I have, naturally, the color of the bike shed and the wool of a Llama take precedence
[fn:potificate] Naturally I haven't really taken time to dive into why, as evidenced by the many many many adjectives
[fn:welldesc] A good initial set of thoughts on this are provided by [[https://emeraldreverie.org/2018/10/03/first-impressions-of-mailpile/][this post]]
* DONE Spack and PyTorch Development Workflows :@notes:ramblings:pytorch:
CLOSED: [2022-04-24 Sun 19:16]
:PROPERTIES:
:EXPORT_FILE_NAME: spack-pytorch-dev-workflow
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("HPC")
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
...
devices. ~micromamba~ is typically my poison of choice where ~nix~ is not
an option.

However, ~micromamba~ doesn't allow a whole lot in the way of setting up
environments which do not use packages already on ~conda-forge~. [[https://spack.readthedocs.io/en/latest/getting_started.html][Spack]] fills a
nice niche for such situations[fn:whatelse]. Additionally, it can be coerced
into use for local development and the same cannot easily be said of ~conda~
based workflows.
** Humble Origins
We'll start from scratch, grabbing ~spack~ and bootstrapping ~clingo~ for its
dependency resolution as per [[https://spack.readthedocs.io/en/latest/getting_started.html][the documentation]].
#+begin_src bash
...
Unlike ~nix~, ~spack~ does not offer strong guarantees of purity. There are
primarily two approaches to using environments in ~spack~, somewhat analogously
to the ~conda~ environment logic.
...
We will use both kinds of environments. Additionally, ~spack~ supports multiple
~variants~ which can be queried by ~spack info $PKG_NAME~. These are used to
...
option to needs to be set to ~together~. We will start by adding some packages.
#+begin_src bash
...
#+end_src
To ensure dependencies are resolved simultaneously, ~concretize~ is set to
~together~.
#+begin_src bash
...
spack -e . install
#+end_src
This establishes baseline environments, but precludes setting up development
workflows. Most packages can be included as described above, with the exception
of compiler toolchains.
*** Compiler Setups
Setting up compiler toolchains like a ~fortran~ compiler (perhaps for
~openblas~) can take a bit more effort. Although this discussion will focus on
obtaining a ~fortran~ compiler it is equally applicable to updating a version.
For ~spack~, unlike many system package managers, ~gcc~ will install the ~C~,
~C++~ and ~Fortran~ toolchains. Thus:
#+begin_src bash
...
spack:
  ...
  concretization: together
#+end_src
One of the nicer parts of ~spack~ being an HPC environment management tool is
that there is first-class support for proprietary compiler toolchains like Intel
and families of compilers can also be specified with the ~%intel~ syntax as
well. Much [[https://spack-tutorial.readthedocs.io/en/latest/tutorial_configuration.html#yaml-format][more fine-tuning]] is also possible including registering system
compilers if required.
** PyTorch Local Development
One of the caveats of local development with ~spack~ is that the base URL needs
to be updated from within the local copy of ~spack~. This means editing:
#+begin_src bash
...
@@ -348,7 +349,8 @@ def enable_or_disable(variant, keyword='USE', var=None, newer=False):
...
~DEBUG~. The application of this patch will rectify this until [[https://github.com/spack/spack/pull/30267][an upstream PR]] is
merged.
#+begin_src diff
diff --git a/var/spack/repos/builtin/packages/py-torch/package.py b/var/spack/repos/builtin/packages/py-torch/package.py
...
branch. To focus on the ~pytorch~ workflow, we will unpin ~python@3.10~ and add
~ipython~ instead.
*** CPU
Normally, for a CPU build we would setup something like the following:
#+begin_src bash
DEBUG=1 USE_DISTRIBUTED=0 USE_MKLDNN=0 USE_CUDA=0 BUILD_TEST=0 USE_FBGEMM=0 USE_NNPACK=0 USE_QNNPACK=0 USE_XNNPACK=0 BUILD_CAFFE2=0 USE_KINETO=0 python setup.py develop
#+end_src
The rationale behind the build flags can be found in the [[https://github.com/pytorch/pytorch/blob/master/CONTRIBUTING.md][upstream contributing
...
spack concretize -f
#+end_src
However, we need to also register this package for development (with the branch
setup previously), which is accomplished by:
#+begin_src bash
spack develop py-torch@npeye -cuda -fbgemm -nnpack -mkldnn -test -qnnpack +debug
#+end_src
This generates a sub-directory ~py-torch~ with the right branch checked out,
along with the dependencies needed for the build. Additionally, the
~python~ version is also localized to the ~spack~ installation spec.
#+begin_src bash
spack install # concretizes and installs
#+end_src
If additional dependencies are required for testing or other purposes, they are
easily obtained. After making changes ~spack install~ will rebuild with the
appropriate flags.
*** CUDA Setup
The good news is that updating the build system to use CUDA is very
straightforward. While changing variants, it is occasionally necessary to
forcibly clear out the cache.
#+begin_src bash
cd py-torch
rm -rf build
...
~spack~ needs to have read/write access to ~/tmp/cuda-installer.log~ because of
a [[https://forums.developer.nvidia.com/t/change-the-path-of-install-log-when-installing-cuda-toolkit/183179][ridiculous upstream bug]].
...
spacktivate pytorchBaseline
spack concretize --force
spack install
# Do tests, compare
# ...
# Wrap up and deactivate
despacktivate pytorchBaseline
#+end_src
** Conclusions
Dependency management is always painful. CUDA management doubly so. Better
workflows with [[https://spack-tutorial.readthedocs.io/en/sc20/tutorial_developer_workflows.html][spack dev-build]] are available for some packages, [[https://github.com/kokkos/kokkos/blob/master/Spack.md][like KOKKOS]], but
~spack dev-build~ doesn't work yet for ~pytorch~, and also appears to be removed
from the present set of tutorials. Personally, I'd still prefer ~nix~, but where
~micromamba~ falls short in terms of source builds, ~spack~ is a good
alternative, if one has the resources to rebuild everything needed.

[fn:whatelse] It also integrates nicely with typical HPC modular workflows like LMod and has reasonable support for Windows
* DONE Supplements for SciPy 2022 :@conferences:presentations:ramblings:fortran:python:posters:
CLOSED: [2022-11-15 Tue 23:28]
:PROPERTIES:
:EXPORT_FILE_NAME: scipycon-2022-meta
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+BEGIN_QUOTE
...
#+END_QUOTE
** Background
I hadn't been in an in-person conference for quite a long while now. There were
multiple things going for SciPyCon. Collaborators at UT Austin (from the [[http://theory.cm.utexas.edu/eon/][EON
project]]). The prospect of meeting folks [[https://quansight.com/][from Quansight]]. Meeting other members
of the [[https://numpy.org/teams/][NumPy community]] I'd been working with virtually for a year.  I had the pleasure
of being the recipient of a [[https://twitter.com/marsbarlee/status/1547580143232618499][SciPy Scholarship]] for [[https://github.com/HaoZeke/wailord/blob/main/docs/scipycon22/wailord.pdf][my poster]] and [[https://conference.scipy.org/proceedings/scipy2022/rohit_goswami_wailord.html][conference
proceeding]] on [[https://wailord.xyz][Wailord]], which sealed the deal for me. I had the privilege of
representing every faucet of my interests, including a poster on LPython (an
LFortran spin-off) and a talk on F2PY (with a distinguished [[https://twitter.com/scientific_py/status/1548033869080772612][panel of maintainers]]).

** Maintaining Fortran in Python in Perpetuity
- Co-Authors :: [[https://melissawm.github.io/][Dr. Melissa Mendonca]] (Quansight Labs), [[https://rgommers.github.io/][Dr. Ralf Gommers]] (Quansight Labs), [[https://github.com/Thirumalai-Shaktivel][Thirumalai Shaktivel]] (Fortran-Lang / LFortran), [[https://github.com/pearu][Dr. Pearu Peterson]] (Quansight Labs)
- Duration :: 20 minutes (15 + 5)
- Medium :: Maintainers Track Talk

#+begin_export html
<a href="https://dx.doi.org/10.25080/majora-212e5952-021"><img src="https://img.shields.io/badge/Paper%20DOI-10.25080/majora--212e5952--021-blue?labelColor=gray&style=flat&link=https://dx.doi.org/10.25080/majora-212e5952-021" alt="Paper DOI" /></a>

<a href="https://doi.org/10.5281/zenodo.7325023"><img src="https://img.shields.io/badge/Slides%20DOI-10.5281/zenodo.7325023-blue?labelColor=gray&style=flat&link=https://doi.org/10.5281/zenodo.7325023" alt="Slides DOI" /></a>
#+end_export
...
*** Slides
- [[file:/revealjs/scipy_2022/fortranInPerpetuity.html][Best viewed here]] using a browser (in a new tab)
- A ~pdf~ copy of the slides are embedded below
- The ~orgmode~ source [[https://raw.githubusercontent.com/HaoZeke/haozeke.github.io/src/presentations/SciPyCon2022/scipy_f2py_maint.org][is here on the site's GH repo]]
#+BEGIN_EXPORT html
<script async class="speakerdeck-embed" data-id="016126f198e247d4812590b8095e6c75" data-ratio="1.37090632140137" src="//speakerdeck.com/assets/embed.js"></script>
#+END_EXPORT
*** Video

#+html: {{< youtube CivzdD3jxB8 >}}

** LPython: Interactive LLVM-based Python Compiler for Modern Architectures
- Co-Authors :: [[https://ondrejcertik.com/][Ondřej Čertík]] (GSI Technology), [[https://www.linkedin.com/in/brianbeckman/][Brian Beckman]] (GSI Technology), [[https://www.linkedin.com/in/namannimmo/][Naman Gera]] (IIT Guwahati), [[https://in.linkedin.com/in/smit-lunagariya-356b93179][Smit Lunagariya]] (IIT BHU), [[https://github.com/czgdp1807][Gagandeep Singh]] (GSI Technology), [[https://github.com/Thirumalai-Shaktivel][Thirumalai Shaktivel]] (Fortran-Lang / LFortran), [[https://www.linkedin.com/in/dylon-edwards-0936bb39/][Dylon Edwards]] (GSI Technology)
- Medium :: General Track Poster

#+begin_export html
<a href="https://doi.org/10.5281/zenodo.7325061"><img src="https://zenodo.org/badge/DOI/10.5281/zenodo.7325061.svg" alt="DOI"></a>
#+end_export

*** Abstract
#+begin_quote
We demonstrate to the community that it is possible to use Python in a modern
...
interactively, and run on all platforms.
...
LLVM can do. In addition it is remarkably easy to customize back ends.
#+end_quote
**** Short Summary
...
- Medium :: Materials & Chemistry Poster
#+begin_export html
<a href="https://doi.org/10.5281/zenodo.7325038"><img src="https://zenodo.org/badge/DOI/10.5281/zenodo.7325038.svg" alt="DOI"></a>
#+end_export
*** Abstract
#+begin_quote
...
[1] https://rgoswami.me/posts/pycon-in-2020-meta/
[2] https://wailord.xyz
[3] https://github.com/erikrose/parsimonious
[4] https://cookiecutter.readthedocs.io/
[5] https://www.kofo.mpg.de/en/research/services/orca
#+end_quote
**** Short Summary
...
** Thoughts
SciPyCon exceeded my expectations. Everyone was kind, and I met a good number of
people from QS and other places who hopefully did not come away with a poor
impression of me. There was a last minute [[https://twitter.com/marsbarlee/status/1548420979814084608][comic rush]] which [[https://twitter.com/marsbarlee][Mars]] arranged. I also
ended up [[https://groups.google.com/g/comp.text.tex/c/aLbKE3ami2g/m/f48XXbuHAwAJ][chatting with Jonathan Fine]] to discuss NumPy [[https://www.mail-archive.com/numpy-discussion@python.org/msg06130.html][dropping PDF documentation
builds]]. Plus I snagged a [[https://twitter.com/scientific_py/status/1547788365101031430][Scientific Python tee-shirt]]. With [[https://ondrejcertik.com/][Ondřej Čertík]] and
[[http://antocuni.eu/][Antonio Cuni]], thanks to a suggestion from [[https://twitter.com/teoliphant][Travis]] there were some neat
...
like [[https://pyiron.org/][PyIron]]'s Dr. Jan Janssen. Interacted with a bunch of people from the
...
in, and to introduce ~f2py~'s GSoC student ([[https://namamishanker.github.io/][Namami Shankar]]) to the community. I
suppose, if invited, I would return.

* TODO Configuring a personal VPS
:PROPERTIES:
:EXPORT_FILE_NAME: configuring-personal-vps
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :csl biblio/csl/ieee.csl
:EXPORT_BIBLIOGRAPHY: biblio/Website.bib
:END:
** Background
I needed a MongoDB instance I could use for handling my HPC runs. I ended up
getting a small VPS at Hostinger. These are some abbreviated setup notes.

The general idea is to have three different layers of control:
- Alpine Linux Firewall :: To redirect traffic, and control orchestration to
  ~tailscale~
- Tailscale :: To prevent unauthorized access to core services like ~ssh~ and
  ~mosh~ (only devices on the "tailnet" can even see them)
- Cloudflare Tunnel :: For handling authentication for services exposed to the
  world
- Reverse Proxy :: This is for load-balancing mostly, and to have all routing
  rules in one place
** Baseline
I opted for Alpine Linux, even though Arch Linux is available, mostly because
I've never used a ~musl~ based distribution before, and also because I'll be
basically using a bunch of ~docker~ images anyway, so I just need a minimal OS.


#+DOWNLOADED: screenshot @ 2024-09-03 08:55:25
#+CAPTION: Doesn't get any easier
[[file:images/Baseline/2024-09-03_08-55-25_screenshot.png]]

*** Netcup
For ~netcup~ I ended up with the ~virtual~ [[https://alpinelinux.org/downloads/][alpine linux image]].

#+DOWNLOADED: screenshot @ 2025-03-02 13:50:56
[[file:images/Baseline/2025-03-02_13-50-56_screenshot.png]]


This needs some setup from the VNC, basically just running ~setup-alpine~. Some notes:
- ~d~ was not being sent via the VNC which was annoying
- ~community~ isn't setup but that's [[https://wiki.alpinelinux.org/w/index.php?title=Enable_Community_Repository&redirect=no][straight-forward]]

Also to get things runnning smoothly we need to setup ~qemu-guest-agent~:
#+begin_src bash
doas apk add qemu-guest-agent
doas rc-update add qemu-guest-agent
#+end_src
** First steps
Some configuration is already done, e.g. the ~/etc/apk/repositories~ is set to
use ~https~.
*** Logging
Since Alpine Linux [[https://www.perrotta.dev/2022/01/alpine-linux-apk-logs-with-etckeeper/][has no logs]], it makes sense to setup ~etckeeper~. However,
since ~etckeeper~ tries to create a bunch of files as soon as it is installed,
due to a ~post-install~ we need to first configure some reasonable defaults:
#+begin_src bash
apk add git
cd /etc
git config --global init.defaultBranch main
git init
git config --local user.name "etckeeperbot"
git config --local user.email "nop@hide.me"
#+end_src

Now we can setup ~etckeeper~, without leaking server information through the
default email or user.
#+begin_src bash
apk add etckeeper
#+end_src

To push this somewhere, we need another ~ssh-key~.
#+begin_src bash
ssh-keygen -t ed25519 -C "rgETCbot@hide.me" -f ~/.ssh/vps_etckeep
eval `ssh-agent -s`
ssh-add ~/.ssh/vps_etckeep
cat ~/.ssh/vps_etckeep.pub
# Add public key to Git server, e.g. Gitlab, and test
ssh git@gitlab.com -T
git remote add origin "git@gitserver.blah:User/Name.git"
# Assumes a bare upstream
etckeeper vcs push -u origin main
#+end_src

Finally, to ensure commits are pushed as soon as they are made,
~PUSH_REMOTE="origin"~ in ~etckeeper.conf~ should work, however this requires an
active ~ssh~ agent, which isn't always true, so, we need to amend the script ~etckeeper~ uses to push:

#+begin_src diff
iff --git a/etckeeper/commit.d/99push b/etckeeper/commit.d/99push
index 3df1350..99e03d1 100755
...
There is also a daily ~cron~ job, which runs once a day to make commits, and is
enabled by default:
...
Note that ~doas etckeeper vcs push~ will still fail, but the post-commit push
will work, e.g. after an ~apk add~ or ~apk del~ [fn:: This is, I believe, a limitation stemming from not having ~systemd~].

*** Sharing SSH authentications with ~doas~

To fix the ~vcs push~ consider modifying ~doas.conf~..
...
*Note*
#+end_alert-heading
Always ensure the ~doas.conf~ works before exiting a session, perhaps by editing in ~doas su~, since it is imperative that it is error-free. From the ArchWiki, @DoasArchWiki:
#+begin_src bash
if doas -C /etc/doas.conf; then echo "config ok"; else echo "config error"; fi
#+end_src
#+end_alert

It turns out, at-least in the Hostinger instance, no amount of ~doas.conf~ work [fn:: Naively I thought ~permit nopass setenv { SSH_AUTH_SOCK } :wheel~ would do the trick]
helps with this, the only workaround is explicitly passing the socket:

#+begin_src bash
doas env SSH_AUTH_SOCK=$SSH_AUTH_SOCK etckeeper vcs push --set-upstream origin main
#+end_src

Or a much saner approach, is probably to simply use the ~HostKey~ found in
~/etc/ssh/ssh_host_ed25519~, but where's the fun in that [fn:: Also more
realistically they might have been generated with sub-optimal settings, though
again, could have just regenerated them].

** User creation
First we need a new SSH key.
#+begin_src bash
# local machine
ssh-keygen -t ed25519 -C "$MY_EMAIL" -f ~/.ssh/$MY_SSH_KEY
cat "$HOME/.ssh/$MY_SSH_KEY.pub"
# dotfile management, optional
chezmoi add --encrypt "$HOME/.ssh/$MY_SSH_KEY"
chezmoi add "$HOME/.ssh/$MY_SSH_KEY.pub"
#+end_src

Now on the server, we're ready to create a new user.

#+begin_src bash
# Create a new user, with admin access
# NOTE(rg): Don't forget to add an ssh-key
setup-user -a
# or if -a is omitted, later
adduser <username> wheel
#+end_src

Note that again, unlike the instructions [[https://wiki.alpinelinux.org/wiki/Setting_up_a_new_user][on the wiki]], the image on Hostinger is
pre-configured, and so ~doas~ is present already, with the ~wheel~ group setup
as well.

Then a few baseline packages:

#+begin_src bash
apk add zsh mosh tmux
chsh # to /bin/zsh
#+end_src

Now might be a good time to revisit having a ~salt~ or ~ansible~ configuration,
but perhaps later, if this happens more than once.

For now, though, after
checking that logging in works alright, we will just "lock" the ~root~ account:

#+begin_src bash
passwd -l root
#+end_src

For the rest of this article we will be operating from the user account accessed
via:

#+begin_src bash
mosh -i --ssh="ssh -i $MY_SSH_KEY" "rgoswami@$MY_VPS_IP"
#+end_src

Also to prevent cruft, emptying ~/etc/motd~ is a good idea.
*** No ~dotfiles~
For the server, to keep things light, I use ~TRAMP~ to access files over ~emacs~
and keep the default ~sh~ shell instead of trying to deploy a variant of my
~dotfiles~.
** SSH setup
For starters, we will reconfigure the ~sshd~ settings, having already setup the
required public ~ssh-key~ for access, or if not, after adding the public key to
~$HOME/.ssh/authorized_keys~. These are pretty standard rules, e.g. discussed in
more [[https://www.digitalocean.com/community/tutorials/how-to-harden-openssh-on-ubuntu-20-04][detail here on DigitalOcean]].

#+begin_src diff
diff --git a/oldsshd b/etc/ssh/sshd_config
index 45c5f7d..e70d6fb 100644
--- a/oldsshd
+++ b/etc/ssh/sshd_config
@@ -32,9 +32,9 @@ Include /etc/ssh/sshd_config.d/*.conf
...
@@ -57,11 +57,11 @@ AuthorizedKeysFile	.ssh/authorized_keys
 #IgnoreRhosts yes
...
Which are validated and applied via:
...
Won't be using them anyway, but, ~Kerberos~ and ~GSSAPI~ are not supported out
of the box on Hostinger.
*** +TOTP additions+
Initially I planned to setup PAM for TOTP, mostly along the lines of [[https://wiki.alpinelinux.org/wiki/HOWTO_OpenSSH_2FA_with_password_and_Google_Authenticator][the
AlpineWiki]], the [[https://wiki.archlinux.org/title/Google_Authenticator][ArchWiki]], and this [[https://www.digitalocean.com/community/tutorials/how-to-set-up-multi-factor-authentication-for-ssh-on-ubuntu-20-04][DigitalOcean tutorial]]. However, I realized
there's an easier way to harden connections to the machine, via ~tailscale~.

** ~tailscale~ baseline
Once ~tailscale~ is setup locally, typically via ~tailscale login~, we can set things up on the VPS.
#+begin_src bash
# Adds a repository and gets the latest variant
curl -fsSL https://tailscale.com/install.sh | sh
# needed for setting up the service
echo tun >> /etc/modules
doas rc-service tailscale start
doas rc-update add tailscale
#+end_src
Now we can test that regular SSH connections from the local machine (logged in
to ~tailscale~) to the VPS work.

#+begin_src bash
tailscale ip # vps
ssh $TAILSCALE_IP # local
#+end_src

Since I have ControlD, I use it for the global nameserver in ~tailscale~ and
override local DNS, but it doesn't really matter in this instance.
** Firewall setup
Rather than use ~ufw~, I wanted to try out ~awall~, the [[https://git.alpinelinux.org/awall/about/][Alpine Wall]], since
~json~ is marginally more endearing than arbitrary plain-text. Partially derived from @SettingAlpineWall.

#+begin_src bash
doas apk add awall
#+end_src

For starters, we can deny everything, everywhere.

#+begin_src json
{
    "description": "Default awall internet policy rejecting everything",
    "variable": {
        "internet_if": "eth0"
    },
    "zone": {
...
Which can (and should) be paired with a rule to accept things from the ~tailnet~.

#+begin_src json
{
    "description": "Default awall policy accepting everything in a tailnet",
    "variable": {
        "tailnet_if": "tailscale0"
    },
    "zone": {
        "tailnet": {
            "iface": "$tailnet_if"
...
As a fallback, it is also good to have a default ~ssh~ option (for the
internet), even though this will largely be disabled.

#+begin_src json
{
    "description": "Allow incoming SSH access over the internet (TCP/22)",
    "filter": [
        {
            "in": "internet",
            "out": "_fw",
...
The granularity of these rules is really neat, and in general, even the
~tailnet~ connections should be validated against attacks, so for example, a
~tln-ssh.json~ might be:

#+begin_src json
{
    "description": "Rate limit incoming SSH access over the tailnet (TCP/22)",
    "filter": [
        {
            "in": "tailnet",
            "out": "_fw",
...
*Note*
#+end_alert-heading
For some reason, files named with ~-~ work, but not named with ~_~, so ~tln-ssh.json~ is fine, but not ~tln_ssh.json~.
#+end_alert

With this, we can now test these out:

#+begin_src bash
doas awall enable default
doas awall enable tailnet
doas awall enable tln-ssh
doas awall list
deny-in  enabled   Default awall internet policy rejecting everything
ssh      disabled  Allow incoming SSH access over the internet (TCP/22)
tailnet  enabled   Default awall policy accepting everything in a tailnet
tln-ssh  disabled  Rate limit incoming SSH access over the tailnet (TCP/22)
doas awall activate
#+end_src

*** Service definitions
The setup above works for using ~ssh~ over the tailnet nicely
[fn:: I haven't really felt the need to setup the [[https://tailscale.com/kb/1193/tailscale-ssh][Tailscale SSH]] (using the ACL interface)],
however ~mosh~ expects a range of ports, by default starting at ~60001~ and
ending at ~61000~.  So we define a custom
~/etc/awall/private/custom-services.json~ for it:
...
Which we will then be able to use in an optional service:

#+begin_src json
{
    "description": "Allow mosh over tailnet",
...
While it is good to deny access to the server, we need not be so restrictive
about outbound connections, so:
#+begin_src json
{
    "description": "Allow basic outgoing traffic from local",
    "filter": [
        {
            "in": "_fw",
            "out": "internet",
            "service": ["dns", "http", "https", "ping", "git", "ssh"],
            "action": "accept"
        },
        {
            "in": "_fw",
...
Where we are more permissive with the ~tailnet~, as always. The service definition for ~git~ is:
...
Though ~ssh~ needs to be open as well.

*** Hostinger hardening
It is possible to apply rules at both the OS level, and mirror them hosting
service (Hostinger) level, for reasons why, [[https://support.hostinger.com/en/articles/8172641-how-to-use-a-managed-vps-firewall][see here]] (TL;DR it can be cheaper to
do so).

As a demonstration, the firewall configuration denying standard ~ssh~ and ~mosh~ access can look like:

#+DOWNLOADED: screenshot @ 2024-09-04 13:34:10
[[file:images/Firewall_setup/2024-09-04_13-34-10_screenshot.png]]


Note that this is a very high level filter, so it cannot be used for connecting
to a reverse proxy or redirection.

** ~podman~ details
The easiest way to handle web-services is via ~docker~, and its also why there's
no need to setup any ~glibc~ contortions. However, I wanted to try ~podman~
anyway, so we shall. Mainly because there's no additional service to run in the
background.
...
Also instead of setting up ~cgroups~ for ~root~ execution, we will instead
follow the [[https://wiki.alpinelinux.org/wiki/Podman][AlpineLinux Wiki]] for ~rootless~ usage.

#+begin_src bash
doas su
usermod --add-subuids 100000-165535 --add-subgids 100000-165535 rgoswami
podman system migrate
exit
doas vi /etc/fstab # add shared to the mount options
reboot
podman run --rm hello-world
#+end_src

We will use ~portainer~ to manage things (securely).
...
* TODO Designing a course website
** Background
In my never ending battle to find a living wage / strive towards a permanent
position within academia, I ended up accepting my first teaching engagement.
Naturally a website for the course was in the works. I thought again of ~nanoc~
(with its excellent multi-document output design) ~pollen~ (not just because of
Matthew, but also because of Typography for lawyers, Beautiful racket)..  or
even ~mdbook~ which worked very well for me in W3cm. Or any of my many other
design attempts from ~sphinx~ to whatever has become of the new "executable
book" ~mystmd~ thing (why everyone wants to "overtake / consume")

In the end I decided to stick to ~hugo~, with a suitable doc theme; though
~amythest~ with backlinks seemed very interesting as well. The reason was
simple; because ~ox-hugo~ provides the best ~orgmode~ integration (beyond
publishing directly with ~orgmode~).
* TODO Talk Supplements for the 2022 PyData Global Impact Program
* TODO Electronic Structure Library and CECAM
* TODO Talk Supplements for the 6th IHPC Workshop
* TODO LaTeX and Snakemake for reactive programming workflows
* TODO Stan Math and Meson
* TODO Analytics again
* TODO Python Projects with Native Extensions
** Background
Building things is a pain. ~python~ projects which use ~c~ extensions are
especially painful in a post ~setup.py~ world. ~pdm~ seems to be a promising
candidate tracking ~pep 518~, so it makes sense to try it out. In the same vein,
~meson~ makes the process of actually building and linking a compiled native
extension trivial. Together, these can be leveraged to develop several
interesting packages combining compiled code and standard ~python~ packaging.
** System packages
Though ~nix~ is definitely the best approach to obtaining development
environments, for usage where it is not commonly available, a ~conda~
environment will suffice. We will prefer to install and manage ~python~
dependencies through ~pdm~, but ~C++~ libraries and ~python~ versions are best
handled separately here. This is to (attempt to) establish a system-agnostic
build development toolchain.
#+begin_src yaml
# To use:
#
#   $ micromamba env create -f environment.yml -p $(pwd)/.envcond  # `micromamba` works too for this command
#   $ micromamba activate $(pwd)/.envcond
#
channels:
  - conda-forge
dependencies:
...
Since we have obtained ~pdm~ as part of the ~conda~ environment, we can use it
directly via ~pdm init~. This asks for user-input about which ~python~
interpreter to use, this workflow is amenable to other package management
schemes, including system packages.

- The build backend does not matter, as it will be changed to ~meson-python~

Note that only committed files will work with ~python -m build~ because of the
way ~meson dist~ works. This means we need ~README.md~, ~pyproject.toml~,
~meson.build~ along with the library files to be committed (~CPP~ and ~python~).

At the moment, ~pdm build~ will fail, even though ~python -m build~ and ~pip
install .~ will both work. This means most things will not work well. The same
is true of most of the other front-ends which also happen to provide backends
and vice versa. So we fall back on an older idea. We would like to use our own
build backend, and a front end which is decoupled from having to rely on a
specific backend. In short, we want to migrate towards something like ~pipenv~.
** Compiled extensions
We will need a simple example. For our purposes, ~pybind11~ will suffice.
*** ~meson~ and compiled extensions
* DONE Customizing ArchLinux Installation Media :@notes:ramblings:setup:archlinux:
CLOSED: [2023-04-30 Sun 15:22]
:PROPERTIES:
:EXPORT_FILE_NAME: customizing-archlinux-im
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("ArchLinux Chronoscope")
:END:
#+begin_quote
A short plug for ~hzArchiso~, and general thoughts on ~archiso~
#+end_quote
** Background
I recently had to switch server boxes, due to a cascade of hardware errors
stemming from a remodeling of my home. The new box, however, did not see fit to
include a WiFi card, which meant I had to add one of my own, a Realtek 88XXau,
the kernel module for which is [[https://aur.archlinux.org/packages/rtl88xxau-aircrack-dkms-git][on the AUR]].

This meant the official ArchLinux ISO would not be able to connect to the
internet, and short of copying over files from PC to PC manually with a USB
stick, my only option was to build my own installation media.

This then snowballed a little into a customized live-USB project called
[[https://github.com/HaoZeke/hzArchiso/][hzArchiso]] with a bunch of bells and whistles I need [fn:kindavague]. This post
is not meant to supplant [[https://wiki.archlinux.org/title/Archiso][the ArchWiki]] and does not provide an in-depth
introduction to using ~archiso~ [fn:whydisclaim].
** Makepkg tweaks
When building system images, I tend to modify my ~/etc/makepkg.conf~ to use all
cores and also use the faster ~mold~ linker (detailed on the [[https://wiki.archlinux.org/title/Makepkg#Parallel_compilation][ArchWiki]]):
#+begin_src bash
MAKEFLAGS="-j$(nproc)"
...
- ~$GITFLAGS~ support needs ~pacman-git~ as of July 2023, check the [[https://wiki.archlinux.org/title/Makepkg#Reduce_source_download_and_extraction_times][ArchWiki for details]].
** Managing AUR packages
~repoctl~ ([[https://github.com/cassava/repoctl][repo]]) is a fantastic tool for setting up local repositories of
...
~archiso~ [[https://wiki.archlinux.org/title/Archiso][ArchWiki entry]].
...
It turns out that my new machine required a wireless card patch. I haven't
compiled a kernel for several years now[fn:sincewhen]. The process was
gratifyingly simplistic, following some discussion threads on the [[https://bbs.archlinux.org/viewtopic.php?pid=2103599#p2103599][Archlinux
Forums]].
...
..
source = (
    "v2-wifi-iwlwifi-pcie-add-device-id51F1-for-killer-1675.patch::https://lore.kernel.org/linux-wireless/20230608082725.353150-1-yi@yikuo.dev/raw"
)
..
b2sums=(
    'e9f30d94b84a019c38842fbb468c64522782d14a347a12442704e185b4d9b131dc25661e7e9b9640ac20e0c949f5fa0375739d0c1badd0a0c637408a56091118'
)
#+end_src

For building the ~nitrous~ kernel (an [[https://wiki.archlinux.org/title/Kernel#Unofficial_kernels][unofficial one]] for newer hardware) then:
#+begin_src bash
...
Building kernels takes around as long as I remember, several hours at  on an
older laptop.
** Using ~hzArchiso~
The general idea is to follow along with the ArchWiki [[https://wiki.archlinux.org/title/Installation_guide][installation steps]]
(perhaps augmented by the [[https://wiki.archlinux.org/title/Dual_boot_with_Windows][dual-booting]] instructions) For some users, the
~archinstaller~ script ([[https://python-archinstall.readthedocs.io/en/latest/index.html][repo]]) might be a better option. There's also an
~offlineInst.sh~ script which can be used to replicate the environment on the
live-USB.
** Conclusions
Modifying ArchLinux installation media is fun. Even if the exact set of packages
bundled with ~hzArchiso~ isn't for most, it serves as a useful blueprint for
...
[[https://github.com/HaoZeke/hzArchiso][on the repo]] and [[https://wiki.archlinux.org/title/Archiso][on the ArchWiki]]. Personally, I expect to update the ~iso~ every now and then with
more of my configuration / defaults / every time I get a new machine. I've also
been using ~btrfs~ for almost a decade without any trouble [fn:why], which is anecdotal
but nice.

[fn:whereswap] This is derived from [[https://nerdstuff.org/posts/2020/2020-004_arch_linux_luks_btrfs_systemd-boot/][this post]], which in turn references the ArchWiki for [[https://wiki.archlinux.org/title/Btrfs#Swap_file][BTRFS]] and [[https://wiki.archlinux.org/title/Swap#Swap_file][swap]].
[fn:why] Transparent compression is a rather neat feature
[fn:kindavague] My first attempts at modifying the installation media date back to [[https://github.com/HaoZeke/hzArchiso/commits/main?after=7fe66527b0029053398ca3a97039d29cb786db5f+69&branch=main&qualified_name=refs%2Fheads%2Fmain][2018 though]]
[fn:sincewhen] Since I largely retired from Android {{{relref(custom kernel / ROM work,linos-maintainer-appre)}}}
[fn:whydisclaim] Seriously, *always* read the [[https://wiki.archlinux.org/][ArchWiki]] over blog posts, sometimes the [[https://bbs.archlinux.org/][ArchLinux forums]] are useful too

* DONE Laptop setup notes ~btrfs~ subvolumes :@notes:ramblings:setup:archlinux:
CLOSED: [2023-05-01 Mon 15:22]
:PROPERTIES:
:EXPORT_FILE_NAME: laptop-setup-btrfs-notes
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("ArchLinux Chronoscope")
:END:
#+begin_quote
...
#+end_alert
** Background
I split this post from the {{{relref(previous one,customizing-archlinux-im)}}}, to make each more self contained.
...
format it as unformatted. This is so I can use [[https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system][encrypted]] BTRFS
[[https://wiki.archlinux.org/index.php/Btrfs#Subvolumes][subvolumes]][fn:anysources].
...
from [[https://github.com/archlinux/archinstall/issues/781][this issue]] (more on layouts [[https://github.com/lnussel/lnussel.github.io/blob/fs/_posts/2020-12-16-fslayout.md][here]] and [[https://www.jwillikers.com/btrfs-layout][here]]):
#+begin_src bash
...
# Manage each home separately
btrfs sub create /mnt/@home # /home
btrfs sub create /mnt/@pkg # /var/cache/pacman/pkg
btrfs sub create /mnt/@.snapshots
# VMs shouldn't be rolled back
btrfs sub create /mnt/@VMs # /var/lib/libvirt/images
btrfs sub create /mnt/@containers # for docker
# Log files should be consistent
btrfs sub create /mnt/@log # /var/log
# Often has licensed software
btrfs sub create /mnt/@opt # /opt
btrfs sub create /mnt/@usr_local # /usr/local
umount /mnt
#+end_src

These can then be mounted with reasonable defaults ([[https://www.jwillikers.com/btrfs-mount-options.html][detailed here]]):

#+begin_src bash
mount -o noatime,nodiratime,compress=zstd,space_cache=v2,ssd,subvol=@ /dev/mapper/luks /mnt
mkdir -p /mnt/btrfs # To mount the btrfs root: https://unix.stackexchange.com/q/367123/246530
...
mkdir -p /mnt/{boot,home,var/cache/pacman/pkg,.snapshots,var/lib/libvirt/images,var/log,opt,usr/local}
...
mount -o noatime,nodiratime,compress=zstd,space_cache=v2,ssd,subvol=@VMs /dev/mapper/luks /mnt/var/lib/libvirt/images
mount -o noatime,nodiratime,compress=zstd,space_cache=v2,ssd,subvol=@log /dev/mapper/luks /mnt/var/log
mount -o noatime,nodiratime,compress=zstd,space_cache=v2,ssd,subvol=@opt /dev/mapper/luks /mnt/opt
mount -o noatime,nodiratime,compress=zstd,space_cache=v2,ssd,subvol=@usr_local /dev/mapper/luks /mnt/usr/local
...
#+begin_src bash
mount -o rw,relatime,fmask=0022,dmask=0022,iocharset=ascii,shortname=mixed /dev/nvme0n1p1 /mnt/boot
#+end_src

Switch to sddm with weston for a better display manager
***** Handling ~swap~
To get the physical offset, as [[https://wiki.archlinux.org/title/Power_management/Suspend_and_hibernate#Hibernation_into_swap_file_on_Btrfs][noted on the ArchWiki]] we can use ~btrfs inspect-internal~[fn:whereswap].
#+begin_src bash
cd /mnt/btrfs/@swap
truncate -s 0 ./swapfile
chattr +C ./swapfile
btrfs property set ./compression none
...
# Locale is set already to C.UTF-8
# Setup timezone
ln -sf /usr/share/zoneinfo/<WHATEVER> /etc/localtime
...
passwd username
usermod -aG wheel username # for sudo
#+end_src
...
#+begin_src
userdel hzlinarch
rm -rf /home/hzlinarch
#+end_src

Some other things to be deleted (should be updated in the script):
#+begin_src bash
sudo rm -rf /var/lib/pacman/local/mkinitcpio-archiso-*
sudo rm -rf /usr/lib/initcpio/hooks/archiso*
sudo rm -rf /usr/share/doc/mkinitcpio-archiso
#+end_src
...
#+begin_src bash
# /etc/mkinitcpio.conf
HOOKS=(base keyboard udev kms autodetect modconf block keymap encrypt btrfs filesystems resume)
#+end_src
...
- Thanks to a [[https://bugs.launchpad.net/ubuntu/+source/file-roller/+bug/1923033][very old]] archive-manager (file-roller) bug, there aren't any really good native options for compression
...
*** Laptop Tweaks
Mostly garnered from the [[https://wiki.archlinux.org/title/Laptop][Archwiki Laptop page]]. I generally setup ~/etc/systemd/{logind,sleep}.conf~ with defaults from my older ~etckeeper~ configuration.
...
[fn:anysources] Based on the setup described [[https://nerdstuff.org/posts/2020/2020-004_arch_linux_luks_btrfs_systemd-boot/][here]],
** Conclusions
At this point, my notes came to an abrupt end. Presumably this meant I was able
to cobble together the rest of the system using the pointers there.
* DONE Handling Legacy Fortran Code :@notes:fortran:f2py:
CLOSED: [2023-04-27 Thu 16:09]
:PROPERTIES:
:EXPORT_FILE_NAME: handling-legacy-fortran-code
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/myWork.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Fortran Frontiers: Bridging Legacy to Modernity")
:END:
#+BEGIN_QUOTE
A short pseudo-tutorial post on working with inherited Fortran.
#+END_QUOTE
** Background
In-spite of the many claims by upper management (e.g. [[https://fortran-lang.discourse.group/t/an-evaluation-of-risks-associated-with-relying-on-fortran-for-mission-critical-codes-for-the-next-15-years/5644?u=rgoswami][at LANL]]) regarding the death
of Fortran, most people with even a passing interest in academic research within
STEM fields will interact with Fortran code, and the community has only been
improving with time @kedwardStateFortran2022. Rewriting *is not* typically an
option, or a good use of time. This set of posts should be applicable to any
blackbox legacy fortran code, though the specific example here is one from my
own field (computational chemistry).
** The Code
As an exemplary example, we will consider an implicitly typed Fortran-77 program
for generating the potential energy surface (and forces) of a system containing
Copper and Hydrogen atoms. The group I'm associated with has a rather nice
Embedded Atom Model parameterized for this particular system [fn:whygood]. The
source can be obtained from here:

#+begin_src bash
git clone https://github.com/TheochemUI/fortran_cuh2_src
#+end_src

Since it was initially meant to work with ~con~ files, the salient points of the
implementation are:
...
#+begin_src bash
   ...
   2862.30      79.5013      86.1495
   3.51236      2.47961      4.21154
  -109.107     -107.555      15355.5
  ....
#+end_src

Additionally the code is in FORTRAN 77, and uses features no longer in vogue
including implicit typing and ~common~ blocks.

*** Initial Cleanup
Rather than hard-coding a path (~pot.par~) it is best to store these values in
the source code itself.
#+begin_src python
potpar = np.loadtxt("pot.par", skiprows=1) # first row has 2 values
for idx, val in enumerate(aa, start=1):
    print(f"potpar({idx}, 1) = {val[0]}\npotpar({idx}, 2) = {val[1]}\npotpar({idx}, 3) = {val[2]}")
#+end_src

There are many more possible optimizations which can be done on the Fortran
side, but this is the bare minimum we need to move on. The state of the project
can be seen in [[https://github.com/TheochemUI/fortran_cuh2_src/commit/1461cdf927f69f2da75a5da75e566faa66b88267][1461cdf]]. The ~main.f90~ issues regarding the file I/O can be
handled from different languages.
** Python Usage
...
#+begin_src bash
f2py -c -m cuh2 eamroutines.f90
#+end_src
Now we can use this from Python:
#+begin_src python
import numpy as np
import cuh2 # Compiled module
...
box = np.array([15.34, 21.702, 100])
natms = np.array([2, 2])
u = np.zeros(1)
F = np.zeros_like(R)
cuh2.force_eam(natms, box, R.ravel(), F.ravel(), u) # Call!
print(u)
[-2.71140933]
#+end_src
This works very well, and in-fact, if this use case (flexibly calling a
function) is all that is needed, then nothing more needs to be done. ~f2py~ also
generates correct signatures, and doc-strings:

#+begin_src python
??cuh2.force_eam
force_eam(natms,box,r,f,u,[ndim])

Wrapper for ``force_eam``.

Parameters
----------
natms : input rank-1 array('i') with bounds (2)
box : input rank-1 array('d') with bounds (3)
r : input rank-1 array('d') with bounds (ndim)
f : input rank-1 array('d') with bounds (ndim)
...
#+begin_src c
extern void c_force_eam(int *natms, int ndim,
                        double *box, double *R,
                        double *F, double *U);
#+end_src
We can always check that the right functions are where they should be:
#+begin_src bash
gfortran -c eam_isoc.f90 eamroutines.f90
nm eam_isoc.o
0000000000000000 T c_force_eam
                 U force_eam_
nm eamroutines.o
000000000000000c C counters_
0000000000000050 C cutoff_values_
00000000000065c8 C eam_
0000000000000130 C eamdblexp_
00000000000025e8 T eamfcu_
000000000000249b T eamfh_
0000000000000000 T eamh2cu_
00000000000029ef T eamphicucu_
00000000000028f8 T eamphihcu_
0000000000002801 T eamphihh_
0000000000002331 T eamrhocu_
00000000000021c7 T eamrhoh_
                 U exp
0000000000003210 T force_eam_
...
0000000000002ae6 T potinit_
                 U __powidf2
                 U __stack_chk_fail
#+end_src
*** MATLAB Interfaces
Nominally, MATLAB has a [[https://www.mathworks.com/help/matlab/call-mex-fortran.html][Fortran API]]. However, it isn't great. The ~C++~ API is
"modern" to the [[https://www.mathworks.com/help/matlab/matlab_external/c-mex-functions.html][extent of of C++11]], which is practically useless. So the ~C~ [[https://www.mathworks.com/help/matlab/cc-mx-matrix-library.html][MEX
API]] is the best way forward here, though *input arrays need to be transposed*
if they are passed to Fortran routines.
** Conclusions
This was a first pass at what to do when confronted with Fortran code, short of
running away, or rewriting entirely. Follow up posts handle integrating into
larger C++ codes, or distributing said libraries with ~pip~ [fn:whynothere].


[fn:whygood]  For a copper slab and hydrogen molecule, the potential is qualitatively correct. It captures the fact that when the slab is fixed, there are effectively only two degrees of freedom, the distance between the hydrogen atoms, and the distance to the slab.
[fn:whynothere] Most of the issues which crop up in embedding and distribution are largely build system related, and require no changes to the Fortran code other than a compatible ~iso_c_binding~ wrapper so they are extracted away.
* TODO MATLAB bindings for Fortran Libraries through ~C~
:PROPERTIES:
:EXPORT_FILE_NAME: matlab-fortran-c
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/myWork.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Fortran Frontiers: Bridging Legacy to Modernity")
:END:
#+begin_quote
C-Matlab is a lot more pleasant to work with
#+end_quote
* TODO C++ Libraries & Python: The cibuildwheel Way
:PROPERTIES:
:EXPORT_FILE_NAME: cpplib-py-cibuildwheel
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/myWork.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Fortran Frontiers: Bridging Legacy to Modernity")
:END:
#+begin_quote
Where we lay out design guidelines and build practicalities for C++
libraries which may have embedded Fortran code
#+end_quote
* DONE CPP Library Blueprints for Python Bindings :@programming:design:bindings:python:cpp:
CLOSED: [2023-06-28 Wed 11:27]
:PROPERTIES:
:EXPORT_FILE_NAME: cpp-library-blueprints-pybind
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:END:
#+begin_quote
...
#+end_quote
** Background
I recently ended up writing and rewriting a series of libraries to essentially
get them to a point where I could write bindings to them. In doing so, some
thoughts on design have evolved into this post.
*** False starts
...
wasn't always backported to ~main~ in a timely fashion
** Setup Preliminaries
...
project ([[https://github.com/TheochemUI/potlib][library]], [[https://github.com/TheochemUI/pypotlib][bindings]]), which is small and innocuous enough to grok
hopefully.
*** Tools
For most new projects, I use ~pybind11~ [fn:nanobind] with ~meson~.
...
- ~cibuildwheel~ is used for actually getting the wheels for ~pypi~
  + This is a bit of a nightmare, need to use [[https://repology.org/project/eigen/versions][repology]] and fiddle around with
    figuring out which version of each OS is used by each specification
  + Any kind of system based package management was exceedingly slow, so
    sticking to ~pip~ and ~meson~ managed dependencies is the safest bet
** Library Layout
...
[[https://git-scm.com/book/en/v2/Git-Tools-Submodules][Git submodules]] are generally the first thing to reach for when working with
repositories which depend on each other, but I found ~meson~ [[https://mesonbuild.com/Subprojects.html][subprojects]] to be
less finicky if the library and bindings are all compatible with ~meson~. The
documentation omits setting up a ~.wrap~ file for a project, so consider the
following (from ~pypotlib~):
#+begin_src toml
[wrap-git]
directory=potlib
url=https://github.com/TheochemUI/potlib.git
revision=5d029b9
...
#+end_src
This makes it easier to pull variables from subprojects in a seamless manner:
Linking to the library itself is best done by declaring a dependency [fn:whatifnot]:
#+begin_src meson
# ---------------------- Library Dependencies
potlib_proj = subproject('potlib', version: '0.1')
# optionally use:
                        # default_options: ['default_library=static'])
potlib_dep = declare_dependency(link_with: potlib_proj.get_variable('_linkto'),
                               dependencies: potlib_proj.get_variable('_deps'))
_deps += [ potlib_dep ]
#+end_src
Setting up targets is then also pretty straightforward:
#+begin_src meson
blahlib = library('blah',
                 src_files,
                 dependencies: _deps,
                 cpp_args: _args,
                 link_with: _linkto,
...
Although a single file can be used for the ~pybind11~ bindings, I prefer a more
...
#+end_src
For a concrete example consider this [[https://github.com/TheochemUI/pypotlib/commit/99b944cf41544e87e2e2170a25eff5e2860b20b7][pypotlib commit]].
** Conclusions
So far this covered creating a single compiled extension for thin bindings to an
existing ~c++~ library. Using PRs and submodules turned out to be insufficient
to maintain code quality and I ended up shifting to a subproject based workflow.
Most of these evolved during my work on [[https://pypi.org/project/pypotlib/][pypotlib]], and a newer version of ~eON~,
the long timescale dynamics code, which isn't (as of April 2024) public
yet[fn:academicFOSS]. A follow up post will cover adding pure python code which
can be used for generating a more pythonic interface and also  the requirements
for distribution via PyPI.

[fn:likewhen] Most recently when working on [[https://github.com/ERGO-Code/HiGHS][HiGHs bindings]] to eventually simplify its integration with SciPy
[fn:nanobind] ~nanobind~ is interesting but lacks ~Eigen~ support, and most of the codes I interface to / from use ~Eigen~ arrays
[fn:academicFOSS] Academia is rather odd about open source development before publication, reviewers almost never bother to even run software, let alone check authorship / provenance
[fn:whatpy] This IMO is a myth, and ~python~ scripts are way more convoluted to share than most things, but they certainly feel like they should be easy to share
[fn:whatifnot] This was a bit of a gotcha, since adding variables to the project's ~_deps~ and ~_linkto~ didn't work
* TODO Packages with Python and Compiled Extensions with ~meson~
** Background
The title is a work in progress. After finally getting the build-bot setup
correctly for the bindings. I realized I needed to refactor to add adapters to
~ase~. More generally, the problem is of embedding a compiled extension in a
package which has other pure-python code snippets as well.
** Distribution
What does distribution really take? I've dabbled with writing extensive
documentation, setting up ~docker~ containers, ~snakemake~ workflows, ~nix~,
~spack~, ~easybuild~, but they all fundamentally require too much effort at both
ends to get things working. This is cuts deeper for things which are _supposed_
to be simple to share, like ~python~ scripts[fn:whatpy].
** Pure Python and ~meson~
~meson~ doesn't support ~python~ as a first class language. The general idea is
to muck around with the fact that copying over ~.py~ files to the "right place"
is essentially an installation.
As seen in ~scipy~:
#+begin_src python
py3.install_sources([
    '__init__.py',
...
#+end_src
Which is, exciting in a way. ~macos_arm64~ is a cross compile currently and so
not a good idea. ~numpy~ has a nice build wheels only if commit contains [wheel
build] which is nice to kang.
** Details
- ~musllinux~ doesn't ship with ~openblas~, so adding ~scipy~ or anything
  depending on it as a project dependency is going to break
  + [[https://github.com/TheochemUI/pypotlib/commit/f46933c22b5ebdcd884d2a93a13589fa0d1deeaf][Kanged and adapted]] ~scipy~'s solution to this, since it is so much faster to
    copy ~openblas~ in than use the ~apk~ package manager
  + It is still massively slow, around 30 minutes to test
  + Either drop ~musllinux~ wheels or just don't test them
    + The dependencies are only resolved during the installation which is done prior to testing the wheel
* TODO MATLAB interfaces for F77 code :@notes:fortran:matlab:c:
#+begin_quote

#+end_quote
** Background
I inherited a Fortran 77 potential function a while ago. Naturally this meant
interfacing said potential to everything under the sun. More specifically I also
inherited a MATLAB codebase which delighted in calling binary executables and
reading in and out of files. This post has some pitfalls relating to the same.

It quickly became apparent, browsing the documentation, that though MATLAB and
Fortran share storage orders, the API for C is much saner than the Fortran
variant, which seems to have been cobbled together almost as an afterthought to
the C MATLAB API.
** Setup

** Implementation
*** Continuous local builds
Although the ~mex~
** Unit Testing
Thankfully, ~MATLAB~ supports rudimentary unit-tests, though they haven't
figured out how to check for approximate equality of matrices.
** Continuous Integration (CI)
Although ~MATLAB~ is *very* proprietary, Github Actions (and other CI services)
have the ability to run ~MATLAB~ and in particular, execute ~runtests~ to
validate the codes. The only subtlety here is that the environment variables
need to be set with some care.
#+begin_src yaml
...
#+end_src
** Conclusions
Hopefully I'll never need to write much more ~MATLAB~. There are a laundry list
of deficiencies in the language, from the naming conventions to the lack of
printing of structured data. I'm not sure any of this would be considered to be
a "best practice", however, it has proven to be satisfactory to me so far.
* TODO Exporting single file Fortran libraries
:PROPERTIES:
:EXPORT_FILE_NAME: export-lib-fortran
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Fortran Frontiers: Bridging Legacy to Modernity")
:END:
#+begin_quote
On distributing Fortran libraries
#+end_quote
** Background
Fortran has no real equivalent to ~pypi~ and friends. There is ~fpm~, which is great, but also not that well supported, and has only patchy support for multi-lingual projects.
** Managing dependencies
Fortran parsing takes up an unholy amount of my time [fn:whyso]. Perhaps because
of that, I was wary of reaching for yet another fully fledged Fortran parser.
Besides, from the point of view of exports, this is really more of a build
system issue. I've mentioned ~fortdepend~ a long time ago when I was starting
out as a GSoC student, where I used it to find module dependencies.
Unsurprisingly it can be used, therefore, to get a reverse list of imports which
are needed for a single file export.

I eventually wrote a tiny package for this.

[fn:whyso] ~f2py~. 'nuff said.
* DONE Dart Sass and Hugo Themes :@notes:webdev:
CLOSED: [2023-06-17 Sat 17:29]
:PROPERTIES:
:EXPORT_FILE_NAME: dart-sass-hugo-themes
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+BEGIN_QUOTE
...
#+END_QUOTE
** Background
Recently I decided that it was annoying for me as a user to not know which links
on my site were internal and which led off to an external site.

#+DOWNLOADED: screenshot @ 2023-06-17 16:48:01
#+caption: The desired end-result, blue for internal links, purple for external links, colors chosen with [[https://www.colorhexa.com/][ColorHexa]]
[[file:images/Background/2023-06-17_16-48-01_screenshot.png]]

However, since ~hugo~ requires ~baseurl~ for generating meaningful ~rss~ and
~sitemap~ files, this meant either hardcoding the variable into [[https://github.com/HaoZeke/hugo-theme-hello-friend-ng-hz][my theme]], or
figuring out how to pass variables from my ~config.toml~ to my theme's
(generated) ~css~.

This about the latter, and is heavily based off of this [[https://discourse.gohugo.io/t/initialize-sass-variables-from-hugo-templates/42053][post on the Hugo
Discourse]].
** The CSS
...
detailed in the [[https://discourse.gohugo.io/t/initialize-sass-variables-from-hugo-templates/42053][discourse post]], it is conceptually clunky, since partials are
not respected when using ~ExecuteAsTemplate~ with [[https://gohugo.io/hugo-pipes/resource-from-template/][hugo-pipes]]. A better SCSS
...
in favor of [[https://sass-lang.com/dart-sass][Dart Sass]] and includes (among other features) a nifty ~math~
module. This had a few side effects:

- I could swap my hand-kanged ~exp~ function for ~math.exp()~ and also had to transition to using ~calc()~ more (as in [[https://github.com/HaoZeke/hugo-theme-hello-friend-ng-hz/commit/3938ac24d3c9e017a6ff633383d3188c5915257f][3938ac2]]).
- I could pass variables from ~hugo~ to ~scss~ and from there to ~css~ for attribute selectors (as in [[https://github.com/HaoZeke/hugo-theme-hello-friend-ng-hz/commit/804d417f1436dc3f58d770a717985fe43cb3813c][804d417]])
*** CI Updates
For various reasons, ~hugo~ (even the extended version) does not seem to include
~dart-sass-embedded~ though the ~snap~ package does (as noted [[https://discourse.gohugo.io/t/using-the-dart-sass-transpiler/41878][here]] on Discourse).
#+begin_src yml
- name: Setup Hugo
...
#+end_src
** Conclusions
Of the many maintenance burdens I have accrued over the years, one of the least
...
any rate.
* DONE Managing Scanned Books :@notes:workflow:ramblings:
CLOSED: [2023-06-26 Mon 21:34]
:PROPERTIES:
:EXPORT_FILE_NAME: managing-scanned-books
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Modern reading")
:END:
#+begin_quote
...
#+end_quote
** Background
It so happened that I recently acquired a rather unique book which (somewhat
oddly) had no corresponding digital variant [fn:whybother]. I've discussed how I modify books
and papers {{{relref(for consumption on my Kobo Aura HD in the past,my-life-in-eink)}}}, but this comes before
that. Much of this is essentially a plug for [[https://github.com/4lex4/scantailor-advanced][Scantailor Advanced]] [fn:whatadvanced].
** Goals
Starting with the initial output ~pdf~ files from a scanner's "scan to me" function [fn:whyfiles] the goal is to have all the bells and whistles of a well-done digital book:
- Fixing orientations :: This is meant to deal with rotations mostly, skew is later
- Splitting pages :: Since most scanners with books take two pages at a time
- Handling skew :: Deskewing works to reorient pages for visibility
- Reworking content selections :: The automated selection worked will for this, and is especially useful if there are artifacts
- Adding back margins :: For padding post selection
- Output determination :: 600 DPI is generally enough, and "mixed" mode works great for images, the one thing to add is that "Higher search sensitivity" for picture shapes is pretty useful

These six steps can be carried out with Scantailor Advanced, which has a very
intuitive GUI for the same but there are but two more things:
- Optical Character Recognition (OCR)
- A proper Table of Contents

We will also have to first get high resolution images out of the ~pdf~ files.
For starters our project might look like this:
#+begin_src bash
❯ ls
appendix.pdf	chap6.pdf		     images
chap2_chap3.pdf  chap7.pdf		     toc_chap1_intro.pdf
chap4.pdf	chap8.pdf
chap5.pdf
#+end_src
** From ~pdf~ to ~tiff~
Without any bells and whistles, a simple bash script will suffice to extract
high resolution ~.tiff~ files from ~.pdf~ inputs. The only caveat is the need to
carefully format the page names so they can be joined together easily. This lead to:

#+begin_src bash
# The first page number (output file name) for the current PDF
export INP_SCAN_NAME=toc_chap1_intro
output_index=1
pdftoppm -tiff "${INP_SCAN_NAME}.pdf"  temp
i=0
for file in temp*.tif; do
  i=$((i+1))
  new_index=$(printf "%03d" $((output_index + i - 1)))
  new_file="page_${new_index}.tiff"
  mv "$file" "images/$new_file"
done
#+end_src

At this point we can plug the images directly into ScanTailor Advanced and work
through the GUI.
** OCR Setup
Working with the TIFF images which form the output at ScanTailor Advanced is a
good bet. System packages needed are:
#+begin_src bash
trizen -S tesseract tesseract-data-eng \
    tesseract-data-isl \
    ghostscript unpaper pngquant \
    img2pdf jbig2enc pdftk
#+end_src
Subsequently I ended up with a throwaway ~conda~ environment for using [[https://github.com/ocrmypdf/OCRmyPDF][OCRmyPDF]]
(like with all ~python~ tools).
#+begin_src bash
micromamba create -p $(pwd)/.ocrtmp pip ocrmypdf
micromamba activate $(pwd)/.ocrtmp
#+end_src
Since ~ocrmypdf~ works on single PDF files or alternatively single images, one
brute-force approach is simply:
#+begin_src bash
cd out # where scantailor puts things
mkdir temp
for file in page*.tif; do
  base=$(basename "$file" .tif)
  ocrmypdf -l eng --output-type pdf "$file" "temp/${base}.pdf"
done
#+end_src
Which leads to a series of ~.pdf~ files.
** Concatenation and Compression
[[https://www.ghostscript.com/][Ghostscript]] is the best way to get a single reasonably *compressed* output from
these files[fn:whatelse]:
#+begin_src bash
export OUTPUT_PDF_DPI=500 # good enough for me
# Or 600 for the Scantailor default
gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite \
    -dCompatibilityLevel=1.7 -dPDFSETTINGS=/ebook \
    -dEmbedAllFonts=true -dSubsetFonts=true \
    -dColorImageDownsampleType=/Bicubic \
    -dColorImageResolution=$OUTPUT_PDF_DPI \
    -dGrayImageDownsampleType=/Bicubic \
    -dGrayImageResolution=$OUTPUT_PDF_DPI \
    -dMonoImageDownsampleType=/Bicubic \
    -dMonoImageResolution=$OUTPUT_PDF_DPI \
    -sOutputFile="output_${OUTPUT_PDF_DPI}.pdf" \
    temp/*.pdf
#+end_src
Where the compatibility level is to [[https://stackoverflow.com/questions/29417217/how-are-pdf-files-able-to-be-partially-displayed-while-downloading][allow for linearization]] [fn:whatlinis] (needs [[http://www.adobe.com/content/dam/Adobe/en/devnet/acrobat/pdfs/pdf_reference_1-7.pdf][PDF 1.7]]).
Sometimes this leads to files which are still too large (Table ref:tbl:sizes).

#+label: tbl:sizes
#+name: tbl:sizes
#+caption: For a color book
| DPI | Size (in MB) |
| 300 |        139.2 |
| 600 |        303.5 |

In such cases ~pdfsizeopt~ (from [[https://github.com/pts/pdfsizeopt][here]]) can be setup in the environment with:
#+begin_src bash
mkdir $CONDA_PREFIX/tmp/pdfsizeopt
cd $CONDA_PREFIX/tmp/pdfsizeopt
...
mv * $CONDA_PREFIX/bin && cd $CONDA_PREFIX/bin
ln -s pdfsizeopt.single $CONDA_PREFIX/bin/pdfsizeopt
rm -rf $CONDA_PREFIX/tmp/*
#+end_src

Which can then be used as:
#+begin_src bash
pdfsizeopt output_600.pdf
#+end_src

The compression takes a while [fn:whywait], but the results are completely
lossless and the reduction was pretty neat, from ~390 MB~ to ~250 MB~, though it
took forever, and [[https://acrobat.adobe.com/link/acrobat/compress-pdf][Adobe Acrobat]] reduced a ~390 MB~ file to ~108 MB~ in minutes.

#+DOWNLOADED: screenshot @ 2023-06-26 21:31:14
#+caption: Lossless compression (~250 MB~) on the left, lossy Acrobat compression (~108 MB~) on the right
[[file:images/Concatenation_and_Compression/2023-06-26_21-31-14_screenshot.png]]


For handling the cover image I ended up with:
#+begin_src bash
# from the ScanTailor 600 DPI image
# 39.3 MB
tiffcp -c zip:p9  cpage-1.tif cpage.tif #  19.7 MB
convert cpage.tif -compress JPEG -quality 90% cpage.jpg # 2.0 MB
#+end_src
Then I dumped it through a [[https://compressjpg.com/][web tool]] [fn:othercompressor](around ~560 KB~) before finishing up
with ~ocrmypdf~ for the final ~pdf~ of ~535 KB~.
** TOC Generation
Sometimes I feel too lazy and end up doing my table of contents editing in
Master PDF editor [fn:reallycommercial]. That being said my most favored way for
a fully fledged TOC with chapters and sub-headings is with the venerable [[https://www.pdflabs.com/tools/pdftk-the-pdf-toolkit/][PDFtk]]:
#+begin_src bash
cat toc.txt
BookmarkBegin
...
#+begin_src bash
pdftk input.pdf update_info toc.txt output output_with_toc.pdf
#+end_src
Sometimes I end up padding the ~pdf~ with a few extra pages to ensure the page
numbers (printed) match the ones in the TOC.
** Linearization
Since we targeted PDF 1.7, with linearization support [fn:whybotherlin]. We need [[https://qpdf.readthedocs.io/][qpdf]]:
#+begin_src bash
micromamba install qpdf
qpdf --linearize output_300.pdf olin300.pdf
#+end_src
...
images from scanner generated ~pdf~ files, and perhaps some other minor
tweaks [fn:likehpc] or other tools [fn:likewhat], though for my purposes, this
...
[fn:whybother] A few papercuts steeled my resolved to get this onto my devices
[fn:whatadvanced] A more maintained fork of the old and venerable Scantailor which now weirdly has [[https://scantailor.org/][a fan-page]] with dubious plugs to something called Truthfinder...
[fn:whyfiles] Scanning the book in one go led to overly large attachments for my email provider. 576.96 766.08
[fn:whatelse] If compression doesn't matter using ~pdftk~ (needed for the TOC) would be fine too.
[fn:whywait] The TOC work can go on simultaneously, and compression can take days
[fn:reallycommercial] They used to have a really nice trial of version 4 which did everything I needed..
[fn:whybotherlin] Some of my devices use web browsers for rendering ~pdf~ files.
[fn:likehpc] ScanTailor could be run on an HPC most probably, as could the ~ocrmypdf~ step, the pdf compression definitely should (took around three days on my ThinkPad X1 Carbon 6th Gen)
[fn:likewhat] [[https://github.com/Krasjet/pdf.tocgen][pdf.tocgen]] is pretty cool
[fn:whatlinis] Swankier [[https://apryse.com/blog/pdf-format/what-is-pdf-linearization][explanation her]]e
[fn:othercompressor] When file-sizes are really a bottle neck there's also the sister [[https://pdfcompressor.com/][pdfcompressor]] site, or for really small files there's the [[https://acrobat.adobe.com/link/acrobat/compress-pdf][Adobe Acrobat compressor]]
* TODO Favorite ~pre-commit~ hooks
** C++ Hooks
** Python Hooks
** Fortran Hooks
* TODO Ruminations on Github Actions for dependent projects :@notes:ramblings:ci:gha:
:PROPERTIES:
:EXPORT_FILE_NAME: ruminations-gha-deps
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
Token hell, or leveraging Github for managing interconnected projects.
#+end_quote
** Background
Projects have dependencies. They may (often) also have very distinct histories
and development goals. Technically ~git~ is fine for these situations as well,
since changes are really tracked at the file level. Nevertheless, it can happen
that projects spin off, but still need rather tight integration, e.g. ~f2py~
changes in ~numpy~ *must* be tested against the latest ~scipy~ to ensure nothing
breaks.

Traditionally, this requirement falls on the maintainers and the onus of
ensuring zero breakage with change-sets to relevant parts of the codebases.
However, this step can also be handled automatically at the CI/CD level.

*** Overview
There is a [[https://github.com/orgs/community/discussions/26323][rather extensive dicussion thread]] over on Github regarding this
situation of dependent repositories. Existing solutions
include [fn:creditmelissa] using trigger-workflow-and-wait as for example in
napari for building their documentation ([[https://github.com/napari/napari/blob/main/.github/workflows/deploy_docs.yml][main repo]], [[https://github.com/napari/docs/blob/main/.github/workflows/deploy_docs.yml][triggered]]).

The rest of this post will drop the generalities and focus on a more concrete
scenario where all repositories on Github [fn:whynotnumpy]. We will consider
three levels of complexity in terms of access controls; where all the
repositories are under a single user, where all repositories are under one
organization, and finally the most general scenario where the repositories to be
linked are under disparate organizations.

*** Basic Workflow Etiquette

We first discuss some of the aspects common to workflow etiquette which will be
elided in subsequent snippets. Concurrency is important, it saves valuable
developer time, especially when there's one active developer per pull request.
When in doubt a reasonable snippet is:
#+begin_src yaml
concurrency:
...
A matrix of operating systems and ~python~ versions can be useful always a good
idea to test the basic trinity of MacOS, Windows and Linux machines. So a
reasonable matrix might be:
...
The tests we discuss in this post can be flaky; ~pypy~ in particular has a
tendency to timeout (or deadlock) so failing fast isn't a great idea, but a
timeout is. Other variables can also be set in the ~matrix~ as needed. Caching
is always important; again, for ~python~ projects one might use:
...
With these general niceties out of the way, we can continue with the basic
requirements.

** Workflow requirements
Will we use stages here, to make sure that each step does what it needs to do.
Essentially we have:
- Building
- Testing
- Triggering upstream / downstream
  + The base library triggers the dependents, and the dependents should have a
    workflow to trigger with the latest upstream as well
- Commenting with the results
  + Minimally on the PR (this should be by triggering another workflow if it succeeds)
  + Ideally would be nice to semi-automatically open an issue as well

Repositories which *depend* on an upstream repository (e.g. bindings) should be
triggered via a user specified comment workflow, since it may not be necessary
to run on each push to the upstream project, and for this, [[https://github.com/drewwyatt/comment-pipeline][comment-pipeline]]
seems useful. Other approaches are using [[https://github.com/marketplace/actions/repository-dispatch][a repository dispatch]], or [[https://github.com/imjohnbo/ok-to-test][ok-to-test]] or
[[https://github.com/peter-evans/slash-command-dispatch][slash-command-dispatch]].

** 0th attempts: Manual checkout and ~cron~
A valid first approximation is to simply checkout the upstream libraries and run
what tests are required for integration with the downstream component in the
repository. This has a few advantages:
- No token / authorization issues for public repositories
- First class support for ensuring all tests show up in the Github UI

However, it doesn't scale very well, and if the number of dependent libraries
grows, it can be rather vexing. There may be workflows which demand secrets.
Additionally, failing a workflow on a repo isn't the best way to signal to the
upstream developers that something might be wrong and their input is needed.
Nevertheless, this first approximation looks (for ~asv~ and ~asv_runner~, [[https://github.com/airspeed-velocity/asv_runner/pull/24/files][latest
...
Taking solace in some sysadmin tools, we can try to decouple the testing
apparatus further with a ~cron~ job.  If a library depends on another, the
simplest would be to run a ~cron~ job every day which manually checks out the
dependent to see if anything broke. This can be useful for projects with many
changes / PRs / rapid development, but also harder to pin down since each change
does not trigger a build. This is the ~dependabot~ style, which itself is
irksome enough to not be an actual role model.

** Triggering workflows
We will handle the somewhat simpler case first, of an upstream library
triggering CI runs on dependent projects, in this case, ~asv_runner~ (core
functionality) and ~asv~ (user-facing library). Here we can simply use the
[[https://github.com/marketplace/actions/trigger-workflow-and-wait][trigger-workflow-and-wait]] action.
*** Triggered
The only things to keep in mind other than the standard workflow rules are:
- Enabling input handling via ~workflow_dispatch~
- Providing the ~git~ ref via the ~client_payload~

This makes the triggered workflow rather simplistic (in the upstream repository,
as for example [[https://github.com/airspeed-velocity/asv/blob/master/.github/workflows/triggered.yml][in asv]]):
#+begin_src yaml
name: Triggered
...
jobs:
  ASVRunner:
    ...
    steps:
      ...
      - name: Run tests with the current asv_runner
        run: ASV_RUNNER="$(pwd)/latest_asv_runner" python -m pytest -v --timeout=300 --durations=100 test
#+end_src

Where the only notable snippet is that the repository is cloned and checked out
to the PR which in turn needs to be passed by the triggering workflow.

*** Trigger Dispatch

Out of pure malice, consider the ideal solution in an ideal world:
    ...
    steps:
      ...
      - uses: convictional/trigger-workflow-and-wait@v1.6.5
        with:
          ...
          repo: asv
          github_token: ghp_blah_mytokeninplaintext
...
However, this, it turns out is less than ideal for security reasons random forks
and pull requests should not and must not have access to repository secrets. It
is difficult to test this on public repositories, since secrets are not directly
shared on pull requests [fn:whatwhynosecrets]. Working around this is a bit
tricky, and another level of indirection is required.

** Tokens and Access
For simplicity, the idea is to use the ~gh~ client token and ~gh~ itself to
trigger workflows. Scoping rules in GitHub and security considerations being
what they are, the focus will be on minimal permissions, and we will first
consider generating a [[https://github.com/settings/tokens][classic personal access token]]. I used the workflow access
scope.
...
[fn:creditmelissa] Brought to my attention at work by the always excellent [[https://melissawm.github.io/about-me/][Dr. Melissa Weber Mendonça]].
[fn:whynotnumpy] So more akin to my doctoral work over at [[https://github.com/TheochemUI][Theochem]] (~potlib~, ~pypotlib~) or [[https://github.com/atomic-solvers][atomic-solvers]] than NumPy / SciPy at the moment.
[fn:whatwhynosecrets] Security reasons, as noted in the [[https://docs.github.com/en/actions/security-guides/encrypted-secrets#using-encrypted-secrets-in-a-workflow][official documentation]] and this [[https://github.com/orgs/community/discussions/33683][community discussion]]
* DONE Writing ~R~ Packages :@notes:r:cpp:bindings:packaging:
CLOSED: [2023-11-10 Fri 18:50]
:PROPERTIES:
:EXPORT_FILE_NAME: writing-r-packages
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
...
plotting systems. These notes arose while binding [[https://github.com/HaoZeke/readCon][readCon]] and [[https://github.com/TheochemUI/potlib][potlib]] for
[[https://github.com/HaoZeke/cuh2vizR][cuh2vizR]]. However, for best practices, my [[https://github.com/ropensci/fastMatMR][fastMatMR]] project should be consulted.
** Idiosyncracies
...
*Warning*
#+end_alert-heading
CRAN doesn't like fixed dependencies, see [[https://github.com/ropensci-review-tools/pkgcheck/issues/149][the discussion here]], and often for
...
#+end_alert
We need to install this with a non-CRAN [[https://docs.ropensci.org/pkgcheck/][source]]:
#+begin_src R
options (repos = c (
...
actually already part of the set of [[https://github.com/r-lib/actions/tree/v2/examples][Github Actions]] supported by ~usethis::~. I
ended up with:
#+begin_src R
...
however, it isn't exactly part of ~R-tools~[fn:yetatleast].
** Appeasing ~pkgcheck~
Some thoughts:
- No ~renv~ in ~.Rprofile~
...
#+begin_src R
covr::package_coverage(line_exclusions=file.path(".pixi", list.files(path=".pixi", recursi
    ve=T)))
#+end_src
...
It is also important, for benchmarking and other long running vignettes, to have
them cached or precomputed, as detailed in [[https://www.kloppenborg.ca/2021/06/long-running-vignettes/][this blog post]] which in turn
references an [[https://ropensci.org/blog/2019/12/08/precompute-vignettes/][older ROpenSci post]]. Vignette images are a whole other beast,
nicely [[https://www.jumpingrivers.com/blog/knitr-rmarkdown-image-size/][covered here]].

** Conclusions
These were my rough notes while developing guidelines during the first release
of [[https://github.com/ropensci/fastMatMR][fastMatMR]]. There were additional changes to my packages, mostly in terms of
...
[fn:yetatleast] Yet.
* TODO Revisiting ~R~ package development :@notes:r:cpp:bindings:packaging:
:PROPERTIES:
:EXPORT_FILE_NAME: revisit-r-pkgdev
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
Handling development dependencies and bugfixes
#+end_quote
** Background
I was an opportunity scholar at posit::conf 2024 which I took as an opportunity
to save ~fastMatMR~ on being [[https://github.com/ropensci/fastMatMR/issues/65][removed from CRAN]] due to its ~cpp11~ dependency.
Most of this is going to be in the context of the previous post.
** Reproducing environments
With ~pixi~ this is fairly straightforward:

#+begin_src bash
gh repo clone ropensci/fastMatMR
cd fastMatMR
gh repo fork
pixi shell
pixi update
git commit -m "BLD: Update pixi lockfile"
#+end_src

That sets up the basic ~radian~ shell we can use for the rest of this post. The
new standard for handling package dependencies is no longer
~devtools::install_*~ or ~remotes::~, but ~pak~, as seen in [[https://github.com/r-lib/remotes/issues/808][this issue]]. In turn,
this requires an adjustment to the [[https://rstudio.github.io/renv/reference/config.html?q=pak#renv-config-pak-enabled][renv configuration]], which in turn, is best handled through a repo-local ~.Renviron~ file [fn:rconfigs] with:

#+begin_src sh
RENV_CONFIG_PAK_ENABLED=TRUE
#+end_src

** The case against reproductions
To quote (and mildly paraphrase) Hadley Wickham [fn:whensaid],
#+begin_quote
.. part of being a package developer is experiencing this (broken dependencies) discomfort before your users
#+end_quote

Which is also echoed by many others in the R community

[fn:rconfigs] Configuration files [[https://support.posit.co/hc/en-us/articles/360047157094-Managing-R-with-Rprofile-Renviron-Rprofile-site-Renviron-site-rsession-conf-and-repos-conf][discussed here]]
[fn:whensaid] In person walk and talk at posit::conf 2024

* TODO ~ROpenSci~ submissions and ~CRAN~
#+begin_quote
Where I discuss my experience with the review process and subsequent CRAN release
#+end_quote
...
** Appeasing ~CRAN~
I used the ~devtools~ workflow discussed in the [[https://r-pkgs.org/release.html][R pkgs book chapter]], along with
extra checks [[https://www.mzes.uni-mannheim.de/socialsciencedatalab/article/r-package/][from this post from Method Bites]] for finalizing the package for
~CRAN~. Essentially, as eventually

* TODO Non-contigous arrays and F2PY
Recently it was noted on the [[https://fortran-lang.discourse.group/t/the-difficulties-of-using-fortran-in-scipy/6753/41?u=rgoswami][Fortran-Lang discourse]] that passing non-contiguous
arrays from NumPy to Fortran is currently slow. However, via careful handling of
~CFI_extent~, discontinous arrays *copy-free* traversal is indeed possible.

* TODO Reproducible ~R~ environments with ~nix-flakes~
:PROPERTIES:
:EXPORT_FILE_NAME: reproducible-r-nix-flakes
:EXPORT_HUGO_PANDOC_CITATIONS: t
:EXPORT_BIBLIOGRAPHY: biblio/Website.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("R x Nix Nexus: Crafting Cohesive Environments")
:END:
#+begin_quote
On constructing reproducible ~R~ environments with ~nix-flakes~, without ~RStudio~
#+end_quote
** Background
I've written about replacing ~jupyter~ with ~orgmode~ {{{relref(in the past,jupyter-orgmode)}}}, and that
workflow scales naturally to the case of writing ~R~ code with ~polymode~ and
~nix~ as [[https://rgoswami.me/posts/emacs-nix-r/][discussed earlier]] and demonstrated in [[https://rgoswami.me/posts/sr2-ch13-ch14/][several other posts]].
** Alternates
*** Why not ~conda~?
The ~conda~ ecosystem is pretty good for quick projects, and I have been using
it (via ~micromamba~) quite a lot recently [fn:wherelse]. Indeed, the recent
[[https://prefix.dev/][pixi]] too improves on the existing user interface as well. However, in-spite of
this, the manner in which ~conda~ interacts with compiled packages, via the
~compilers~ meta-package, (especially for ~R~) make it difficult to manage,
especially when loading or using multiple ~devtools~ based packages. There are
some good resources for both running ~RStudio~ @sturmRunningRstudioServer2023
and managing ~R~ within ~conda~ @ManagingEnvironmentsUsing2022.
*** Why not ~docker~?
- Security (though there's ~singularity~ too)
- Speed
- Sanity
That being said ~docker~ is perfectly fine for shipping a whole system alongside
some scripts. Not exactly lightweight though. Personally, I prefer to use
~docker~ only when I specifically need to swap operating systems (e.g. grabbing
~python-dbg~ as [[https://rgoswami.me/snippets/docker-dev-envs/][discussed here]]).
*** What about ~spack~ or ~easybuild~?
**Building** the world from scratch for each new project (e.g. [[https://rgoswami.me/posts/spack-pytorch-dev-workflow/][for PyTorch]]) is
only really feasible for high performance computing environments, where I do
prefer ~spack~ to any of the other alternatives discussed. For personal projects
meant to be run on different laptops though, this is not recommended.
** Flakes 101
There are far too many resources covering ~nix-flakes~ [fn:howtoflake].
** Conclusions
More of an introduction to ~nix-flakes~ than anything else, this was long
overdue, given my association with the Summer of Nix and other ~nix~ community
efforts. Flakes can seem a little, flakey to say the least, but are a robust
option for (approximately) reproducible [fn:whyapprox] personal projects.

[fn:reallystillexp] They're **still** experimental though
[fn:howtoflake] Some introductory options
[fn:wherelse] Much of the Scientific Python ecosystem relies on ~conda~ for CI services too
[fn:whyapprox] Some members of the [[https://reproducible.nixos.org/][community]] [[https://github.com/NorfairKing/nix-does-not-guarantee-reproducibility][disagree]] with the assertion of reproducible ~nix~
* TODO Pure ~python~ projects with ~pdm~ :@notes:python:packaging:
:PROPERTIES:
:EXPORT_FILE_NAME: pure-python-proj-hatch
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Python packaging")
:END:
#+begin_quote
Ruminations on packaging and distributing *pure* ~python~ projects.
#+end_quote
** Background
There are too many competing approaches to packaging in the ~python~ ecosystem.
Sometimes, typically for simple parse and plot packages, it makes sense to
remain relatively pure[fn:whyrela]. This is an opinionated starter guide geared
towards ~pdm~ and ~hatch~ usage.

When specifics are required, they will be in the context of the [[https://github.com/HaoZeke/chemparseplot][chemparseplot package]].
** Initialization
Typically we still need a way to lock down the ~python~ version. For this I
generally end up with ~pixi~ now. Even though we'd like to test a range of
versions later, as a baseline it makes sense to start with a reasonably recent
version[fn:but312].

#+begin_src bash
pixi init -c conda-forge
pixi add python>=3.10 pip hatch pdm
pixi shell
#+end_src

That's all we need from ~pixi~. Lets start with the rest of the setup.

#+begin_src bash
hatch new chemparseplot
cd chemparseplot
mv * ../
rm -rf chemparseplot
#+end_src

Personally I'm not super fond of the ~src~ layout, but I'd rather not quibble
with [[https://hatch.pypa.io/latest/config/project-templates/#src-layout][the defaults]]. That being said, it is a better idea to move away from the
default versioning method and use [[https://github.com/ofek/hatch-vcs][hatch-vcs]] instead. Also it is important to setup the ~include~ keys:
...
At this point I ended up migrating from ~hatch~ to ~pdm~ though with ~pdm~ as
the mangement system (mainly since ~hatch~ doesn't have easy dependency
management). For example:

#+begin_src bash
pdm add -dG test pytest "coverage[toml]"
#+end_src

This was worth the switch for me.

[fn:whyrela] Relative here in the sense that dependencies like ~numpy~ and
friends are always going to bring in compiled extensions...
[fn:but312] *3.12* with compiled extensions is a sticky choice because of the
~distutils~ deprecation, but we're relatively pure so it should be fine..
* TODO General ~CPP~ project management :@notes:cpp:packaging:
:PROPERTIES:
:EXPORT_FILE_NAME: cpp-meson-proj-manage
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("C++ packaging")
:END:
#+begin_quote
Ruminations on packaging and distributing ~C++~ projects.
#+end_quote
** Background
Something about academic work leads to multiple tiny C++ libraries. These
generally have pretty similar project management techniques.
** Guidelines
Some things which should be a part of every repository:
- LICENSE :: I now prefer the ~MIT~ license, via ~licensor~
- Readme :: This needs to be present, either generated from an ~orgmode~ source or directly in markdown / REsT.
*** Pre-commit hooks
As [[https://rgoswami.me/snippets/local-precommit-dependencies/][noted elsewhere]], local pre-commit hooks are superior to hosted ones. For ~CPP~ projects I generally have:
#+begin_src yaml
repos:
- repo: https://github.com/pre-commit/pre-commit-hooks
  rev: v4.4.0
...
*** ~orgmode~ sources
To use ~orgmode~
*** API Docs with ~doxyYoda~
I use ~doxygen~ (version ~1.9.1~) with the [[https://github.com/HaoZeke/doxyYoda][doxyYoda theme]].
#+begin_src bash
mkdir -p doc/apidocs
cd doc/apidocs
doxygen -g Doxyfile.cfg
#+end_src
This needs to be coupled with a minimal project specific configuration file:
#+begin_src conf
# Doxyfile-proj.cfg
@INCLUDE                = "doc/apidocs/Doxyfile.cfg"
# XML
GENERATE_XML            = NO
XML_PROGRAMLISTING      = NO
...
RECURSIVE              = YES
EXTRACT_ALL            = YES
INPUT                  = doc/index.md CppCore images/
EXCLUDE                = CppCore/tests
...
GENERATE_HTML          = YES
USE_MATHJAX            = YES
HTML_HEADER            = "doc/apidocs/doxyYoda/html/header.html"
HTML_FOOTER            = "doc/apidocs/doxyYoda/html/footer.html"
HTML_EXTRA_STYLESHEET  = "doc/apidocs/doxyYoda/css/doxyYoda.min.css"
LAYOUT_FILE            = "doc/apidocs/doxyYoda/xml/doxyYoda.xml"

# Extract everything
EXTRACT_ALL            = YES
#+end_src
Also the theme is easiest used with a tagged version, so a small script is
usually a good idea, something like this:
#+begin_src bash
#!/bin/bash
...
#+end_src
**** CI Setup
To do this via Github actions, a basic configuration would be something like this:
#+begin_src yaml
...
#+end_src
Where blocks would need to be modified if for example, ~micromamba~ is not used
for package management.
** Release management
*** Changelog
I tend to use ~towncrier~ for this, with a fairly [[https://towncrier.readthedocs.io/en/stable/markdown.html][standard setup]]. The only
change is that in keeping with the other conventions, the ~changelog.d~ folder
lives in ~doc/~.
*** Version changes
For this, I use ~tbump~ with a specialized configuration.
** Conclusions
Actually most of these steps are pretty much applicable to projects in most
languages, with minor differences to the API documentation section. However, for
my own needs, it is nicer to have separate posts for separate languages.

* DONE Managing cloud based ~calibre~ libraries :@personal:workflow:tools:ramblings:books:
CLOSED: [2023-08-19 Sat 13:33]
:PROPERTIES:
:EXPORT_FILE_NAME: managing-cloud-based-calibre
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Modern reading")
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
...
#+end_quote
** Background
I have a set of different devices across which I need to sync my library
(described in [[https://rgoswami.me/posts/my-life-in-eink/][an earlier post]]). Ideally this should be a lazy synchronization
mechanism and be as general as possible.
** Constraints
...
I use [[https://www.pcloud.com/][pCloud]], which has a nifty GUI client (and CLI) across my operating systems
which essentially provides a mounted drive.
...
**** Cons
- This doesn't really sync immediately, and the delay might be pretty
  significant if earlier jobs are skipped
*** 3rd: Use ~rclone~ manually
~rsync~ is really meant for SSH connections, while [[https://rclone.org][Rclone]] is fantastic for
working with cloud providers. Setting this up is a breeze.
#+begin_src bash
rclone config
# Best to encrypt it
rclone sync -P /path/to/calibre/library remote-name:/path/in/pcloud
#+end_src
**** Pros
- Simple setup
**** Cons
...
machines have ~gnome-keyring~ installed, and ~rclone~ [[https://rclone.org/docs/#configuration-encryption][supports secrets]] stored in
password managers so:
#+begin_src bash
secret-tool store --label="rclone" service "rclone" user "calisync"
#+end_src

Which can then simply be set to ~RCLONE_PASSWORD_COMMAND~ and used directly:
#+begin_src bash
export RCLONE_PASSWORD_COMMAND='secret-tool lookup service "rclone" user "calisync"'
...
Type=Application
Name=CaliSync
Exec=/home/username/.local/bin/calibre_and_sync.sh
Icon=calibre-gui
GenericName=E-book library management with cloud sync
Comment=Start Calibre and sync library with pCloud
Terminal=false
Categories=Office;Utility;
#+end_src

Do keep in mind that environment variables are not expanded, so generalizing
this would require a suitable dotfile management system like ~chezmoi~ (as [[https://github.com/HaoZeke/Dotfiles/commit/7e6fbb192135fa43ea55638c5722ea4cd528b52d][seen
here]]).
*** Visual Tweaks
The script discussed in the previous section is fine, but it is often nicer to
have a visual indication of what's going on in terms of the upload, especially
...
I've dicussed working with {{{relref(Zotero before,my-life-in-eink)}}}, but when working with cloud synced
calibre libraries, to prevent duplication of files, relative linking of base
directory attachments works really well[fn:1].

#+DOWNLOADED: screenshot @ 2024-07-06 12:46:20
#+CAPTION: Zotero settings for linking files consistently
[[file:images/Integrating_with_~zotero~/2024-07-06_12-46-20_screenshot.png]]

Some reasons to do this:

- The main goal of this is to enable keeping notes per-book, which Calibre
  doesn't support
  + Also to get easy access without duplicated files on the backing WebDAV
    service (pCloud in my case)
- Allows easier, more natural ~bibtex~ handling in Zotero, without going through
  a sync plugin like [[https://forums.zotero.org/discussion/60898/calibre-zotero-metadata-importer-plug-in][ZMI]]
...
roller-coaster, especially since Calibre Companion was [[https://www.mobileread.com/forums/showthread.php?t=348027][sold off and abandoned]],
but for now, it works, as does [[https://calibresync.bitbucket.io/][Calibre Sync]] and [[http://koreader.rocks/][KoReader]]. That being said, the
best approach would be pairing this setup with something like [[https://foldersync.io/][FolderSync]] and
[[https://www.appbrain.com/app/leger-calibre/com.calibre.local][Leger Calibre]] would be the best but Leger Calibre has gone the [[https://www.mobileread.com/forums/showthread.php?t=267147&highlight=leger+calibre][way of the dodo]],
leaving one with precious little options, though [[http://koreader.rocks/][KoReader]] is still the best
...
[fn:1] As first [[https://blogs.princeton.edu/librarian/2013/05/organizing-my-research-life-updated/][noted here]] by Wayne Bivens-Tatum
* DONE Lowering resource usage with ~foot~ and ~systemd~ :@notes:workflow:tools:
CLOSED: [2024-06-02 Sun 15:00]
:PROPERTIES:
:EXPORT_FILE_NAME: lowering-resource-usage-foot-systemd
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Wayland Wayfinders")
:END:
#+begin_quote
Melding ~systemd~ and ~foot~ for rapid configuration updates and reduced memory
consumption in ~sway~ with persistent ~tmux~ state
#+end_quote
** Background
...
decided to fiddle around with custom ~systemd~ targets, since ~sway~ isn't
really good at managing long running processes like my terminal (~foot~) server.
** Starting point
Some years ago I ended up with a rather ad-hoc scratchpad configuration,
reproduced for clarity ([[https://github.com/HaoZeke/Dotfiles/tree/chezmoi/dot_config/sway][current config here]]):
#+begin_src conf
...
When everything works (e.g. at start-up) it essentially leads to a scratchpad
setup like so:

#+DOWNLOADED: screenshot @ 2024-06-02 14:10:26
[[file:images/Background/2024-06-02_14-10-26_screenshot.png]]
...
Which ideally starts a ~foot~ server, and subsequently spawns ~footclient~
instances on demand.

** Annoyances
This setup served me quite well, for several years I had no major issues
...
instances of ~foot~, one for each scratchpad.

#+DOWNLOADED: screenshot @ 2024-06-02 14:15:22
#+CAPTION: Snapshot of memory consumption (from ~btop~), clients take around 10x less memory
[[file:images/Background/2024-06-02_14-15-22_screenshot.png]]

On its own, given that I mostly use relatively high-end machines, this was not
really a problem, more an aesthetic annoyance.
*** Applying ~foot~ configurations
If the main (~sway~) server is restarted to apply a configuration update it will
...
theme via ~foot.ini~ ([[https://github.com/HaoZeke/Dotfiles/tree/chezmoi/dot_config/foot][crrent config here]]):
#+begin_src conf
# include = ~/.config/foot/foot.d/theme-acario-light.ini
include = ~/.config/foot/foot.d/theme-solarized-dark-normal-brights.ini
#+end_src
Would entail a whole ~killall -9 foot~ song and dance which was also rather
annoying, especially under the current configuration, the ~tmux~ sessions would
also die with the ~foot~ instances.
*** ~exec_always~ doesn't restart failed processes
Nor should it, as the [[https://man.archlinux.org/man/sway.5.en][documentation explicitly mentions]], it is basically like
~exec~ but it also gets re-executed after a reload. For something like a
...
configuration (and redo the ~tmux~ state) just because aesthetic changes needed
...
lifetime to ~systemd~. User configurations for ~systemd~ live in
~$HOME/.config/systemd/user~ and require some standard commands for interacting
with them:
#+begin_src bash
...
WantedBy=default.target
#+end_src
With much more extensive documentation found in the ~man~ pages (or [[https://www.freedesktop.org/software/systemd/man/latest/systemd.service.html][online, here]]).
*** Terminal Server
The ordering of the services are also fairly obvious, both the scratchpads (and
indeed, the sway terminals) need one primary ~foot-server~ instance to be
running at all times. The easiest approach is to reuse ~XDG_RUNTIME_DIR~ which
is typically the same as ~/run/user/$(id -u $USER)/~, so the server runs via:
#+begin_src bash
/usr/bin/foot --server=/run/user/%U/foot-server.sock
#+end_src
Since ~foot -s~ doesn't fork itself, the default ~Type=simple~ will suffice.

Additionally, we will need to ensure that all the ~footclient~ instances we call
use the same server socket (as shown in [[Sway Scratchpads and Config][Sway Scratchpads and Config]]).
*** Scratchpad clients
The clients themselves are equally simple, except that the ~[UNIT]~ stanza will
specify they require ~foot-server.service~ and will be run after
~foot-server.service~.
#+begin_src bash
...
*** ~tmux~ sessions
Additionally, since the goal is to reload ~foot~ often via simple ~killall -9
foot~ to apply new configurations, it makes sense to add a ~tmux~ server into
...
- The server will be a ~forking~ service, since ~tmux start-server~ essentially
  puts itself in the background.
- The clients should be ~oneshot~, since they should complete before other
  dependent services start[fn:2]
- We need to set remain after exit since we have a "rollback" for our service
  (~ExecStop~) and since we modify system state, i.e. we don't want to re-run
  the service actions without stopping[fn:3]
*** False starts
I thought I wanted to use sockets ([[https://www.freedesktop.org/software/systemd/man/latest/systemd.socket.html][online man-page]]), but for a constantly
...
** Complete configurations[fn:1]
#+DOWNLOADED: screenshot @ 2024-06-02 14:38:48
#+CAPTION: Final single server many client view (via ~btop~)
[[file:images/Conclusions/2024-06-02_14-38-48_screenshot.png]]
...
reach for ~tmux~, for me this works out very nicely. It also helps handling
configuration updates to ~foot~, since all instances use the same server.
...
persistent ~tmux~ services is likely to be useful in any case.

Nevertheless, the minor annoyances have been quelled, and  I had never really
worked with ~systemd~ services before barring a few backup setups, so this was
pretty gratifying.

[fn:1] for now, updates will probably appear only on [[https://github.com/HaoZeke/Dotfiles/tree/chezmoi/][my Dotfiles repo]]
[fn:2] There is an excellent visual explanation of the differences b/w [[info:][simple and oneshot types here]] by Thomas Stringer
[fn:3] Aside from the [[https://www.freedesktop.org/software/systemd/man/latest/systemd.service.html][official documentation]], [[https://stackoverflow.com/a/38377069/1895378][this SO answer]] is quite nice
* DONE Talk Supplements for Scientific Computing in Rust 2024 :@conferences:presentations:rust:
CLOSED:         [2024-07-21 Sun 23:57]
:PROPERTIES:
:EXPORT_FILE_NAME: scicomprs-2024-meta
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments false
:END:
#+BEGIN_QUOTE
...
- ~bless~ :: [[https://github.com/HaoZeke/bless][repo]] (Github)
- Workshop listing :: [[https://scientificcomputing.rs/2024/talks/goswami.html][conference site]]
Blurb:
#+begin_quote
...
- A ~pdf~ copy of the slides are embedded below
#+BEGIN_EXPORT html
<script defer class="speakerdeck-embed" data-id="ea09a267822146a6af7cb9989fdac577" data-ratio="1.7772511848341233" src="//speakerdeck.com/assets/embed.js"></script>
#+END_EXPORT
** Video

#+html: {{< youtube H58LOUDPpaU >}}

* TODO Packaging practices for Rust
** Background
Recently spoke about ~bless~, which had the distinction of being my first Rust
package.
** Tools
My general release tools are:
- towncrier :: For keeping a changelog
- tbump :: For tracking versions

Along with some ~rust~ specific tools
- oranda :: For handling the static site
- cargo-dist :: For distribution on crates.io
* TODO HPC Usage Tricks
** Background
Highly personalized set of tasks and workflows I often reach for.
** Persist with ~tmux~
The idea is to have a ready-to-use persistent setup on your login node. This is
useful.
** Job management
*** ~tmux~ permissions
Occasionally the following may manifest.
#+begin_src bash
tmux new -s "blah"
# Some error about a missing directory
mkdir $WHATEVER_MISSING
# Later
tmux attach
directory /tmp/tmux-20255 has unsafe permissions
#+end_src
This is a sign of poor system management. Report it to your admins and also
circumvent it via:
#+begin_src bash
chmod -R 700 /tmp/tmux-20255
#+end_src
Since it is in ~/tmp/~ it should be writable.
** Interactivity
*** Analysis
I tend to have a simple ~conda~ based setup for this.
#+begin_src bash
micromamba create -n do_jup -c conda-forge
micromamba activate do_jup
micromamba install jupytext jupyterlab
#+end_src
Ideally of course, this should be project scoped, and handled in a ~pixi~
configuration perhaps, however, without going into the "system wide" ~pixi
global~ setup and still having a modicum of dependency management is nice.
* TODO A gentle introduction to high throughput calculations :@notes:hpc:ramblings:chem:
:PROPERTIES:
:EXPORT_FILE_NAME: intro-highthrough-calc
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
HPC and MongoDB for fault tolerant calculations
#+end_quote
** Background
*** What
[[https://materialsproject.github.io/fireworks/][Fireworks]], from the Materials Project[fn:whatelse] is the main setup target for
this post. To be precise, setting up a [[https://www.mongodb.com/][MongoDB instance]] for running [[https://materialsproject.github.io/jobflow/tutorials/8-fireworks.html][Jobflow]] in
a couple of ways to use HPC resources in the best possible way while incurring
the least possible sysadmin ire.
*** Why
For most of my doctoral research, I don't run large systems, or even many
systems. Method development rarely requires such things (until production). For
when I have to, I've found various ways to circumvent[fn:wailordplug] the
requirement of actually writing bash scripts by hand for hundreds of systems
with soul crushingly similar, but completely arbitrarily presented parameters in
"free-form" random input data files[fn:seriously] which are the /lingua franca/
of computational chemistry codes[fn:notallcodes].

Concurrently, computational high-throughput "materials science" has become a
thing, with the admirable goal of "running things" until suitable properties are
found[fn:isthatall]. This pragmatic approach led to more effective compute
intensive approach, which is none-the-less useful to have "data" to discuss
"chemistry".
*** TL;DR
This is a post about setting up workflow management systems in a traditional HPC
environment.
** Baseline
The plan is to start out locally with ~jobflow~[fn:basejobflow], then configure
a MongoDB instance (still local), the finally export to an HPC stack. We will
use a chemical system as a reference job, but the workflow could be adapted to
any domain.
*** System setup
**** Computational
This will vary a bit from stage to stage. For starters, a ~pixi~ environment to
handle [[https://prefix.dev/docs/pixi/overview][conda dependencies]] will suffice.
#+begin_src bash
mkdir -p $HOME/Git/Github/jflow_mongodb && cd "$_"
pixi init -c conda-forge
pixi add mongodb mongo-tools
#+end_src
Weirdly ~mongodb~ and ~mongo-tools~ don't include ~mongosh~, the shell client. So:
#+begin_src bash
pixi add nodejs
npx mongosh
#+end_src
**** Chemical
The job which will be used as a reference is a dirt simple scan of energies with
respect to the distance between atoms of a system consisting of a single H2
molecule fig:kolos. Some other details:
- ORCA will be used as the calculator
  + To change various potentials / basis sets etc.
- It shall be run as an executable

The last point might be a bit off, since we created the initial configuration
using ~ASE~, and ~jobflow~ is a Python library, when the stars align and the
stack is in Python it is often best to use it throughout. Given the distribution
of quantum chemistry codes though[fn:howdist], and in order to be more general,
we will work with executables.

#+name: fig:kolos
#+caption: Sample scan over H2 bond for various potentials generated with ~wailord~
[[file:images/throughput_chem/plotH2B1plt.png]]

To get this setup, assuming ORCA binaries are available on ~$PATH~ or its equivalents:
#+begin_src bash
pixi add ase ipython pip
#+end_src

We can construct general molecules in ~ase~ pretty easily.

#+begin_src python
from ase.build import molecule
h2 = molecule('H2')
h2.write('h2_base.xyz', comment="\n")
#+end_src

Or in one shot:

#+begin_src bash
python -c "from ase.build import molecule; h2 = molecule('H2'); h2.write('h2_base.xyz', comment='\n')"
#+end_src

Note that the metadata added by `ase` isn't compatible with ORCA, hence the
newline comment. Now we just need an ORCA input file:
#+begin_src yaml
!OPT UHF def2-SVP
%geom Scan
...
This slightly confusing notation is because ORCA will only scan in a geometry
optimization problem, and one will need to create and run mind-numbingly similar
files for each energy function or basis set.

Running this is fairly simple:
#+begin_src bash
...
Where we note that we redirect both stderr and stdout to a file and
simultaneously to screen, because:
- ~ORCA~ writes a lot of files, *~78* in this small example
  + The most important information including number of
  iterations, times taken, scan results, etc are only written to ~stdout~
- The ~which orca~ thing is for parallel runs, but not a bad best practice
- ~ORCA~ really writes a lot, so it makes sense to make a folder for this

Also, since this is plain text, it makes sense to compress it with ~gzip~, since
that is also natively supported by Python.
...
#+begin_src bash
du ../uhf/scan_uhf scan_uhf.gz
1396	../uhf/scan_uhf
108	scan_uhf.gz
#+end_src

Further discussion of the intricacies of working with ORCA would take us again
too far afield, but it is worth noting a tiny script to parse the output and
eventually generate the plot[fn:alsowailord].

#+begin_src python

#+end_src

That's quite a lot of *ugly* scripting before we can even get one set of data
points. In practice, not all codes will even support scans, in which case we'd
need some machinery (preferably not a grad student) to run a calculation for
each data point, keep track of them, and then generate the plot.

***** ~jobflow~ setup
Some light refactoring of our existing setup is required to cast the problem
into a ~jobflow~ compatible format.
# **** Chemical alternate via ~wailord~
# The example being considered, i.e. using ~ORCA~ for a host of energy calculations has an explicit solution already by
# using [[https://wailord.xyz][wailord]], which will setup the entire set of runs, and parse their outputs
# neatly into a ~pandas~ dataframe.

# #+begin_src bash
# # say, in ~/Git/Github
# git clone git@github.com:HaoZeke/wailord
# cd wailord
# git checkout nextRel
# pixi run pip install .
# #+end_src

# Now we can

# This is clearly the best option for the problem at hand. However, for workflows
# not covered by ~wailord~ e.g. other HPC codes, we will continue to explore Fireworks.
** Local ~MongoDB~ Instances
For starters we will just setup a basic instance.
#+begin_src bash
pixi shell
mkdir -p $(pwd)/data/database
...
Annoyingly there's a warning raised about the Nodejs version, but that doesn't
matter for now. We will run through some basic steps:
#+begin_src js
❯ npx mongosh
Current Mongosh Log ID:	6564daef4aec767d07c9a9f5
...
#+begin_src js
test> use science
switched to db science
science> db.chem
science.chem
#+end_src
Adding entries (and [[https://www.mongodbtutorial.org/mongodb-crud/mongodb-deletemany/][bulk insertions]]):
#+begin_src js
science> topic = {"title": "Physical chemistry", "power_level": 3000}
{ title: 'Physical chemistry', power_level: 3000 }
science> db.chem.insertOne(topic)
{
  acknowledged: true,
  insertedId: ObjectId('6564db4c4aec767d07c9a9f6')
}
#+end_src
Finding things:
#+begin_src js
science> db.chem.find().pretty()
[
  {
    _id: ObjectId('6564db4c4aec767d07c9a9f6'),
    title: 'Physical chemistry',
    power_level: 3000
  }
]
#+end_src
Along with deletion (with [[https://www.mongodbtutorial.org/mongodb-crud/mongodb-deletemany/][a filter]]):
#+begin_src js
db.chem.deleteMany({ "title": /chemistry/i })
#+end_src
The key concept here is that the entries can be queried, and they pick up their
own ~_id~ variables. This alone will be most of the benefit we need from a
workflow management system.
** Fireworks
Now we can follow along with the basic tutorial:
#+begin_src bash
pixi add Fireworks
gh repo clone materialsproject/fireworks /tmp/fire
mv /tmp/fire/fw_tutorials .
rm -rf /tmp/fire
#+end_src

I find it easier to work in a ~tmux~ session for these situations. In any case,
start the MongoDB server in one terminal via ~mongod --dbpath
$(pwd)/data/database~ and in a new terminal:
...
** Putting ~ORCA~ runs together
Now to actually use these workflow management tools effectively, quite a bit of
boilerplate is needed[fn:seeatomate]. We need something similar.

** Conclusions
The MongoDB setup, and general workflow management idea works great. It will
doubtless usher in a revolution of reduced calculations, if only more people
could be compelled to use it[fn:perhapscoursework]. I would recommend it
wholeheartedly as a user, sysadmin, reviewer and under all my other academic
hats. It seems also, that hosted MongoDB instances are good enough in terms of
bandwidth and other configuration settings for the HPC calculations Fireworks is
typically used for[fn:howcomemongofree]. Will cover scaling up via hosted Atlas and VPS instances along with HPC interaction in another post.
*** Opinions
With this setup, the actual work can now continue uninterupted by stray qualms
like "but what if you turn off parameter X in system Y because domain
knowledge". Just run them all, and let the data speak for itself[fn:butchem].
Without covering the ethics of "science per watt" as demanded by most HPC
systems [fn:wtfsciencepwatt], it is useful to be able to generate reproducible
runs for production systems. Far better than hand-rolling them. Just, be mindful
of an oft forgotten fact from the preface of
@schaeferQuantumChemistryDevelopment2004:

#+begin_quote
Running a few molecular orbital calculations doesn't make one an electronic
structure theorist any more than sleeping in one's garage makes him/her an
automobile.
#+end_quote

Naturally this is applicable across all domains.

[fn:notallcodes] Most of them anyway, e.g. [[https://sites.google.com/site/orcainputlibrary/general-input?authuser=0][ORCA]], [[https://www.vasp.at/wiki/index.php/Category:Input_files][VASP]], [[https://manual.q-chem.com/5.0/sect0010.html][QChem]], [[https://www.scm.com/doc/AMS/Input_Output.html][AMS]].. Better designs like [[https://www.bigdft.org/][BigDFT]]'s ~yaml~ inputs or even [[https://theory.cm.utexas.edu/code/eon/docs/_build/html/documentation.html][EON's INI]] are rare.
[fn:wailordplug] e.g. [[https://wailord.xyz/][Wailord]] ([[https://conference.scipy.org/proceedings/scipy2022/pdfs/rohit_goswami_wailord.pdf][SciPyCon22 paper]]) for ORCA.
[fn:seriously] I could go on about this sadsack state of affairs for hours but that would detract from the point, there are also some efforts to fix this, e.g. ~wailord~ or ~cclib~ or I guess ~ase~ (different goals though).
[fn:isthatall] Yes that's very reductive.
[fn:whatelse] Also home to more quantum chem scoped job management systems like [[https://www.aiida.net/][AiiDA]].
[fn:basejobflow] Familiarity with ~jobflow~ [[https://materialsproject.github.io/jobflow/tutorials.html][tutorials]] is recommended, especially the [[https://materialsproject.github.io/jobflow/tutorials/1-quickstart.html][quickstart]].
[fn:perhapscoursework] I suggest homework. Mandatory homework. Across the whole range of academics.
[fn:howcomemongofree] It is perhaps not surprising that the MongoDB free tiers geared for actual database operations are more than enough to cover these runs.
[fn:wtfsciencepwatt] A term I was introduced to at a [[https://www.lumi-supercomputer.eu/][LUMI workshop]].
[fn:howdist] Binaries. Large, blobby, statically linked binaries.
[fn:butchem] Of course one should ideally understand exactly what's going on and tune parameters for each system...
[fn:alsowailord] As noted, many of these problems are alleviated by ~wailord~
[fn:seeatomate] e.g. The ~atomate~ project is essentially a VASP compatibility layer
* TODO VPS Baselines and MongoDB
** Background
The key reason to decouple a MongoDB instance from existing HPC architecture is
to be more robust to changes in clusters. Also, it makes it easier to run on
multiple HPC centers without repeating the setup steps.
* DRAFT Saner conda-forge interactions with a custom forge
:PROPERTIES:
:EXPORT_FILE_NAME: sane-custom-forge
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
** Background
I recently handled releasing a ton of packages to ~conda-forge~ and realized
that the ~docker~ setup didn't work very well for me.
* DRAFT Easier ~TeX~ preambles with two-phase ~org~ exports
e.g. as done for my CV
* DRAFT Weird academic polishing opportunities
:PROPERTIES:
:EXPORT_FILE_NAME: weird-academia
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:

e.g. Mendeley profiles? XD Academia.edu? Kudos? whatever..?

* DONE ~xonsh~ Excursions :@notes:ramblings:tools:shell:
CLOSED: [2024-07-22 Mon 00:09]
:PROPERTIES:
:EXPORT_FILE_NAME: xonsh-excursion
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
** Background
...
[[https://xon.sh/][Python based xonsh]].
#+begin_src bash
pipx install xonsh[full]
pipx inject xonsh xcontrib-sh
echo 'xontrib load sh' >> ~/.xonshrc
#+end_src
Where we use the [[https://pipx.pypa.io/latest/docs/#pipx-inject][injection mechanism]] to add the [[https://github.com/anki-code/xontrib-sh][xcontrib-sh]] plugin for running
shell commands without using ~xonsh~, since we might not want to [[https://xon.sh/bash_to_xsh.html][translate]] every
command to be compliant with ~xonsh~.
*** Additions
- ~xonsh-mode~ works well with ~emacs~ and is on MELPA
- [[https://github.com/xxh/xxh][xxh]] makes it pretty trivial to take into foreign SSH machines
...
With [[https://sh.readthedocs.io/en/latest/index.html][sh]], shell calls become more ergonomic, however, it is still easier to
...
[fn:1] Like those mentioned in [[https://itsfoss.com/shells-linux/][this list]]
[fn:2] True of all [[https://chriswarrick.com/blog/2023/01/15/how-to-improve-python-packaging/][python packages]]
* TODO Dotfile encryption and secrets with ~chezmoi~ and Enpass :@personal:workflow:tools:
#+begin_quote
Ruminations on secrets within shared configurations
#+end_quote
** Background
A lot of configuration files contain "secrets", but are also generic enough to
not warrant file-level encryption for the whole configuration. An easy example
is the following:
#+begin_src toml
[General]
    optionA = solA
    # .... lots more, would ideally be shared
[Secrets]
    user = "Not secret but will change"
    pass = "VerySecret"
#+end_src
~chezmoi~ has enough fine grained integrations with password management tools to make it feasible to skip block encryption.
** Tools
I personally [[https://enpass.io/][use Enpass]] for handling my passwords.
Also ~pass~.
* DRAFT Encrypting Thunderbird Profiles for PCloud :@personal:tools:email:workflow:
#+begin_quote

#+end_quote
** Background
I still believe Astroid is the best way to handle email. However, I also often
switch operating systems and machines, so it is best to have a cross-platform
tool to handle my email as well. Despite being stripped of XUL, Thunderbird is
still marginally useful.
** Encryption
Cryptomator is an easy way to have dynamically sized encrypted data.

* DONE Storing data with DVC and OneDrive :@personal:tools:workflow:repro:science:wip:
CLOSED: [2025-08-06 Wed 16:27]
:PROPERTIES:
:EXPORT_FILE_NAME: dvc-onedrive
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Reproducible research")
:END:
#+begin_quote
...
[fn:: Also discussed in [[http://rgoswami.me/posts/managing-cloud-based-calibre/][this post on managing cloud based calibre libraries]]] and
~dvc~ can be made to be of more use.
** Baseline
Like most of my newer projects, ~pixi~ is used to manage the tooling required,
along with ~git-subrepo~ for handling local changes to projects.

#+begin_src bash
pixi add dvc rclone
#+end_src

As before, we will assume ~gnome-keyring~ is present, and then since ~rclone~
[[https://rclone.org/docs/#configuration-encryption][supports secrets]] stored in password managers we have[fn:: Remember that any unique set of key value pairs can be used, see ~man secret-tool~ for details]:
#+begin_src bash
secret-tool store --label="dvc" service "rclone" project "gprd_dimer"
#+end_src

Which can then simply be set to ~RCLONE_PASSWORD_COMMAND~ and used directly:
#+begin_src bash
export RCLONE_PASSWORD_COMMAND='secret-tool lookup service "rclone" project "gprd_dimer"'
rclone config --ask-password=false
#+end_src

We don't want duplication. So there has to be a question of if we want to use
the artifact uploaded directly to an S3 instance or if we want to use local and
let DVC handle putting it on a bucket if need be.

Rather than use the local NAS approach of letting DVC take an absolute path to
an ~rclone mount~ point, it is a bit nicer to use an ~rclone server~ WebDAV
instance instead.

#+begin_src bash
pixi add dvc-webdav
# In one terminal
rclone serve webdav HIOneDrive:/.dvcstore --vfs-cache-mode full --addr unix:///tmp/dvc.hi.socket
# In another
dvc remote add -d HIOneDrive webdavs:///tmp/dvc.hi.socket
#+end_src

Where we assume that ~rclone~ is configured as per the [[https://rclone.org/onedrive/][OneDrive instructions]].
...
Annoyingly, it turns out that even with ~ssl_verify false~, an SSL record layer
...
Pushing
ERROR: unexpected error - [SSL] record layer failure (_ssl.c:1000): [SSL] record layer failure (_ssl.c:1000)
#+end_src

Which means that we need to setup a key pair, with the defaults, which is OK
since it will only be used locally:
...
#+begin_src bash
rclone serve webdav HIOneDrive:/.dvcstore --vfs-cache-mode full --addr localhost:9677 --cert cert.pem --key ec_key.pem
#+end_src
...
~net::ERR_CERT_AUTHORITY_INVALID~ warnings [fn:: However, this post isn't about using LetsEncrypt or ~certbot~ to get a proper certificate, nor is it relevant] but will now merrily accept the connection (since ~ssl_verify~ is still off):
...
- Ignore folders in the ~.gitignore~, else ~dvc~ may complain that the relevant ~.dvc~ is ignored by ~git~
  + i.e. to ignore ~best_trial_8/~ don't ignore ~best_trial*~ but use ~best_trial*/~
** Conclusions
...
[[https://archive.materialscloud.org/record/2025.91][Materials Archive]] and a [[https://rgoswami.me/snippets/cold-storage-compression/][strong compression technique]] for personal storage via
~borg~ it is fairly resilient.

* DRAFT Per-project Spack Baseline with ~git-subrepo~ :@notes:ramblings:workflow:
:PROPERTIES:
:EXPORT_FILE_NAME: pp-spack-base-2025
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("HPC")
:END:
#+begin_quote
~spack~ first ~git-subrepo~ setup for reproducible HPC repositories.
#+end_quote
** Background
[[https://spack.readthedocs.io/en/latest/getting_started.html][Spack]] is pretty useful for generating optimized binaries for the HPC. ~pixi~ is
generally useful for projects which can make use of the ~conda~ ecosystem, but
for most high performance computing software, optimizing compilers and host of
interdependent build requirements lead to either Spack, Easybuild, Singularity
or Nix [fn:: Though most clusters don't ship with ~nix~]. More background on
~spack~ including local environment use was covered in a {{{relref(previous post,spack-pytorch-dev-workflow)}}}.

At the same time, the [[https://github.com/ingydotnet/git-subrepo][git-subrepo]] helper allows vendoring repository contents,
which somewhat alleviates the fragility of ~submodules~ while retaining the
ability to push and pull from upstream [fn:: *Never* rebase the subrepo target,
and *always* have a forked branch for the subrepo in question!]. Taken together,
these can work synergistically with a queue aware workflow manager like
~snakemake~ for building and running high throughput calculations.
** Incorporating with ~subrepo~
Assuming a local clone of ~spack~ and a repo where reproduction is required, we
will first create a new branch for the project:
#+begin_src bash
git checkout --branch gprd_sella_bench # or whatever
git push -u origin gprd_sella_bench
#+end_src
This branch *should not* be force pushed to, otherwise the ~subrepo~ linkage
will break and need to be setup from scratch.

In the project repository now:
#+begin_src bash
git subrepo clone git@github.com:HaoZeke/spack subrepos/spack -b gprd_sella_bench -m "MAINT: Add spack for reproduction"
#+end_src
...
*Note*
#+end_alert-heading
This is a fairly heavy operation, and is really only meant for projects which
need it, otherwise consider simply using ~pixi~.
#+end_alert

Now we can effectively follow along with [[https://spack.readthedocs.io/en/latest/getting_started.html][the documentation]], bootstrapping
~clingo~ for dependency resolution.
#+begin_src bash
. subrepos/spack/share/spack/setup-env.sh # Assumes zsh || bash
spack spec zlib # Or anything really
spack --bootstrap find # Should have clingo
#+end_src

Spack will still continue to use ~$HOME/.spack~ for [[https://spack.readthedocs.io/en/latest/getting_started.html#compiler-configuration][most of the configuration]],
and indeed this allows easy usage beyond the project directory [fn:: Unlike
~pixi~ but like ~micromamba~ or ~conda~ named environments].

The point this is to ensure the exact state of the ~spack~ recipes used within
the repository for posterity. Additionally, new recipes for the project can be
easily pushed via ~git subrepo~ to make opening pull requests post validation
much easier.
* DRAFT Thoughts on free threading Python development
:PROPERTIES:
:EXPORT_FILE_NAME: thoughts-ft-pydev
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
Where I pontificate on supporting free threaded Python.
#+end_quote
** Background
Quansight Labs, in my final few days there, had a vested interest in ensuring
most of the Python ecosystem has support for free threaded builds[fn:: Not
necessarily bug free or optimal, but atleast to a point where the GIL is not
re-enabled due to a downstream dependency.].
** Setting up sanitizers
The easiest
* DONE Simplified SSH access via container VPNs :@notes:ramblings:setup:remote:
CLOSED: [2025-05-11 Sun 17:44]
:PROPERTIES:
:EXPORT_FILE_NAME: remote-access-cvpn
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Remote usage")
:END:
#+begin_quote
Leveraging containers for streamlined remote interactive and mounted file access to intranet machines
#+end_quote
** Background
Most academic or corporate networks gate access to internal resources (like
...
university subscriptions, but it becomes problematic when:
- You only want some of your traffic to go through the VPN.
- You need to connect to multiple distinct services, each behind its own
  separate VPN. Managing these simultaneously or switching between them on a
  host system is often cumbersome or impossible.

These considerations not only affect interactive SSH sessions but also
complicates using tools for remote file management or other services that need
to operate through these VPNs.
*** Tools
We'll be using [[https://podman.io/][Podman]] with [[https://www.infradead.org/openconnect/][OpenConnect]] and [[https://www.openssh.com/][OpenSSH]] to build a VPN jumphost for
simplified ~ProxyJump~ configurations and [[https://rclone.org/][rclone]] for SFTP mounting.
** Normal usage
To recap, the standard workflow, for say, an ~anyconnect~ VPN is as follows:

#+begin_src bash
sudo openconnect $VPN -u $VUSER --useragent AnyConnect
# Another terminal
ssh $LMACHINE
#+end_src

Where ~LMACHINE~ is normally only accessible from the intranet, access to which
is provided by ~VPN~ authenticated for ~VUSER~. ~sudo~ is there since the
networking of the current machine, say ~CMACHINE~ is affected. This works, but
...
*** Caveats
- Full Network Routing :: Your entire machine's internet usage goes through the VPN.
- Multiple VPNs :: Switching between VPNs is slow, and running multiple
  simultaneously is generally not feasible.
- Streamlining Access :: Even with one VPN, accessing many internal machines via
  direct SSH can be made more convenient with a jumphost and the SSH ProxyJump
  feature.
** Container setup
Containerization offers a powerful solution by isolating the VPN connection
within a dedicated environment, leaving your host system's networking untouched.
This container can then act as a secure SSH jumphost.

A basic container using an Alpine base with a non-root user for this is
basically:
#+begin_src conf
FROM alpine:latest
...
#+end_src
Normally, I'm a fan of using ~tinyssh~ {{{relref(for basic containers,tinyssh-dockerdev-env)}}} but since this is
meant to be a jump server, we need ~AllowTcpForwarding~ and so ~openssh~ is
...
exec openconnect sample.vpnhost \
    -u me@sample.vpnhost \
    --useragent='AnyConnect' \
    --script=/etc/vpnc/vpnc-script
#+end_src

Here the command is hardcoded, but it can be adapted or parameterized via an
environment variable if required.

Built with:
#+begin_src bash
# Remember to modify this for your key
export PUB_KEY_CONTENT=$(cat ~/.ssh/cstuff.pub)
# Build the container called vpn-smart-jumphost
docker build \
...
Host *.vpnhost !vpnhost
    User rgoswami
    ProxyJump vpn-docker-jump

Host someone.cstuff
  Hostname something.vpnhost
  User rgoswami
  ProxyJump vpn-docker-jump
#+end_src
Replace ~~/.ssh/cstuff~ with the path to the private key that corresponds to the
public key you used during the build. Replace ~rgoswami~ with your username on
the target systems if it's different.
*** Usage
Once the container is running, the VPN is connected within it, and your
~~/.ssh/config~ is set up, you can SSH to your remote machines from a new
...
#+begin_src bash
ssh some-other-server.vpnhost
#+end_src
The first time you connect via the jumphost, you may be asked to verify and
accept the SSH host key of the container.
** Mounting SFTP Remotes
The ~ProxyJump~ setup in ~~/.ssh/config~ is excellent for interactive SSH
sessions, ~scp~, and direct ~sftp~ commands. However, for ~rclone~ ([[https://rclone.org/][homepage]]),
which can mount a remote SFTP share, some more work is necessary for the VPN
jumphost.

~rclone~ can leverage SSH capabilities, but it does not parse directives from
the global SSH config directly.

We can achieve this by constructing an appropriate ProxyCommand and passing it
to rclone via its SFTP backend options.

#+begin_src bash
ssh -o "ProxyCommand ssh jumphostuser@127.0.0.1 -p 2200 -i ~/.ssh/cosmolab -o IdentitiesOnly=yes -W %h:%p" $USER@$VPN_HOSTNAME
#+end_src

The ProxyCommand effectively tells SSH how to connect to the intermediate
jumphost first, and then tunnel the connection to the final SFTP server.

Essentially we have ~rclone config edit~ (followed by ~n~ and then ~SFTP~) to
end up with:
- *type* :: ~sftp~
- *host* :: ~$RCLONE_REMOTE~
- *user* :: ~$USER~
- *pubkey_file* :: ~$SSH_KEY~
- *key_use_agent* :: ~true~
- *ssh* :: ~ssh -o "ProxyCommand ssh jumphostuser@127.0.0.1 -p 2200 -i ~/.ssh/cosmolab -o IdentitiesOnly=yes -W %h:%p" $USER@$VPN_HOSTNAME~

Final usage proceeds in the usual fashion:
#+begin_src bash
rclone mount "$RCLONE_REMOTE:/home/$USER" ~/mnt/somevpn --vfs-cache-mode full
#+end_src
...
~rclone~ also works well, which means integration with ~emacs~ and other IDEs is
a snap.

A more robust variant of this basic concept developed into the [[https://github.com/HaoZeke/vpn-proxyjump][vpn-proxyjump]]
project.
* DONE Parameter passing in NWChem and the Runtime Database :@notes:hpc:science:components:parameters:nwchem:
CLOSED: [2025-08-16 Sat 19:06]
:PROPERTIES:
:EXPORT_FILE_NAME: parameter-passing-nwchem-rtdb
:EXPORT_BIBLIOGRAPHY: biblio/Website.bib
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :link-citations true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Simulation software patterns")
:EXPORT_HUGO_PANDOC_CITATIONS: t
:END:
#+begin_quote
...
#+end_quote
** Background
Despite a plethora of publications
[@apraNWChemPresentFuture2020;@mejia-rodriguezNWChemRecentOngoing2023], NWChem
has little to nothing in terms of developer documentation [fn:: A trait shared
by its ill fated successor, NWChemEX [@kowalskiNWChemNWChemExEvolving2021].].
For better integration over sockets with EON (described elsewhere), I ended up
having to add user defined parameters, from which this short note stems.
** Inputs in NWChem
The input file format in NWChem, like most codes of the era, consist of a
keyword driven, whitespace delimited, block structured set of directives which
are parsed once into a database and propagated across other modules.
** Runtime Database (RTDB)
The run time database, as noted in
[@bernholdtParallelComputationalChemistry1995;@guestHighperformanceComputingChemistry1996], is similar to the GAMESS dump
file or GAUSSIAN checkpoint file. Allegedly it is
easier and safer than both, and arrays of typed data are stored with ASCII
strings for keys. Though the GAMESS and GAUSSIAN notes are not an immediately
recognizable point of reference either, but it is essentially a key value store
which is used to circumvent having an interoperable data-structure.

This centralized key-value approach decouples modules from the specifics of the
input file; a routine only needs to know the key for a parameter, not how or
where it was parsed. This contrasts with approaches where configuration objects
must be passed through long call stacks or where multiple components parse the
same configuration file independently.
...
- Parsing :: To extract data from the input
- Storing :: Committing parsed values into the RTDB with ~rtdb_put~
- Retrieving :: Accessing the values from the RTDB with ~rtdb_get~
...
*Note*
#+end_alert-heading
The complete changeset required is enumerated in the [[https://github.com/nwchemgit/nwchem/pull/1145][pull request]] with a
documentation update to the wiki [[https://github.com/nwchemgit/nwchem-wiki/pull/39][here]].
#+end_alert
*** Parsing optional keyword arguments
Naming follows a conventional scheme[fn:: Though as always, a full ~grep~
through is recommended], so the ~src/driver/driver_input.F~ is the first port of
call. The implementation involves setting defaults and then looping through
remaining fields on the line to check for overrides.
#+begin_src f90
...
Where ~inp_a~ parses a keyword, ~inp_compare~ checks against an expected value,
and ~inp_i~ parses the value as an integer.
*** Committing values to the RTDB
Once stored in a local variable, the value is then committed to the RTDB using
~rtdb_put~, which is prefixed into a namespace for modularity and to reduce key conflicts.
#+begin_src f90
if (inp_compare(.false.,'retries',field)) then
...
#+end_src
Where the signature notes the number of elements (~1~) being stored along with
the type (~mt_int~).
*** Retrieval and usage
Where the values are to be retrieved, say, here in ~src/driver/socket_driver.F~,
defaults are used if the key is missing.
#+begin_src f90
 if (.not. rtdb_get(rtdb, 'driver:socket_retries', mt_int, 1, max_retries)) then
    max_retries = 30
 end if
#+end_src
Before being used as needed. The signature of ~rtdb_get~ conceptually mimics
that of ~rtdb_put~. In practice, since these are FORTRAN 77 files in ~nwchem~,
there are a few more continuation line characters and other marginalia, but
otherwise this is all that is necessary to expose user parameters to modules in
NWChem.
** Conclusions
Although not the design taken by more modern software, the RTDB is pretty intuitive to
work with, and probably better than GAMESS, GAUSSIAN, or even the bring your own
data into a simulation development environment idea of NWChemEx
[@richardDevelopingComputationalChemistry2019]. Better documentation would
probably stemmed the split. Modern tools (e.g. EON) improve upon this by using
more structured input files, e.g. using TOML ensures typing which can then be
independently read into C/C++/Fortran etc. though the issue of consistency at
runtime remains.
* TODO Practical ~python~ script parallelism :@notes:python:workflow:tools:
:PROPERTIES:
:EXPORT_FILE_NAME: practical-python-script-parallelism
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
Excursions into I/O bound scripts.
#+end_quote
** Background
One of the most common tasks I end up with involves walking over relatively
large nested file hierarchies.
* TODO Monorepo management for reproducible research
* TODO An excursion into ~nushell~
** Background
I've been using ~zsh~ for over a decade at this point. In general things work
well. However over time most of my scripts became reliant on ~rust~ tools, like
~starship~ to be cross compatible across machines.
** Dotfile conversion
To help streamline the inevitable, a new branch of my ~chezmoi~ dotfiles
contained the changes needed to get to an approximation of my previous setup
with ~nushell~. These were not [[https://github.com/HaoZeke/Dotfiles/compare/chezmoi...toNu][particularly hard but fairly repetitive]].
** Conclusions
~nushell~ is perfectly usable; even pleasant for many use-cases. However, the
struggle involved in re-writing things to work with it is a bit higher than
anticipated. ~fish~ seems like another stop along the way back towards ~bash~.
* TODO Swimming with ~fish~
** Background
Having left ~nushell~ behind mostly due to incompatibilities with ~spack~. I
decided to give ~fish~ a chance for the first time in a few decades.
** Conclusions
* TODO Taming submodules with ~git-subrepo~ :@programming:tools:git:workflow:
:PROPERTIES:
:EXPORT_FILE_NAME: taming-submodules-git-subrepo
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
Essentially this works in two phases:
- ~pixi~ is used to ensure locally editable packages are ready for immediate use
- ~git-subrepo~ is used to changes are easily upstreamed

This workflow is easier than using a fork only approach, due to the locality of
data. Additionally, it allows for a "fat" release of the analysis.

Essentially:
- Make a new branch in each project / tool which the primary project depends on
  + This may entail a fork
- Clone each of these (with the right branch) into a local directory with ~git-subrepo~
- Automatically add them and changes to the environment via ~pixi~

In practice this could look like:
#+begin_src bash
cd $MY_TOOL
git checkout -b my_project
git push -u origin my_project
cd $MY_PROJECT
mkdir subrepos
git subrepo clone git@github.com:$GITHUB_ID/$MY_TOOL_NAME.git subrepos/$MY_TOOL_NAME -b $MY_PROJECT
git commit --amend # rename to something like MAINT: Pin $MY_TOOL_NAME locally with git-subrepo
#+end_src
Additionally, within the ~pixi.toml~:
#+begin_src toml
[pypi-dependencies]
$MY_TOOL_NAME = { path = "./subrepos/$MY_TOOL_NAME", editable=true}
#+end_src

*** Handling ~setuptools_scm~ for Python versions

Most packaging managers using ~vcs~ information rely on ~setuptools_scm~ under the
hood (e.g. ~hatch_vcs~ does). While this approach works well with ~git submodule~
systems, it turns out that with ~git subrepo~ the installation cannot proceed
because of the ~.git~ folder not being part of the checkout.

That being said, a workaround is to simply set the
~SETUPTOOLS_SCM_PRETEND_VERSION~ variable instead.

For instance:
#+begin_src bash
export SETUPTOOLS_SCM_PRETEND_VERSION="v0.0.dev0+$MY_PROJECT_$(git rev-parse HEAD)"
#+end_src

Which can then be stored in an activation script for ~pixi~ commands to pickup on:
#+begin_src toml
[activation]
scripts = ["scripts/env_setup.sh"]
#+end_src

Another workaround would be to use the actual repository for the ~pixi~ bit, i.e.
the one stored in ~.git/tmp/subrepo/subrepos/$MY_TOOL_NAME~

*** Using ~git sub-repo~
The contributing flow is basically:
#+begin_src bash
git subrepo branch subrepos/$MY_TOOL_NAME
git subrepo push subrepos/$MY_TOOL_NAME
#+end_src
Make changes in ~$MY_TOOL~

* TODO Revisiting Linux Email :@personal:rant:tools:email:workflow:
:PROPERTIES:
:EXPORT_FILE_NAME: revisiting-linux-email
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("Managing Email")
:END:
#+begin_quote
Securing Email backups with Evolution and ~notmuch~
#+end_quote
** Background
I've used pretty much every Linux email client over the years. Thunderbird has
been in decline for ages. Astroid unfortunately seemed to run out of steam, as
did Mailspring. One of the worst parts of this entire situation is having to
handle backups separately. Also HTML emails have largely displaced plain text,
making more elegant CLI readers a bit limited. Most of the baseline setup is
very similar to the upstream documentation or my previous post.

This leads to a fairly obvious middle ground. Handle backups and synchronization
via CLI tools as before, but use Evolution as a front-end for interacting with
the ~maildir~ and sending email.
** IMAP accounts
** Exchange accounts
The setup from the previous post is still valid, with multiple ~davmail~
configurations required for each additional Outlook email to be managed.
** Encryption
*** Cloud storage
Easiest to mount a dedicated folder with ~rclone~ or ~pcloud~ and then use ~cryptomator~. An ~rclone-crypt~ remote would work just as well.

#+begin_src bash
sudo mkdir -p /media/mail
sudo chown -R $USER:$USER /media/mail
#+end_src

Easiest to mount the folder as a sub-directory wrapped around which a ~pixi~
configuration will be present. There are problems though. Constant
synchronization is slow and network intensive.

In fact, this turned out to be a dead end.

*** Backup only
In practice however, there is not much use for a locally encrypted backup
especially given the degraded performance. Instead, a ~cron~ script works better
via ~borg~, keeping encryted, compressed backups, while having fast access to
local mail.
* DONE Back to ~bash~ :@personal:tools:workflow:dots:
CLOSED: [2025-11-12 Wed 19:28]
:PROPERTIES:
:EXPORT_FILE_NAME: back-to-bash
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:EXPORT_HUGO_CUSTOM_FRONT_MATTER+: :series '("$HOME...")
:END:
#+begin_quote
...
cannot comfortably bring ~$HOME~ ergonomics [fn:: Also my ~zsh~ startup times
hovered an intolerable 50ms, which I later found out was mostly due to ~spack~].
*** The road to ~bash~ is paved with ~$SHELL~ s
There are far too many today to bother enumerating, but in short order [fn:: Discounting many more ephermeral tests]:

- ~nushell~ :: Pleasant for many use-cases, the process of converting my
  dotfiles was [[https://github.com/HaoZeke/Dotfiles/compare/chezmoi...toNu][fairly repetitive]], and its incompatibility with ~spack~ made it a
  non-starter [fn:: Fun for data pipelines and nicer multi-line commands].
- ~fish~ :: Adopted to resolve the ~spack~ issue. However, I found I was the
  wrong audience for it; features like global, browser-based customization are
  not useful for my terminal-centric, ~chezmoi~ focused workflow [fn:: Plus breaking POSIX for the features offered seemed like a bad trade-off].
- ~xonsh~ :: [[https://rgoswami.me/posts/xonsh-excursion/][Ergonomic]], and better than ~subprocess~ for scripts, but a
  non-starter in terms of being a good ~$SHELL~ [fn:: i.e. barely any community support]

For the most part I had resigned myself to ~fish~. It was fast, needed very few
plugins [fn:: So fewer failure modes than my favorite ~zsh~].

Eventually I came across [[https://www.codyhiar.com/blog/switching-back-to-bash-from-zsh/][this post by Cody Hiar]] on ~ble.sh~, and decided to take
...
# ** Designing literate ~bash~ snippets
# Having decided to return to ~bash~, I was nevertheless not eager to return to
# handling my entire configuration in a series of snippets, despite the
# {{{relref(re-structured setup described
# earlier,dotfiles-dotgit-bombadil#current-approach)}}}. Taking inspiration from
# ~doom~, I ended up with a literate configuration with the
# TODO(rg): finish this!!
** Caveats
...
stick to ~bash~ and save my "[[https://steveklabnik.com/writing/the-language-strangeness-budget/][strangeness budget]]" [fn:: A concern I was recently re-sensitized to by [[https://github.com/Luthaf/][Luthaf]]] for new-fangled programming
languages instead of shells[fn:: At any rate, I do relish not having to transform one-liners from ~nushell~ and friends back to ~bash~ to pass to co-workers.].
* TODO ~jj~ crib-notes
#+begin_quote
Reducing maintainer burdens for development workflows
#+end_quote
** Background
** Initialization
Getting ~jj~ locally through ~cargo binstall~ works well.
#+begin_src bash
❯ cargo binstall jj-cli
❯ jj --version
jj 0.36.0-70fd8f7697fbc20a9329a6e2f790ef86a8e284d1
#+end_src
Which can be setup now.
#+begin_src bash
# Setup
jj git init
# View history
...
#+end_src
** Stop clobbering ~.jj~
Initialization warns that ~git clean -dfx~ removes the ~.jj~ folder.
In practice:
#+begin_src bash
metatensor [ dlpWriter][△ v4.2.0][🐍 v3.13.7][🦀 v1.91.1]
❯ git clean -dfxn
Would remove .jj
Would remove Cargo.lock
#+end_src

However, ~-e~ excludes additional arguments.
...
Which then ends up being an alias.
...
#+begin_src bash
❯ jj edit finalDLP_1
Error: Commit 808499726ff2 is immutable
Hint: Could not modify commit: wtzlnukk 80849972 finalDLP_1 finalDLP_1@origin | chore(api): regenerate declarations
Hint: Immutable commits are used to protect shared history.
Hint: For more information, see:
      - https://docs.jj-vcs.dev/latest/config/#set-of-immutable-commits
      - `jj help -k config`, "Set of immutable commits"
Hint: This operation would rewrite 10 or more immutable commits.
#+end_src
Which comes from having pushed to a fork, however only ~main~ and ~upstream~ really should be considered immutable, the rest get force-pushed to routinely during review anyway.
#+begin_src bash
❯ jj config set --repo revsets.immutable-heads 'main@origin | main@upstream'
#+end_src

** Replacing ~rerere~
** Caveats
I use a lot of ~git-subrepo~ [fn:: for production archives], which isn't a good fit for ~jj~ at the moment.
** Conclusions
* Important Note :ignoreheading:ignore:
# ALWAYS at the bottom, to ensure fresh builds do not break
* DONE About
:PROPERTIES:
:EXPORT_HUGO_SECTION: /
...
sporadically updated {{{relref(collection of web-links,rg-collection-weblinks)}}} involving me. There is also, a
separate list of quotes, or other things I have something to say about.
** Tangible Positions
- From May 2025, I am [[http://archive.today/AlM2e][now associated]] with [[https://www.epfl.ch/labs/cosmo/][labCOSMO]] at the École Polytechnique Fédérale de Lausanne (EPFL), working closely with [[https://guillaume.fraux.fr/][Dr. Guillaume Fraux]] and [[https://people.epfl.ch/michele.ceriotti?lang=en][Prof. Michele Ceriotti]]
** Intangible Positions
This is a set of things which are primarily online and/or voluntary in a
non-academic sense.
- I [[https://numpy.org/teams/][maintain within NumPy]], mostly parts of ~f2py~
- I serve [[https://nordic-rse.org/about/governance/][on the board]] of the [[https://nordic-rse.org/][Nordic Research Software Engineers association]]
...
me[fn:growingUp]. Many of these are tangible, but better ordered in this manner.
*** Kingston
- I spent four months in 2025 officially with the lab of [[https://www.laurentkarimbeland.com/][Prof. Laurent Béland]] at
  Queen's University
  + As a work permit exempt TEER-1 research visitor
*** Reykjavík
- I [[https://english.hi.is/staff/rog32][am associated]] with the [[https://notendur.hi.is/hj/researchgroup.html][reputed Jonsson group]] of the [[http://raunvisindastofnun.hi.is/the_science_institute][Science Institute]] at the
  [[https://english.hi.is/school_of_engineering_and_natural_sciences][University of Iceland]], where I benefit from the guidance of the erudite and
  inspiring [[https://notendur.hi.is/hj/indexE.html][Prof. Hannes Jonsson]]
  + From January 2021 to 2025, I held a three year doctoral fellowship [[https://en.rannis.is/news/allocation-from-the-icelandic-research-fund-for-the-financial-year-2021-1][from the Icelandic Research Fund (Rannís)]] for my project on "_Magnetic interactions of itinerant electrons modeled using Bayesian machine learning_" tenable at the Science Institute with [[https://notendur.hi.is//~hj/indexE.html][Prof. Hannes Jónsson]] as my supervisor and [[https://english.hi.is/staff/birgirhr][Prof. Birgir Hrafnkelsson]] as my co-supervisor
...
- From August 2021 to June 2025, I was with [[https://labs.quansight.org/][Quansight Labs]] as a Software Engineer (I-->II) and reported to ([[https://rgommers.github.io/][Dr. Ralf Gommers]] --> [[https://melissawm.github.io/about-me/][Dr. Melissa Weber Mendonça]])
  + My work initially revolved around ~f2py~ through the [[https://labs.quansight.org/blog/2020/11/a-second-czi-grant-for-numpy-and-openblas/][CZI NumPy project]] with [[https://melissawm.github.io/about-me/][Dr. Melissa Weber Mendonça]] and [[https://github.com/pearu][Dr. Pearu Peterson]]
  + Passed through working on PyTorch and writing ~meson~ bindings to OpenBLAS
  + Picked up maintaining ASV (in addition to F2PY)
- I collaborated closely with the brilliant [[https://ondrejcertik.com/][Dr. Ondřej Čertík]] over the course of both on my {{{relref(GSoD 2020 long project for SymEngine,symengine-gsod20)}}} and over {{{relref(GSoC 2021 on the LFortran compiler,gsoc21-fin-reprot)}}}
- I was a Fellow at the Institute for Pure and Applied Mathematics (IPAM) for its Spring 2021 program on “[[http://www.ipam.ucla.edu/programs/long-programs/tensor-methods-and-emerging-applications-to-the-physical-and-data-sciences/?tab=participant-list][Tensor Methods and Emerging Applications to the Physical and Data Sciences]]”
- I have also benefited from sitting in on some formal coursework here, which
  was a fascinatingly useful experience
*** Kanpur
...
- I honed coursework development and teaching [[https://www.univ.ai/team/rohit-goswami][with univ.ai]]
*** Bombay
...
#+caption: A collage of mugshots, shuffled and not ordered by date
[[file:images/avatarCollage.jpg]]

** Donations
If you've gotten this far, you might also want to check out the
following[fn:patreon]:
- [[https://github.com/sponsors/HaoZeke][Github sponsors page]]
...
#+END_QUOTE
** Background
I mentioned {{{relref(on my about page,about)}}}, that it is nigh impossible to keep track of every
...
** Profiles
*** Professional
- [[http://archive.today/TyXoV][EPFL page]]
- [[https://sh.turtletech.us/ui-rg][University of Iceland]]
- [[https://femtolab.science/people/rohit][FemtoLab]]
*** Voluntary
- [[http://archive.today/cmC4v][RealPython author]]
- [[http://archive.today/AlQks][NumPy Team]]
- The Carpentries
...
*** About me
- Lab COSMO [[http://archive.today/AlM2e][joining note]]
** Pages and Articles
*** By Me
- RealPython article on [[http://archive.today/2UYbl][optimal python script structure]].
- Articles written for the [[https://studentabladid.is/efni?author=6166dde6acb3ff544309ecd6][Student Paper of the University of Iceland]]
...
- Rowan [[http://archive.today/ljniL][blog-post on transition states]]
**** Lists
...
- [[https://twitter.com/aiidateam/status/1281323480290660352][AiiDA Virtual Tutorial 2020]]
...
**** Teaching
- [[https://www.iopconferences.org/iop/frontend/reg/thome.csp?pageID=1032921&eventID=1664][IOP Workshop on C++ II - Libraries and Simulations]] :: April 13, 2020 (instructor) ({{{relref(materials,iop-cpp-2021-meta)}}})
...
- Emacs News
  - [[https://sachachua.com/blog/2021/07/2021-07-26-emacs-news/][2021-07-27]] :: for {{{relref(Doom Emacs and Language Servers,emacs-lang-servers)}}}
  - [[https://sachachua.com/blog/2020/05/2020-05-11-emacs-news/][2020-06-22]] :: for {{{relref(Temporary LaTeX Documents with Orgmode,org-arb-tex)}}}
  - [[https://sachachua.com/blog/2020/05/2020-05-11-emacs-news/][2020-06-15]] :: for {{{relref(Emacs for Nix-R,emacs-nix-r)}}}
  - [[https://sachachua.com/blog/2020/05/2020-05-11-emacs-news/][2020-05-11]] :: for {{{relref(An Orgmode Note Workflow,org-note-workflow)}}}
  - [[https://sachachua.com/blog/2020/05/2020-05-04-emacs-news/][2020-05-04]] :: for {{{relref(Pandoc to Orgmode with Babel,org-pandoc-babel)}}}
  - [[https://sachachua.com/blog/2020/04/2020-04-27-emacs-news/][2020-04-27]] :: for {{{relref(Using Mathematica with Orgmode,org-mathematica)}}}
  ...
  - [[https://sachachua.com/blog/2020/04/2020-04-06-emacs-news/][2020-04-06]] :: for {{{relref(Replacing Jupyter with Orgmode,jupyter-orgmode)}}}
** Videos
*** Of Me
...
I've been sitting in on these for a while thanks to [[https://www.math.ias.edu/~ke.li/][Ke Li]], but this section lists some of the lectures I asked a question in
  ...
  - "[[https://www.youtube.com/watch?v=jVBKWfoYRng][A Blueprint of Standardized and Composable Machine Learning]]" by Eric Xing

[fn:whereswaldo] If you do think you have seen me somewhere not on this list, drop me an email

* COMMENT Local Variables
This file is too large to be auto-exported on each save.
