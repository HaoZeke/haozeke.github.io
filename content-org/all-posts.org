...
* TODO Reclaiming Email with Astroid :@personal:rant:tools:email:
:PROPERTIES:
:EXPORT_FILE_NAME: reclaim-email-astroid
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc true :comments true
:END:
#+begin_quote
Migrating 23 mail accounts from GUI clients to Astroid
#+end_quote
** Background
Initially, I had planned this post to start with a brief history of the decline of email clients for Linux. That quickly got out of hand, and was therefore [[The Decline of Linux Email][spun out into a post of its own]]. To keep things brief. Thanks to the incredible ineptitude of the Thunderbird steering committee, I ended up requiring a new mail client. Having despaired of the GUI based bloat heavy approaches of most clients, I decided to go the old fashioned route and build one up in a modular manner.
** The Accounts
In no order of preference, for a variety of reasons, I have 23 distinct email accounts.
- Outlook 365 :: A school account, best handled with ~davmail~
- Gmail :: There are a few of these, two personal, and two institutional
- Yahoo :: Two standard free accounts
- Mail.ru :: One
** InSync
** Notmuch
This needs to be set up first.
** Gmaileer
The general setup is simply to make
*** IEEE
...
** Astroid
The bulk of this setup is by the numbers, with the exception of the poll script.
** Poll Script
Natively only bash appears to be supported. However, with ~nix~, we can use a reproducible python script instead.
#+begin_src python
    ...
    if dirs.is_dir():
        for gmi in GMAIL_IDENTIFIERS:
            if gmi in dirs.name:
                ...
                sh.gmi("sync", _cwd=dirs, _bg=False)
#+end_src
