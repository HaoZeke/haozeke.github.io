...
** DRAFT Precommit conventions :regex:packaging:precommit:
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: precommit-conventions
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
**** Expressive ~regex~
~pre-commit~ accepts ~exclude~ keys which take ~python~ regular expressions,
