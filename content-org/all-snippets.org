...
** DRAFT Keeping a laptop responsive under ~rust-analyzer~ memory pressure :linux:rust:systemd:zram:cgroups:chezmoi:
:PROPERTIES:
...
Magic SysRq stays as an escape hatch for the rare case where something wedges the compositor or GPU path and the keyboard can still reach the kernel:
...
All four files live under ~chezmoi~ at [[https://github.com/HaoZeke/dotfiles][HaoZeke/dotfiles]]: ~dot_config/systemd/user/rust-analyzer.slice~, ~dot_local/bin/executable_rust-analyzer~, and a ~.chezmoiscripts/run_onchange_linux_memory_tuning.sh.tmpl~ that installs the ~/etc/*~ blobs under ~sudo~ with ~cmp -s~ guards so unchanged content never re-prompts. Reimaging a laptop restores the full stack with one ~chezmoi apply~.
* Footnotes
