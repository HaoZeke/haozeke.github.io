...
Recently a bunch of my pre-commit CI jobs started failing. The easiest way to ensure reliable usage is to have ~local~ runs. Consider:
#+begin_src yaml
...
#+end_src
Which leaves one dependent on an external repo, can be rewritten as:
#+begin_src yaml
- repo: local
  hooks:
    - id: cppcheck
      ...
      args: ["--error-exitcode=0"]
#+end_src
With this local setup, it's necessary to adjust the CI configuration:
#+begin_src yaml
name: pre-commit
on:
  pull_request:
