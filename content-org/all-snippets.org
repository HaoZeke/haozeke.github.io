...
** DRAFT ~precommit~ selections :regex:precommit:
:PROPERTIES:
...
file isn't present.
** TODO ~git~ tips
1. Open the first unmerged file in ~emacsclient~
#+begin_src bash
...
** TODO Desktop files and ~tmux~ sessions
...
** DRAFT Handling environments with ~cibuildwheel~
...
#+end_src
** DRAFT Terminal Slack with Pidgin
...
Later, setup ~gntrc~ from ~man finch~ to use ~nchat~ like bindings.
** DRAFT Remote ~lldb~ on MacOS
