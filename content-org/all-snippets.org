...
Along with an additional macro for the challenge headings, so ~{{{mdheader(### Details)}}}~ with the ~begin_discussion~ segments work: 
#+begin_src
...
# End:
#+end_src
Which together, correctly render lessons for ~Rscript -e "sandpaper::serve()"~ as [[https://haozeke.github.io/python_packaging_workbench/][seen here in this Python lesson]].
