...
#+begin_src bash
#+end_src
** TODO Trivial LLMs on old hardware
Apropos nothing. R1 is the remote machine. L1 is the local machine. Tailscale connects the two.
#+begin_src bash
...
A first check on the local machine is:
#+begin_src bash
