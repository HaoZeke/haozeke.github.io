...
** TODO Listing things
*** Zotero plugins
For getting started with Zotero (including WebDAV sync and migrating from Mendeley), see [[https://turtletech.us/posts/mendeley-to-zotero-webdav/][this walkthrough]].
...
#+end_src
** DONE Search and merge with ~git~ filtered repositories :git:cmdline:
CLOSED: [2025-09-04 Thu 02:10]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: git-filter-cherry-pick
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
...
#+begin_src bash
git rev-parse --short :/"header or other commit message text"
#+end_src
...
** DONE Mesa Bugs with Sway :@note:ramblings:
CLOSED: [2025-01-26 Thu 19:52]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: mesa-sway-bugs
:EXPORT_FILE_NAME: index
...
** DONE ~act~ locally for ~sccache~ :cmdline:build:
CLOSED: [2026-01-31 Sat 08:08]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: act-local-caching
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
Cache helpers like [[https://github.com/mozilla/sccache][sccache]] tend to break local ~act~ usage, necessitating:
#+begin_src diff
      - name: Setup sccache
+        if: ${{ !env.ACT }}
...
** TODO Audio wrangling
#+begin_src bash
# convert from aac to mp3
ffmpeg -i onNotice.aac output.mp3
# split into 49MB chunks
mp3splt -S 49 output.mp3
# or, native but might cut frames
split -b 49M output.mp3 49mb_part_ --additional-suffix=.mp3
#+end_src
...
#+end_src
does the trick.
** DONE Refactoring an ~ox-hugo~ site for newer ~orgmode~
CLOSED: [2025-08-29 Fri 00:01]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: refactoring-ox-hugo-orgmode
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
On newer versions of ~orgmode~ (i.e. anything ~9.7~ or newer) something upstream
breaks the manner in which ~ox-hugo~ handles links for sub-tree exports[fn::
xref [[https://github.com/kaushalmodi/ox-hugo/pull/755#issuecomment-2700856101][this comment]]]. The most robust workaround is to stop using native Org links
and switch to a manual Hugo short-code via an Org macro.

The process has two parts. First, we need the ~relref~ macro in the Org file with ~#+macro: relref @@hugo:[$1]({< relref "$2" >})@@~ [fn:: Wrap that the link with another ~{}~].

Then every internal link needs to be refactored using the target file slug, optionally appending the anchor if required.
#+begin_quote
Before:
#+begin_example
As I mentioned in my [[On-boarding for Code in Place][last post]]...
#+end_example

After:
#+begin_example
As I mentioned in my {{{relref(last post,scp-onboarding)}}}, I'm leading a section...
#+end_example
#+end_quote

I ended up starting with a manual conversion, which was very tedious, but does
result in a more stable site with links that are properly managed by Hugo's
content engine, insulating the site from future Org mode changes.

Eventually I used [[https://github.com/HaoZeke/haozeke.github.io/blob/src/scripts/convert_links.py][a script]] to generate most of the ~relref~ changes, with the
exception of titles with code snippets:
#+begin_src bash
pixi s
python ./scripts/convert_links.py content-org/all-posts.org all-posts-converted.org
#+end_src

*** Update [2026-07-10]
The ~ox-hugo~ same-file cross-post failure under Org 9.7+ is fixed in the
exporter (path/search-option split + restoring ~file:~ after interpret). This
site loads that fix from ~setup/vendor/ox-hugo.el~. Internal post links in
~content-org/all-posts.org~ were converted back to normal Org links
(~[[Post title][description]]~); reverse of the ~relref~ macro conversion above
via ~scripts/convert_relref_to_org_links.py~. The historical workaround text in
this snippet is unchanged.

** DONE ~pixi~ global environments consistent formatting :tools:R:pixi:
CLOSED: [2025-09-25 Thu 15:46]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: pixi-global-consistent
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
Formatting ~R~ code which is typically managed in per-project environments can be quite annoying [fn:: since I do not keep ~R~ installed system-wide]. For formatting, defining a global environment with ~pixi~ makes life easier.
...
** DONE Co-authoring Github commits with ~pixi~ :git:pixi:workflow:
CLOSED: [2026-02-05 Thu 11:02]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: pixi-shebang-ghcoauth
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
Seth Larson has a nice [[https://sethmlarson.dev/easy-github-co-authored-by][short post]] on handling co-authors on Github without
...
as the [[https://en.wikipedia.org/wiki/Shebang_(Unix)][shebang]] works well [fn:: now managed by ~chezmoi~ within [[https://rgoswami.me/posts/mig-dots-chezmoi/][my Dotfiles]]]:
#+begin_src bash
...
#+begin_src bash
ghcoauthby HaoZeke
Co-authored-by: Rohit Goswami <4336207+HaoZeke@users.noreply.github.com>
# more practically
git commit -m 'feat(mta): add amazing thing' \
    --trailer '$(coauthoredby HaoZeke)'
#+end_src
** DONE Notify on ~pdf~ compilation
CLOSED: [2025-10-04 Sat 01:52]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: notify-pdf-compilation
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
...
watchexec \
  ...
  -- \
  sh -c '
    # Main source file – change this if you compile a different document.
    src="rgDraft_I.tex"
    pdf="${src%.tex}.pdf"

    # Run Tectonic with shell‑escape enabled.
    if tectonic -X compile -Z shell-escape "$src"; then
      notify-send \
        -u normal \
        -i document-save \
        "✅  TeX compiled" \
...
** DONE Lightweight C++ config with ~compile_flags.txt~ :cpp:config:
CLOSED: [2026-01-22 Thu 19:35]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: compile-flags-txt
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
For CMake driven C/C++ projects, ~compile_commands.json~ generation can be
slightly painful, especially for mixed language projects [fn:: like [[https://metatensor.org][metatensor]]].
A simpler route involves ~clangd~ support for ~compile_flags.txt~, which
includes simple newline-separated list of flags.

For my recent contributions to the mini-metatomic project, the file looked like
this:
#+begin_src text
-I/usr/include/python3.13/
-I./metatomic-core/include
-fopenmp
#+end_src
Which allows LSP support in ~emacs~ to function correctly without a
~compile_commands.json~.

** DONE Disabling Rust auto-formatting in Emacs per-project :emacs:rust:
CLOSED: [2026-01-22 Thu 19:35]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: dir-locals-formatting-rust
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
...
semantics [fn:: like [[https://metatensor.org][metatensor]]]. Disabling this generally requires setting
...
((rustic-mode . ((rustic-format-on-save . nil)
                 (eval . (setq-local +format-with-lsp nil)))))
((nil . ((eval . (progn
...
configuration.
** DONE Using ~pixi~ environments for orgmode notebooks :pixi:orgmode:workflow:
CLOSED: [2026-01-29 Thu 08:20]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: pixi-orgmode-notebooks
:EXPORT_FILE_NAME: index
...
#+begin_src conf
#+PROPERTY: header-args:bash :session vizSH :results silent
#+end_src

Initialize the environment within the session:
#+begin_src bash :session vizSH
export GITROOT=$(git rev-parse --show-toplevel)
pixi shell -e eon
#+end_src
*** Python
Basically the executable must be set to the right environment ~:python
(string-trim (shell-command-to-string "pixi run \"which python\""))~. This means
a working setup for a per-file notebook can be:

#+begin_src conf
+PROPERTY: header-args:python :session onePy
+PROPERTY: header-args:python :results output :exports both
+PROPERTY: header-args:python :cache yes :tangle pylog.py
+PROPERTY: header-args:python :python "pixi run -e eon python"
#+end_src
...
#+end_src
** TODO Branching development with ~subrepo~
#+begin_src bash
#+end_src
** DONE Trivial LLMs on old hardware :ai:hardware:workflow:
CLOSED: [2025-11-22 Sat 19:46]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: trivial-llms-old-hardware
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
Apropos nothing. R1 represents the remote machine. L1 denotes the local machine.
Tailscale connects the two.
...
Tunneling the API port over SSH via Tailscale ensures a secure connection
without exposing the Ollama instance to the open web. This workflow facilitates
the usage of substantial models on lightweight clients, effectively offloading
the heavy lifting to dedicated hardware, and making good use of older machines.

Also see this post for [[https://markaicode.com/gemma-3-single-gpu-ollama-optimization/][more details]].
** DONE ~orgmode~ for workbench lessons :workflow:orgmode:
CLOSED: [2026-02-10 Tue 16:50]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: orgmode-workbench
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
#+begin_quote
...
(defun export-as-workbench-md ()
  "Export the current Org buffer to Carpentries-style Markdown using Pandoc."
  (interactive)
  (let* ((source-file (buffer-file-name))
         (output-file (concat (file-name-sans-extension source-file) ".md"))
         ;; Enable fenced_divs to convert #+begin_name blocks to ::: name
         (pandoc-cmd (format "pandoc %s -f org -t markdown+fenced_divs -o %s"
...
While this works for exporting to the same file name, with only the extension
changed, using the ~EXPORT_FILE_NAME~ ends up trying to export an ~orgmode~
block containing the variable, so a bit more effort is needed.
...
(defun export-as-workbench-md ()
  ...
  (let* ((src (buffer-file-name))
         ...
         (cmd (format "grep -vE '^#\\+(EXPORT_FILE_NAME|PROPERTY):' %s | pandoc -f org -t markdown-smart+fenced_divs --shift-heading-level-by=1 -o %s"
...
# End:
#+end_src
Which together, correctly render lessons for ~Rscript -e "sandpaper::serve()"~ as [[https://pydissem.rgoswami.me][seen here in this Python lesson]].
** DONE ~conda-smithy~ notes for off-brand ~conda~ packages :build:packaging:conda:
CLOSED: [2026-02-01 Sun 06:50]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: off-brand-conda
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
*** Migrations
For dependency resolution and CI workflow snippet generation, rather than
hard-code dependencies, often we prefer "migration" files, as discussed in [[https://conda-forge.org/docs/maintainer/knowledge_base/#migrators-and-migrations][the
knowledge base]]. Most commonly these define version constraints, and are ~conda
smithy rerender~'d to ensure maximum compatibility.

Instead of using the upstream variants, these may also be defined by hand. For
instance, consider handling an update to PyTorch 2.9 (e.g. from this [[https://github.com/metatensor/gromacs-metatomic-feedstock/blob/main/.ci_support/migrations/pytorch29.yaml][GROMACS
metatomic package]]):
#+begin_src yaml
# .ci_support/migrations/pytorch29.yaml
__migrator:
  build_number: 1
  kind: version
  migration_number: 1
...
#+end_src
** DONE ~julia~ and the ~conda~ libraries :julia:packaging:conda:
CLOSED: [2026-02-17 Tue 19:06]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: julia-conda-libopenlibm
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
Environments with ~julia~ often seem to have trouble with ~Pkg.install()~ and other commands with
#+begin_src bash
ERROR: Unable to load dependent library /path/to/project/.pixi/envs/docs/bin/../lib/julia/libopenlibm.so
Message:/path/to/project/.pixi/envs/docs/bin/../lib/julia/libopenlibm.so: cannot enable executable stack as shared object requires: Invalid argument
#+end_src

Not sure why this happens yet, but the fix is:
#+begin_src bash
# needs to be set for each env!
execstack -c ./.pixi/envs/default/lib/julia/libopenlibm.so
# e.g. for a doc env...
execstack -c ./.pixi/envs/doc/lib/julia/libopenlibm.so
#+end_src
** DONE PyTorch woes across ~conda~ and ~pip~ :pytorch:packaging:conda:
CLOSED: [2026-02-17 Tue 23:11]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: pytorch-conda-pip
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
Sometimes, within the ~pixi~ enviroment, multiple ~torch~ includes can clog the build. In this case:
#+begin_src bash
rm -rf .pixi/envs/$ENVNAME/include/torch/csrc/api/include/
#+end_src
Probably something to be fixed in the upstream build system..
** DONE Double-checked locking for SLURM container tasks :hpc:ci:slurm:
CLOSED: [2026-02-27 Fri 11:40]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: slurm-double-checked-locking
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:

When a SLURM job launches N tasks from the same container image, every rank runs the entrypoint in parallel. Commands like ~ldconfig~ [fn:: Needed for a [[https://github.com/metatensor/metatrain/pull/1056][CSCS workaround on metatrain]]] that write to shared paths (~/etc/ld.so.cache~) collide: multiple tasks try to write the same file simultaneously, the losers get a corrupt or truncated cache, and the job step crashes.

One slightly naive fix involves serialization with ~flock~:
#+begin_src yaml
- flock /tmp/ldconfig.lock -c "ldconfig || true"
#+end_src
...
#+begin_src yaml
- flock /tmp/ldconfig.lock -c "test -f /tmp/ld_$CI_JOB_ID || { ldconfig || true; touch /tmp/ld_$CI_JOB_ID; }"
#+end_src
...
** DONE Standalone TikZ to PNG with ~tectonic~ and ~pixi~ :tex:pixi:build:
CLOSED: [2026-02-24 Mon 12:00]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: tikz-tectonic-pixi
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
For blog posts and slides I normally embed Graphviz DOT directly in ~orgmode~ and let the export handle rendering. That works well until a paper schematic needs inline math or a font family that does not cover enough Unicode glyphs [fn:: [[https://indestructibletype.com/Jost.html][Jost]], for instance, is wonderful for headings but lacks the math operators and Greek letters a pipeline diagram inevitably requires.]. At that point, TikZ in a ~standalone~ document class is the path of least resistance: full LaTeX math rendering, arbitrary fonts through the TeX ecosystem, and tightly cropped output without manual trimming.
...
TIKZDIR="v1/pccp/imgs/tikz"
OUTDIR="v1/pccp/imgs/tikz"
DPI="${DPI:-300}"

for texfile in "${TIKZDIR}"/*.tex; do
    name="$(basename "${texfile}" .tex)"
    echo "Building ${name}..."
    tectonic "${texfile}" --outdir "${OUTDIR}"
    pdftoppm -png -r "${DPI}" -singlefile \
        "${OUTDIR}/${name}.pdf" "${OUTDIR}/${name}"
...
#+begin_src toml
[tasks]
tikz-figs = { cmd = "bash v1/pccp/imgs/tikz/build_tikz.sh" }

[feature.texbuild.dependencies]
tectonic = "*"
poppler = "*"

[environments]
texbuild = {features = ["texbuild"]}
#+end_src
...
** DONE Deterministic PyTorch regression tests :pytorch:testing:python:
CLOSED: [2026-03-11 Wed 12:00]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: pytorch-deterministic-regression
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
...
#+begin_src python
torch.testing.assert_close(result, expected, rtol=5e-3, atol=0.05)
#+end_src
...
See [[https://github.com/metatensor/metatrain/pull/1070][metatensor/metatrain#1070]] for the full changeset.
** DONE Togglable inline review comments in ~orgmode~ LaTeX export :orgmode:tex:workflow:
CLOSED: [2026-03-27 Thu 17:30]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: orgmode-inline-review-comments
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
When co-authoring a paper in ~orgmode~ while collaborators edit the LaTeX on Overleaf, the review loop gets awkward: comments live in the ~.tex~, changes happen in the ~.org~, and neither side sees the other's annotations. A togglable inline comment system bridges this gap: each collaborator gets a colored command, and a single boolean flag strips all comments for submission.

In the ~orgmode~ preamble, define a conditional block:
#+begin_src org
,#+LATEX_HEADER: \usepackage{xcolor}
,#+LATEX_HEADER: \newif\ifshowreviewnotes
,#+LATEX_HEADER: \showreviewnotestrue  % change to \showreviewnotesfalse for submission
,#+LATEX_HEADER: \ifshowreviewnotes
,#+LATEX_HEADER:   \newcommand{\mg}[1]{{\color{cyan}[MG: #1]}}
,#+LATEX_HEADER:   \newcommand{\rg}[1]{{\color{green!50!black}[RG: #1]}}
,#+LATEX_HEADER: \else
,#+LATEX_HEADER:   \newcommand{\mg}[1]{}
,#+LATEX_HEADER:   \newcommand{\rg}[1]{}
,#+LATEX_HEADER: \fi
...
Adding a new reviewer is just another ~\newcommand~ inside the ~\ifshowreviewnotes~ block -- pick a color and a two-letter prefix.

Then scatter comments inline using ~orgmode~'s ~@@latex:...@@~ syntax:
#+begin_src org
The reflector inverts the force component along $\hat{\mathbf{d}}$.
@@latex:\mg{what is d here? the dimer axis or the NEB tangent?}
\rg{Clarified: $\hat{\mathbf{d}} = \hat{\mathbf{N}}$ for the dimer.
Added a sentence defining the notation.}@@
#+end_src

This renders as colored bracketed text in the PDF:
- =[MG: what is d here?]= in cyan
- =[RG: Clarified: ...]= in green

The comment and reply sit right next to the text they refer to, so the conversation is spatially grounded. When a collaborator pastes their Overleaf ~\mg{...}~ comments, you can drop them into the ~.org~ with the response and the actual fix all in the same spot.

For submission, flip the flag:
#+begin_src diff
- \showreviewnotestrue
+ \showreviewnotesfalse
#+end_src

All ~\mg~, ~\rg~, etc. become no-ops -- zero output, no stale comment text leaking into the published PDF, and no need to hunt through the source deleting annotations. The comments remain in the ~.org~ as a permanent record of the review conversation.

This pairs well with ~latexdiff~ for generating the highlighted-changes PDF that journals want alongside the clean submission.
** DONE Continuing numbered Org lists across intervening paragraphs :orgmode:tex:workflow:
CLOSED: [2026-06-12 Fri 20:45]
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: orgmode-enumerate-counter-cookies
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
Org mode numbered lists can mislead when ordinary paragraphs sit between items.
This often appears when each numbered item has a short explanatory paragraph.
The source can look numbered:

#+begin_src org
1. *[First item]* ...

Extra notes for the first item.

2. *[Second item]* ...

Extra notes for the second item.

3. *[Third item]* ...
#+end_src

but Org emits a fresh ~enumerate~ block after each intervening paragraph, so
LaTeX prints each later item as ~1.~.

The fix uses an Org counter cookie. Place the cookie after a real ordered-list
marker:
...
Extra notes for the third item.

1. [@4] *[Fourth item]* ...
#+end_src

The tempting shorthand fails:

#+begin_src org
[@4]. *[Fourth item]* ...
#+end_src

That line lacks an ordered-list marker. With Org mode 9.7 it exports literally
as ~[@4].~ in the LaTeX/PDF path.

The corrected export carries an explicit LaTeX counter before each item:

#+begin_src tex
\begin{enumerate}
\setcounter{enumi}{3}
\item \textbf{{[}Fourth item]} ...
\end{enumerate}
#+end_src

For a quick regression check, inspect both the intermediate ~.tex~ and the
rendered PDF text:

#+begin_src bash
make export
rg -F '\setcounter{enumi}{3}' document.tex
pdftotext -layout document.pdf - \
  | rg '^[[:space:]]*4\. \[Fourth item'
#+end_src

That catches both failures: no LaTeX counter, and a PDF that prints another
~1.~ where the fourth item belongs.
** DRAFT Keeping a laptop responsive under ~rust-analyzer~ memory pressure :linux:rust:systemd:zram:cgroups:oomd:chezmoi:
:PROPERTIES:
:EXPORT_HUGO_BUNDLE: rust-laptop-freeze-mitigation
:EXPORT_FILE_NAME: index
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :toc false :comments true
:END:
A 30 G laptop running ~rust-analyzer~ and ~cargo check~ on a multi-crate workspace freezes hard enough that even the terminal stops accepting input. The kernel stays alive; every userspace process blocks on I/O, so ~Ctrl+C~ and ~kill~ cannot land. The diagnostic that settles it in one line is ~free -h~:

#+begin_src text
               total        used        free      shared  buff/cache   available
Mem:            30Gi        23Gi       900Mi       607Mi        10Gi       7.3Gi
Swap:           14Gi       432Mi        14Gi
#+end_src

23 GiB used, 7.3 GiB available, and ~swap at 432 MiB of 14 GiB~. That inversion is the bug. The kernel ran out of reclaimable page cache before it touched swap, so it kept evicting file-backed pages that ~rustc~ immediately re-read -- I/O thrash against ~target/~ with every build.

The usual desktop-tuning advice lowers ~vm.swappiness~ (10 is a popular value). That backfires on compile workloads: the large-and-idle anon memory belongs to ~rust-analyzer~ and the small-and-hot page cache holds ~*.rlib~ files, source trees, and build-script outputs. Low swappiness forces the kernel to evict the cache -- the opposite of what a compile loop needs.

~zram~ makes the correct fix trivial. Compressed in-RAM swap runs at ~1 microsecond latency with zstd at roughly 3:1 compression, which puts ~/dev/zram0~ at priority 100 above any disk swapfile, and lets the kernel page out idle anon memory without touching the filesystem:
...
The matching VM settings invert the low-swappiness rule. ~vm.page-cluster = 0~ disables swap read-ahead, which is pointless for RAM-to-RAM transfers. ~vm.compaction_proactiveness = 0~ disables the background compaction thread that can stall userspace for hundreds of milliseconds when high-order allocations materialize. ~vm.min_free_kbytes = 262144~ gives ~kswapd~ an emergency reserve of ~256 MiB~ so it never allocates under pressure:
...
~sudo sysctl --system~ makes the kernel rebalance within seconds. Same workload, ~free -h~ after the switch:

#+begin_src text
               total        used        free      shared  buff/cache   available
Mem:            30Gi        17Gi       415Mi       600Mi        14Gi        13Gi
Swap:           14Gi       2.1Gi        12Gi
#+end_src

Available memory moves from 7.3 G to 13 G, page cache grows to 14 G, and 2.1 G of idle anon pages sit compressed inside ~/dev/zram0~. The terminal-freezing phase ends.

A per-LSP ceiling catches the remaining failure mode where one runaway workspace pushes ~rust-analyzer~ past 14 G RSS. A dedicated user slice holds the cap in a single config file:

#+begin_src conf
# ~/.config/systemd/user/rust-analyzer.slice
[Slice]
MemoryHigh=10G
MemoryMax=14G
MemorySwapMax=6G
...
A ~PATH~ shim launches the real binary into that slice without touching any editor config. Every editor that resolves ~rust-analyzer~ through ~PATH~ -- ~eglot~, ~lsp-mode~, ~nvim-lspconfig~, ~VS Code~, ~Zed~ -- picks up the cap transparently:

#+begin_src bash
# ~/.local/bin/rust-analyzer
#!/usr/bin/env bash
set -u
REAL=$(rustup which rust-analyzer 2>/dev/null)
[ -x "$REAL" ] || REAL=/usr/bin/rust-analyzer
...
~rustup which~ resolves the toolchain-specific binary directly, so the shim bypasses ~PATH~ and cannot recurse into itself. ~systemd-cgls --user | grep rust-analyzer~ confirms the process sits inside ~rust-analyzer.slice~; ~systemctl --user show rust-analyzer.slice -p MemoryCurrent~ shows live RSS.

The kernel OOM killer fires only when ~MemAvailable~ approaches zero, which is after the thrash has already started. ~systemd-oomd~ watches kernel PSI -- the cumulative time in the last 10 seconds that tasks spent blocked on memory -- and kills 10-30 seconds earlier. Arch ships it but leaves it disabled:

#+begin_src conf
# /etc/systemd/oomd.conf.d/aggressive.conf
[OOM]
...
After ~sudo systemctl enable --now systemd-oomd~, any user session whose memory-pressure PSI exceeds 50% for 20 seconds loses its largest process to a SIGKILL instead of a multi-minute freeze. ~oomctl~ lists monitored slices; ~journalctl -u systemd-oomd~ shows kill history with per-process scores and the PSI numbers that triggered the action.

Magic SysRq stays as a final escape hatch for the rare case where something wedges the compositor or GPU path and neither oomd nor the kernel OOM can reach it, but the keyboard still reaches the kernel:
...
=Alt+SysRq+F= forces an OOM kill on the largest memory consumer; =R E I S U B= (with ~1 s gaps while holding =Alt+SysRq=) reboots cleanly and flushes the journal.

One operational footnote: a pre-existing disk swapfile at priority ~-1~ may still hold pages that were written out before zram came up. ~swapon --show~ reveals this (non-zero ~USED~ against the file entry). ~sudo swapoff /path/to/swapfile~ followed by ~sudo swapon /path/to/swapfile~ drains those pages back through RAM so zram absorbs the cold ones via normal reclaim -- a one-time migration after enabling zram.

All six files live under ~chezmoi~ at [[https://github.com/HaoZeke/dotfiles][HaoZeke/dotfiles]]: ~dot_config/systemd/user/rust-analyzer.slice~, ~dot_local/bin/executable_rust-analyzer~, and a ~.chezmoiscripts/run_onchange_linux_memory_tuning.sh.tmpl~ that installs ~/etc/sysctl.d/99-memory.conf~, ~/etc/systemd/zram-generator.conf~, ~/etc/systemd/oomd.conf.d/aggressive.conf~, and ~/etc/systemd/system/user-.slice.d/50-oomd.conf~ under ~sudo~ with ~cmp -s~ guards so unchanged content never re-prompts. Reimaging a laptop restores the full stack with one ~chezmoi apply~.
* Footnotes
