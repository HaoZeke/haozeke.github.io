...
The process has two parts. First, we need the ~relref~ macro in the Org file:
#+begin_src orgmode
...
exception of titles with code snippets[fn:: and footnotes, where absolute paths work best]:
#+begin_src bash
pixi s
