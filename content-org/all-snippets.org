...
(defun export-as-workbench-md ()
  ...
  (let* ((source-file (buffer-file-name))
         (keywords (org-collect-keywords '("EXPORT_FILE_NAME")))
         (export-prop (cadr (assoc "EXPORT_FILE_NAME" keywords)))
         
         ;; Fallback to source filename if no export name is defined
         (output-file (or export-prop 
                          (concat (file-name-sans-extension source-file) ".md")))

         ;; Enable fenced_divs to convert #+begin_name blocks to ::: name
         (pandoc-cmd (format "pandoc %s -f org -t markdown-smart+fenced_divs -o %s"
                             (shell-quote-argument source-file)
                             (shell-quote-argument output-file))))
    (if source-file
        (progn
          (save-buffer)
          ;; Ensure the directory exists if using a relative path like ./episodes/
...
Along with an additional macro for the challenges: 
#+begin_src
