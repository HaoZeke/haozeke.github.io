...
Basically followed along the directions for [[https://github.com/rien/reStream?tab=readme-ov-file][reStream]] (~toltec~ doesn't support OS versions 3.x and up).
...
The only slight point of deviation was using ~chezmoi~ to grab the local script via ([[https://github.com/HaoZeke/Dotfiles/tree/chezmoi][all dotfiles]]):
#+begin_src sh
...
With this, finally getting the shared window is a snap:
#+begin_src sh
